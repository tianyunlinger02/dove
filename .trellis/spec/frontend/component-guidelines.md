...
- Every Skill accepts optional user text and uses the same Dove agent behavior with available and approved host file, coding, execution, writing, figure, and research tools directly.
...
- `research` advances a confirmed goal through Dove's default multi-round progression; there is no separate Auto Skill or command.
- Ambient routing never selects a separate Auto Skill or command.
...
- Review follows the requested author-side scientific self-check, Conditional Delivery Review, `dove-review` handoff, faithful return import, or context inspection; author-side scientific self-check is read-only, while `dove-review` is a genuinely isolated persistent context with frozen near-submission materials.
...
3. for `dove-review` handoff preparation, assemble a frozen handoff with the current full paper, authoritative LaTeX source and actual compiled output, actual submission appendices or supplements, and other venue-facing files that would accompany submission, identify the target venue, then invoke a genuinely isolated persistent `dove-review` context when the host can provide one;
4. for each independent round, keep the Reviewer scope to the explicit frozen list for that round; old Reviews, historical returns, author private transcript, and unlisted materials are not visible by default;
5. for import, locate the corresponding Review document and preserve the actual user-obtained Markdown faithfully with a clear boundary from existing text, without triggering external search merely because Review was invoked; and
6. for inspection, read and report existing Review context without creating a new document, handoff, or unnecessary external search.

Do not automatically mix author response or revision into import. Add author interpretation only when requested; substantive response and changes remain Rebuttal work. Review findings are evidence to analyze, not direct rewrite or claim-narrowing triggers: if a feasible high-level author-side action exists, do that first; narrow only when evidence or a real boundary requires it. Changing the confirmed mainline, contribution, or completion meaning belongs to the user. Dove does not package a native user-switchable reviewing role or certify `dove-review`; it may invoke only a genuinely isolated persistent `dove-review` context under the frozen handoff boundary, and it never treats author-side scientific self-check as independent external review.
...
Project initialization creates software integration, `.dove/install/manifest.json`, and the minimal researcher-owned `RESEARCH.md` bootstrap in one transaction; research paths are not manifest-managed resources. Claude integration also declares only `.mcp.json#/mcpServers/dove-paper-search`, `.mcp.json#/mcpServers/exa`, a project-scoped `permissions.deny` entry for built-in `WebFetch`, and hidden support Skills for pinned external paper acquisition and hosted ordinary webpage retrieval. Built-in `WebSearch` remains available. Dove does not install its runtime, write credentials, approve project trust, or provide CLI/shell/fetch fallbacks. Optional ordinary `DOCTOR.md` feedback about Dove itself remains under `.dove/install/`; there is no Doctor JSON state, issue lifecycle, research telemetry, or scientific health score. Installation hashes protect managed bytes only.
