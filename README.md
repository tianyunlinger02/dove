...
Dove records notes when you ask, when the research direction or conclusion changes, or when saving evidence and continuation context is genuinely useful. Explicit update, reinstall, uninstall, and the read-only SessionStart inspection preserve existing `.dove/research/**` content, `.dove/reviews/**` review records, and `.dove/runs/**` local run receipts. Dove no longer installs `UserPromptSubmit`, hidden intake, or a replacement per-prompt hook. When the project has no existing user status line, Claude integration installs a read-only Dove line showing the host model, total context capacity, remaining context percentage, absolute project path, Git branch, and current Claude session duration. The branch is not a scientific-mainline claim and the duration is not cumulative research time. Explicit `dove update` replaces local edits to manifest-owned integration and tells you what was replaced; SessionStart only inspects and may suggest that explicit update. After compact/resume it supplies only a small read-only facts card, not a reconstructed research mainline; startup/clear gets no research card.
