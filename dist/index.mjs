...
var injectedVersion = true ? "3.0.4" : null;
...
var DOVE_RESEARCH_CLAIM_STANDING_BOUNDARY = "Preserve certainty, causality, scope, generality, quantitative qualifiers, and novelty boundaries unless inspected evidence changes the judgment; user decisions may change the goal or requested expression, not establish stronger facts. Say what changed and why.";
...
function contract(slug) {
  if (slug === "research") return capabilityContract({
    ...
    actions: [
      ...
      action("research-progression", `${RESEARCH_ADVANCE} For a submission goal, use Review to establish the current whole-paper judgment before declaring completion.`),
...
function renderBody(command, heading, hostId = null) {
  ...
  const shared = hostId === "dsh" ? [
    ...
    command.id === "dove.status" ? "For Status, use these principles only to inspect and report; do not execute research actions or maintain documents." : renderDoveAuthorStanceSection({ compact: true, coveredText })
  ].join("\n\n") : "";
