...
// src/core/research-constitution.mjs
var freeze = (value) => {
  if (value === null || typeof value !== "object" || Object.isFrozen(value)) return value;
  for (const item of Array.isArray(value) ? value : Object.values(value)) freeze(item);
  return Object.freeze(value);
};
var RESEARCH_CONSTITUTION_GUARANTEE_CLASSES = freeze(["hard-invariant", "transition-precondition", "required-assessment"]);
var RESEARCH_CONSTITUTION_OWNER_IDS = freeze(["research-planner", "evidence-owner", "experiment-owner", "author", "public-report-renderer"]);
var RESEARCH_CONSTITUTION_EVIDENCE_REQUIREMENTS = freeze(["static-contract-evidence", "current-recorded-evidence", "experiment-record-evidence"]);
var RESEARCH_CONSTITUTION_VERIFICATION_REQUIREMENTS = freeze(["none"]);
var RESEARCH_CONSTITUTION_VERIFICATION_CATEGORIES = freeze(["unit-contract-test", "integration-contract-test", "assessment-contract"]);
function clause(number2, clauseId, title, canonicalOwner, enforcementPoints, evidenceRequirement, guaranteeClass, category, reference) {
  return freeze({ number: number2, clauseId, title, canonicalOwner, enforcementPoints, evidenceRequirement, verificationRequirement: "none", guaranteeClass, verification: { category, reference } });
}
var RESEARCH_CONSTITUTION = freeze([
...
var RESEARCH_AUTHORITY_RECORD_TYPES = freeze(["Workspace", "Mission Contract", "ResearchDecision", "Receipt/lineage records", "Experiment records", "Claim records", "Narrative/Public report"]);
function authority(authorityId, recordType, canonicalOwner, authorityScope, mutationAuthority, fieldAuthorities) {
  return freeze({ authorityId, recordType, canonicalOwner, authorityScope, mutationAuthority, fieldAuthorities });
}
function fields(names, sourceOfTruth, writeAuthority, verificationAuthority) {
  return freeze({ fields: names, sourceOfTruth, writeAuthority, verificationAuthority });
}
var RESEARCH_AUTHORITY_OWNERSHIP_MATRIX = freeze([
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        for (const entry of entries) {
          const childPath = `${normalized3}/${entry.name}`;
...
function sealed(value, fields2, label) {
  plain(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
...
function sealed2(value, fields2, label) {
  plain2(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
...
function assertFields(value, fields2, label) {
  assertObject(value, label);
  const allowed2 = new Set(fields2);
...
function assertSealed(value, fields2, label) {
...
function sealed3(value, fields2, label) {
  plain3(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
...
var RESEARCH_DECISION_KINDS = RESEARCH_DECISION_DISPOSITIONS;
...
function sealed4(value, fields2, label) {
  plain4(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
}
function exact(value, fields2, label) {
  sealed4(value, fields2, label);
  const missing = [...fields2].filter((field) => !Object.hasOwn(value, field));
...
function currentResearchDecisionHead(decisions, options = {}) {
  return validateResearchDecisionChain(decisions, options).at(-1) ?? null;
}
...
function sealed5(value, fields2, label) {
  object(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
  return value;
...
function assertSealed2(value, fields2, label) {
...
function assertSealed3(value, fields2, label) {
...
function currentOwnedJson(root, relativePath, missionId2, receiptLedger, fields2, label) {
  ...
  assertSealed3(value, fields2, label);
  return value;
}
...
function assertSealed4(value, fields2, label) {
...
function readCurrentResearchDecision(root, missionId2) {
  const normalizedMissionId = safeId8(missionId2, "missionId");
...
function sealed6(value, fields2, label) {
  plain5(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
...
var PROJECT_IDENTITY_SCHEMA_VERSION = 2;
...
function allowed(value, fields2, label) {
...
function assertSealedDomainArgs(args, fields2, label) {
  if (!args || typeof args !== "object" || Array.isArray(args)) throw new Error(`${label} arguments must be a plain object.`);
  const unknown = Object.keys(args).filter((field) => !fields2.has(field));
...
function sealed7(value, fields2, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`${label} must be a plain object.`);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
  return value;
...
function assertSealedRecord(value, fields2, label) {
...
function normalizeObjects(value, fields2, label, normalizeItem) {
  assertJsonArray(value, label);
  return Object.freeze(value.map((item, index) => {
    const itemLabel = `${label}[${index}]`;
    assertSealedRecord(item, fields2, itemLabel);
    for (const field of fields2) if (!Object.hasOwn(item, field)) throw new Error(`${itemLabel} requires $.${field}.`);
...
function buildResearchNarrative(value) {
...
var ARTIFACT_FIELDS4 = /* @__PURE__ */ new Set(["path", "kind", "sha256"]);
var VALIDATION_INPUT_FIELDS = new Set(VALIDATION_FIELDS);
var CRITERION_FIELDS4 = /* @__PURE__ */ new Set(["criterionId", "evidenceRefs"]);
var RECEIPT_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,127}$/u;
var HASH_PATTERN3 = /^[0-9a-f]{64}$/u;
var EVIDENCE_REF_PATTERN = /^(artifact|validation|source):(.+)$/u;
...
function hashString(value, label) {
  const hash9 = nonEmptyString2(value, label).toLowerCase();
  if (!HASH_PATTERN3.test(hash9)) {
...
function parseProducedAt(value) {
  ...
  const timestamp3 = Date.parse(producedAt);
  if (!Number.isFinite(timestamp3) || new Date(timestamp3).toISOString() !== producedAt) {
    throw new Error("producedAt must be an exact ISO-8601 timestamp.");
  }
  return producedAt;
}
function executionReceiptPath(receiptId) {
  return path19.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`);
}
...
function normalizeArtifacts2(root, mission, value) {
  if (value === void 0) return [];
  if (!Array.isArray(value)) throw new Error("artifacts must be an array.");
  const seen = /* @__PURE__ */ new Set();
  const artifacts = value.map((item, index) => {
    const label = `artifacts[${index}]`;
    assertAllowedFields(item, ARTIFACT_FIELDS4, label);
    ...
    const sha25613 = hashString(item.sha256, `${label}.sha256`);
    const inspected = inspectHashedFile(root, rawPath, sha25613, label);
    ...
    return { path: inspected.path, kind, sha256: sha25613 };
  });
  return artifacts;
}
...
function normalizeValidations(root, value) {
  ...
  return value.map((item, index) => {
    ...
    const target = inspectValidationTarget(root, nonEmptyString2(item.targetReference, `${label}.targetReference`), hashString(item.targetHash, `${label}.targetHash`), `${label}.targetReference`);
...
function typedReferenceEvaluation(root, missionId2, reference) {
...
function normalizeCriteria2(root, mission, value, artifacts, validations, ledger, receiptId) {
  ...
  const criteria = value.map((item, index) => {
    ...
    assertAllowedFields(item, CRITERION_FIELDS4, label);
    ...
    const evidenceRefs2 = item.evidenceRefs.map((reference, evidenceIndex) => {
      const normalized3 = nonEmptyString2(reference, `${label}.evidenceRefs[${evidenceIndex}]`);
      if (artifactRefs.has(normalized3)) {
        evidenceBindings.push({ reference: normalized3, sha256: artifacts.find((artifact) => `artifact:${artifact.path}` === normalized3).sha256, receiptId });
        return normalized3;
      }
      if (validationRefs.has(normalized3)) {
        evidenceBindings.push({ reference: normalized3, sha256: validations.find((validation) => `validation:${validation.reference}` === normalized3).outputHash, receiptId });
        return normalized3;
      }
      if (normalized3.startsWith("artifact:")) {
        const artifactPath = normalized3.slice("artifact:".length);
        const owner = ledger.currentOwnership.find((item2) => item2.path === artifactPath);
        if (!owner || owner.missionId !== mission.missionId) throw new Error(`${label}.evidenceRefs[${evidenceIndex}] is not current mission-owned artifact evidence: ${normalized3}.`);
        const inspected = inspectHashedFile(root, artifactPath, owner.sha256, `${label}.evidenceRefs[${evidenceIndex}]`);
        evidenceBindings.push({ reference: normalized3, sha256: inspected.sha256, receiptId: owner.receiptId });
        return normalized3;
      }
      const evaluation = typedReferenceEvaluation(root, mission.missionId, normalized3);
      if (evaluation.eligible === true && HASH_PATTERN3.test(String(evaluation.evidenceSha256 ?? ""))) {
        evidenceBindings.push({ reference: normalized3, sha256: evaluation.evidenceSha256, receiptId: evaluation.owner?.receiptId ?? receiptId });
        return normalized3;
      }
      throw new Error(`${label}.evidenceRefs[${evidenceIndex}] is not current eligible typed evidence: ${normalized3} (${evaluation.reason ?? "unresolved"}).`);
    });
    if (new Set(evidenceRefs2).size !== evidenceRefs2.length) throw new Error(`${label}.evidenceRefs contains duplicates.`);
    return { criterionId, evidenceRefs: evidenceRefs2, evidenceBindings };
  });
  return criteria;
}
function validateExecutionReceipt(root, args = {}) {
  ...
  const receiptId = safeId10(args.receiptId, "receiptId");
  ...
  if (!fs14.existsSync(path19.resolve(root, missionRelativePath))) {
    throw new Error(`Mission does not exist: ${missionId2}.`);
  }
  ...
  if (contractDigest !== currentContract.contractDigest) throw new Error(`contractDigest does not match the current mission contract for ${missionId2}.`);
  ...
  if (mutationContext ? mutationContext.fileExists(receiptRelativePath) : fs14.existsSync(path19.resolve(root, receiptRelativePath))) {
    throw new Error(`Execution receipt id is already occupied: ${receiptId}.`);
  }
  const artifacts = normalizeArtifacts2(root, mission, args.artifacts);
  ...
  const criteriaSatisfied = normalizeCriteria2(root, mission, args.criteriaSatisfied, artifacts, validations, workspace.receiptLedger, receiptId);
...
function normalizeObjects2(value, fields2, label, limit, normalizeItem) {
  assertJsonArray2(value, label);
  assertBoundedLength(value, limit, label);
  return Object.freeze(value.map((item, index) => {
    const itemLabel = `${label}[${index}]`;
    assertSealedRecord2(item, fields2, itemLabel);
    return Object.freeze(normalizeItem(item, itemLabel));
  }));
}
...
function buildProjectResearchNarrative(value) {
...
function sealed9(value, fields2, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`${label} must be a plain object.`);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length > 0) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
  return value;
...
function reviewPath(reviewId) {
  return path20.posix.join(ARTIFACT_PATHS.reviewsDir, `${reviewId}.json`);
}
function reportPath(reviewId) {
  return path20.posix.join(ARTIFACT_PATHS.reviewsDir, `${reviewId}.report.md`);
}
function canonicalReviewRecordPath(value, label) {
  const candidate = domainNonEmptyText(value, label);
  const fileName = path20.posix.basename(candidate);
  if (!/^review-[a-f0-9]{24}\.json$/u.test(fileName) || candidate !== path20.posix.join(ARTIFACT_PATHS.reviewsDir, fileName)) {
...
function readJsonFile(root, relativePath, label) {
  try {
    return JSON.parse(fs15.readFileSync(path20.resolve(root, relativePath), "utf8"));
  } catch (error) {
...
function exactReplay(root, input, paths2, digest3) {
  const presence = [paths2.record, paths2.report, paths2.receipt].map((relativePath) => fs15.existsSync(path20.resolve(root, relativePath)));
  ...
  const report = fs15.readFileSync(path20.resolve(root, paths2.report), "utf8");
...
function assessRecord(root, workspace, requestedMissionId, relativePath) {
  ...
  try {
    ...
    const reportFullPath = path20.resolve(root, record.reportPath ?? "");
    if (record.reportPath !== reportPath(record.reviewId) || !fs15.existsSync(reportFullPath) || domainSha256(fs15.readFileSync(reportFullPath)) !== record.reportSha256) failures.push("review-report-stale");
...
function queryReviewRecords(root, args = {}) {
  ...
  const directory = path20.resolve(root, ARTIFACT_PATHS.reviewsDir);
  const assessments = fs15.existsSync(directory) ? fs15.readdirSync(directory, { withFileTypes: true }).filter((entry) => entry.isFile() && /^review-[a-f0-9]{24}\.json$/u.test(entry.name)).map((entry) => assessRecord(root, workspace, mission.missionId, path20.posix.join(ARTIFACT_PATHS.reviewsDir, entry.name))).filter((item) => item.record && missionCanReadMission(workspace.missionGraph, mission.missionId, item.record.missionId)) : [];
  return {
    status: "ok",
    zeroWrite: true,
    authority: "not-established",
...
function filePath(directory, id3, suffix) {
  return path21.posix.join(directory, `${id3}.${suffix}.json`);
}
...
function upsertClaims(root, args = {}) {
  ...
  const writes = args.claims.map((item, index) => {
    ...
    records.push(claim);
    return {
      path: path21.posix.join(".dove/claims", `${claimId}.json`),
      kind: "data",
...
function runExperienceWorkflow(root, args = {}) {
  ...
  if (!fs16.existsSync(path21.resolve(root, planPath))) {
...
`, derivedReferences: [] }] }), plan: plan2, result: null, hostBoundary: { executesExperiment: false, mintsIndependentAuthority: false } };
  ...
  if (fs16.existsSync(path21.resolve(root, resultPath))) throw new Error("Experiment result is immutable once recorded; use a new experimentId for another run.");
...
function archiveArtifact(root, actionId, label, args, fields2, prepareSpecializedFields = () => ({})) {
  assertSealedDomainArgs(args, fields2, label);
...
function queryDomainIntegrity(root, missionId2 = null) {
  ...
  const stale = domainArtifacts.filter((item) => !fs16.existsSync(path21.resolve(root, item.path)));
...
function readCurrentMissions(root, options = {}) {
  ...
  const missions = fs17.readdirSync(missionsRoot, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => {
    ...
    let mission;
    try {
      mission = JSON.parse(fs17.readFileSync(path22.resolve(root, relativePath), "utf8"));
    } catch (error) {
...
function sealed10(value, fields2, label) {
  plain7(value, label);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
...
function sha25611(value) {
  return crypto19.createHash("sha256").update(typeof value === "string" || Buffer.isBuffer(value) ? value : stableWorkspaceSerialize(value)).digest("hex");
}
...
function recordResearchOutcome(root, args = {}) {
  ...
  sealed10(args, RECORD_FIELDS2, "recordResearchOutcome");
  const context = currentMutationContext(root);
  ...
  const handoff = createResearchHandoff(decision, { issuedAt: decision.createdAt, expiresAt: new Date(Date.parse(decision.createdAt) + FOUR_HOURS_MS2).toISOString() });
  ...
  context.requireCommitLock(".dove/.receipt-ledger-append.lock", { label: "Execution receipt ledger append lock" });
  assertReceiptAppendable(workspace.receiptLedger, receipt, { artifactHandoffs: workspace.artifactHandoffs });
  writeJson(root, path23.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`), receipt);
  return { status: "recorded", accepted: true, evidenceComplete: outcome.missingRequiredEvidence.length === 0, missingRequiredEvidence: outcome.missingRequiredEvidence, scopeDeviation: outcome.scopeDeviation, scopeDeviationReasons: outcome.scopeDeviationReasons, outcomeStatus: outcome.status, performedActionCount: outcome.performedActionCount, receipt, awaitingReevaluation: true, decision, writes: [path23.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`)] };
}
...
function sealed11(value, fields2, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`${label} must be a plain object.`);
  const unknown = Object.keys(value).filter((field) => !fields2.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
...
function reevaluateResearchDecision(root, args = {}) {
  ...
  sealed11(args, FIELDS4, "reevaluate-research-decision");
  ...
  const handoff = nextAction ? createResearchHandoff(appended.decision, { issuedAt: createdAt, expiresAt: new Date(Date.parse(createdAt) + FOUR_HOURS_MS3).toISOString() }) : null;
...
function decodeBinding(value) {
  ...
  const fields2 = Object.keys(payload).sort();
  if (fields2.join(",") !== ["currentHash", "version", "workspace", "workspaceIdentity"].sort().join(",") || payload.version !== DOVE_LESSONS_BINDING_VERSION || typeof payload.workspace !== "string" || typeof payload.workspaceIdentity !== "string" || !SHA256.test(String(payload.currentHash ?? ""))) {
    throw new Error("The Lessons read binding is invalid. Read the current Lessons document again.");
  }
...
function operationForCommand(commandId) {
...
function operationStatuses(operation) {
  const selected = typeof operation === "string" ? operationById(operation) : operation;
...
function publicArtifacts(items) {
  ...
  return items.flatMap((item) => {
    const path25 = publicPath(typeof item === "string" ? item : item?.path ?? item?.reference);
    if (!path25) return [];
    const result = { path: path25 };
...
function publicItems(items, fields2) {
  if (!Array.isArray(items)) return [];
  return items.filter(isPlainObject).map((item) => Object.fromEntries(fields2.flatMap((field) => {
...
function valueAtPath(value, path25) {
  return path25.split(".").reduce((current, field) => isPlainObject(current) ? current[field] : void 0, value);
}
...
function configPaths(root, env) {
  const paths2 = [];
  const xdg = normalizedString(env.XDG_CONFIG_HOME) ?? path24.join(os.homedir(), ".config");
  paths2.push(path24.join(xdg, "dove", "config.json"));
  if (root) {
    paths2.push(path24.resolve(root, ".dove", "config.json"));
    paths2.push(path24.resolve(root, ".dove", "config.local.json"));
  }
  return paths2;
}
...
export {
  ...
  DOVE_LESSONS_BINDING_VERSION,
  DOVE_LESSONS_SECTIONS,
  ...
  RESEARCH_DECISION_ACTION_KINDS,
  RESEARCH_DECISION_DISPOSITIONS,
  RESEARCH_DECISION_KINDS,
  RESEARCH_DECISION_SCHEMA_VERSION,
  ...
  VALIDATION_KINDS,
  VALIDATION_LEVELS,
  VALIDATION_PRODUCER_KINDS,
  VALIDATION_RESULTS,
  appendResearchDecision2 as appendResearchDecision,
  archiveReviewRecord,
  ...
  normalizeResearchDecisionAction,
  ...
  readCurrentResearchDecision,
  readDoveLessons,
  readExecutionReceipts,
  readResearchDecisions,
  recordDoveDraft,
  recordDoveFigure,
  recordDoveRebuttal,
  recordResearchOutcome,
  reevaluateResearchDecision,
  registerSource,
  ...
  researchDecisionNarrativeDirective,
  researchDecisionPath,
  resolveCurrentReviewFinding,
  ...
  validationContributesToCompletion,
  verifySource
};
