---
name: dove-init
description: "Propose and exactly confirm sealed schema 7 workspace initialization."
---

# Dove Init

Propose and exactly confirm sealed schema 7 workspace initialization.

## Daily use

- Request a zero-write schema 7 initialization proposal.
- Inspect and replay the exact confirmation data only when approved.
- Targeting: Initialization is bound to the canonical workspace, not a task target.
- Confirmation: Replay the exact proposal digest and workspace identity.
- Outcome: A sealed minimal schema 7 workspace exists without workflow side effects.

## Examples

- `/dove:init Initialize this research workspace`
- `/dove:init Archive invalid Dove state and initialize schema 7`
...
4. This request has one listed project action: `node ./bin/dove-package.mjs init . --goal "<project goal>" --mutation-mode direct-process`. Run it in the host's current working directory without changing directories or reinterpreting a parent repository as the target; `.` is the Dove workspace being operated on. Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
5. Use init only to establish minimal project identity; it must not create tasks, packets, checklists, runtime, orchestration, or navigation state.
...
12. Proposal is strictly zero-write; exact confirmation creates only manifest, project identity, ownership/lineage indexes, and required directories.
