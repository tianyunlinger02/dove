...
4. This request has one listed project action: `node ./bin/dove-package.mjs source . --mission-id "<mission id>" --source-id "<source id>" --title "<source title>" --locator "<url or doi>" --mutation-mode direct-process`. Run it in the host's current working directory without changing directories or reinterpreting a parent repository as the target; `.` is the Dove workspace being operated on. Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
5. Registration creates candidate material only; public verification can reject but cannot issue positive trust.
...
13. Public verification is rejection-only; positive verification requires a trusted internal receipt bound to current material and fingerprint.
