...
4. This request has one listed project action: `node ./bin/dove.mjs figure . --intent "<figure request>"` from the project root; Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
