...
11. Treat the user request as one figure intent and answer whether this figure is ready, waiting for SVG output, missing materials, missing drawing configuration, or needs current-figure fixes.
12. Use the hand-drawn SVG plan as the normal default path and tell the operator when SVG output is needed.
13. Use OpenAI image generation only when explicitly selected or configured; the OpenAI key must come from the OPENAI_API_KEY environment variable, never inline text.
14. Resolve the target task before updating figure state, then gather linked sections, claims, experiments, sources, notes, review concerns, and material hints automatically.
15. Do not mark a figure ready until imported output has source support, caption, and a clean current-figure check.
16. Captions must explain the figure purpose and linked evidence; default replies should not make the operator reason about paths or workspace-wide extra details.
