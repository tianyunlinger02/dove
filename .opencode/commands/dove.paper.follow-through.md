...
3. Keep every record explicit and proposal-only. Recording follow-through never auto-creates packets, checklist items, revision tasks, or config changes; use `project:dove.materialize` only when you intentionally want to cross into one real task packet.
