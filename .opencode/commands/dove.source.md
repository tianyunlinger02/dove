...
4. This request has one listed project action: `node ./bin/dove.mjs source . --target "<task title>" --title "<source title>" --locator "<url or doi>"` from the project root; Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
