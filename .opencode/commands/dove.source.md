...
10. Only add external material when it has a real title, locator, citation, URL, or operator-provided links or material.
11. If search, fetch, or verification finds no trustworthy material, say no source was added and explain the next retrieval step.
12. When the target task is unclear, ask the operator to choose from visible context instead of inspecting project state.
13. When source work cannot finish here, say the source material is ready and has not yet been added to the task; the natural Chinese phrasing is `这条来源还没加入任务`.
14. Batch multiple sources when the operator provides them together.
15. Keep source intake separate from synthesis; use note or document evidence for summaries and conclusions.
