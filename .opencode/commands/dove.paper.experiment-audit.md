...
Persist an experiment audit in `.dove/experiments/audits.json` that is distinct from the raw experiment result and explicit about integrity flags, reviewed artifacts, and confidence.

## Workflow

1. Read `.dove/experiments/plans.json`, `.dove/experiments/results.json`, `.dove/experiments/audits.json`, and the linked evidence artifacts.
2. If `dove` MCP is available, call `run_experiment_audit`.
