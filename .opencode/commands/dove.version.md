---
description: "Snapshot current mission artifacts or compare two snapshots immediately without writes."
---

# dove.version

Snapshot current mission artifacts or compare two snapshots immediately without writes.

## Daily use

- Snapshot current mission artifacts before a meaningful revision.
- Compare two explicit snapshots with a zero-write query.
- Targeting: Provide missionId and version ids explicitly.
- Confirmation: Snapshot inputs must resolve to current mission-owned artifacts; comparison is read-only and requires no confirmation.
- Outcome: Version snapshots are durable; hash comparison results are immediate and zero-write.
...
4. Use only the Dove MCP tool matching the requested operation from this command's allowed tools: `create_version_snapshot`, `compare_versions`.
...
13. Snapshots are immutable ids and contain current hashes and receipt lineage.
14. Comparison rejects stale snapshots and returns the current artifact differences without persisting a comparison artifact.
