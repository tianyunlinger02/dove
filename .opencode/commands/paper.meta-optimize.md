...
8. Treat the output as proposal-only guidance: convert strong recommendations, ranked conversion paths, remediation packs, playbooks, or execution bridge candidates into explicit board tasks, revision items, checklist steps, or artifact repairs only when an operator decides to act.
9. Do not auto-patch prompts, configs, code, or workflow files from this surface alone.
