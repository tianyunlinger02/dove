...
Persist a result-to-claim bridge event in `.dove/claims/bridge-log.json` so confidence and status updates are reviewable instead of implicit side effects.

## Workflow

1. Read `.dove/experiments/results.json`, `.dove/experiments/audits.json`, `.dove/evidence/index.json`, and `.dove/claims/bridge-log.json`.
2. If `dove` MCP is available, call `bridge_result_to_claim`.
