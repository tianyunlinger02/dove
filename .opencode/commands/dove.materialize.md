...
Materialize accepted proposal-only guidance into one real durable Dove task packet.
...
4. Treat materialization as explicit work creation, not execution: the new packet should become visible in the task graph, packet context, workspace index, runtime status surfaces, and follow-through ledger, but it should not be auto-run.
5. If planner should supervise a bounded non-planner packet step later, stamp the packet with an explicit worker role envelope during materialization rather than relying on implicit role inference.
6. If the packet should participate in a program-level operating surface, stamp `programId`, `programRunId`, `approvalId`, and when needed `allowedStepType` during materialization so runtime can verify an approved program run before any program-linked execution step occurs.
...
Do not use this command to create duplicate work for guidance already bound to a live packet or explicitly superseded.
