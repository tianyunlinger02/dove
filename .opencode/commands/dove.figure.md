...
- Outcome: The figure is planned, awaiting review, needs fixes, or validated by trusted proof.
...
4. This request has one listed project action: `node ./bin/dove-package.mjs figure . --mission-id "<mission id>" --figure-id "<figure id>" --intent "<figure request>" --purpose "<purpose>" --material "<artifact path>" --prompt "<drawing prompt>" --mutation-mode direct-process --json`. Run it in the host's current working directory without changing directories or reinterpreting a parent repository as the target; `.` is the Dove workspace being operated on. Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
5. Provider execution stays host-side; import output with an exact hash, caption, QA, and independent-review boundary.
...
12. Provider execution stays host-side; Dove accepts only current imported output with a matching hash.
13. Clean QA is diagnostic only; validated requires authoritative proof for the exact final artifact hash.
