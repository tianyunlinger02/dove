...
function normalizeRelativePath2(relativePath) {
  ...
  const normalized = path3.posix.normalize(relativePath.replace(/\\/g, "/"));
  if (path3.isAbsolute(relativePath) || normalized === "." || normalized.startsWith("../") || normalized === "..") {
...
var MutationContext = class {
  ...
  makeAnchoredDirectory(anchor, relativePath, createdDirectories) {
    const normalized = path3.posix.normalize(relativePath || ".");
...
function workspaceDigest(value) {
  return sha2563(stableWorkspaceSerialize(value));
}
...
function previewReviewExchangePreparation(root, args = {}) {
...
function assertPreparationReceipt(workspace, mission, manifestLeaf, inputLeaf, manifestPath, inputPath, packageArtifacts) {
  ...
  const receipt = workspace.receiptLedger.receipts.find((item) => {
    ...
    return artifacts.size === expected.size && [...expected].every(([artifactPath, sha2565]) => artifacts.get(artifactPath)?.sha256 === sha2565);
  });
...
function finalizeReviewImport(root, options) {
  ...
  const baseReceipt = {
    ...
    summary: domainNonEmptyText(options.summary, "summary"),
    artifacts: receiptArtifacts.map(({ path: artifactPath, kind, sha256: sha2565 }) => ({ path: artifactPath, kind, sha256: sha2565 })),
    validations: [],
    criteriaSatisfied: [],
    producedAt: recordedAt,
    recordedAt,
...
function toolInteractionFor(name, args = {}) {
  if (name === "prepare_review_exchange" && args.policy === "local-preflight") return "read";
  const contract = TOOL_INTERACTION_CONTRACTS[name];
  if (!contract) throw new Error(`Unknown MCP interaction contract: ${name}`);
...
var COMMAND_SURFACES = commandSurfaces.map((command) => {
  const interaction = COMMAND_INTERACTION_CONTRACTS[command.id];
  if (!interaction) throw new Error(`Missing command interaction contract: ${command.id}`);
...
function resolveExecutionReceiptPostCommit(root, result, options = {}) {
  ...
  if (!publicResult2.completion || publicResult2.completion.missionId !== missionId || publicResult2.completion.assessment !== null) {
...
function normalizeHostWorkspacePath(root, value, label = "Host path") {
  ...
  if (path20.isAbsolute(value)) {
    const normalizedAbsolute = path20.resolve(value);
    if (normalizedAbsolute !== value) invalidPath(label, value);
    const relative = path20.relative(resolvedRoot, normalizedAbsolute);
    if (!relative || relative === ".." || relative.startsWith(`..${path20.sep}`) || path20.isAbsolute(relative)) invalidPath(label, value);
    return relative.split(path20.sep).join("/");
  }
...
function lessonPath(lessonId) {
  return path21.posix.join(ARTIFACT_PATHS.lessonsDir, `${lessonId}.json`);
}
...
function readLessons(root) {
  ...
  const directory = path21.resolve(root, ARTIFACT_PATHS.lessonsDir);
  if (!fs18.existsSync(directory)) return [];
  return fs18.readdirSync(directory, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).sort((left, right) => left.name.localeCompare(right.name)).map((entry) => readJson(root, path21.posix.join(ARTIFACT_PATHS.lessonsDir, entry.name), null));
}
...
function buildProposal3(root, args) {
  ...
  const context = currentMutationContext(root);
  const occupied = context ? context.fileExists(relativePath) : fs18.existsSync(path21.resolve(root, relativePath));
...
function assessPinnedArtifacts(root, missionId, references) {
  ...
  return references.map((reference) => {
    ...
    const currentHash2 = sha256File(path21.resolve(root, reference.path));
...
function configPaths(root, env) {
  const paths = [];
  const xdg = normalizedString(env.XDG_CONFIG_HOME) ?? path22.join(os.homedir(), ".config");
  paths.push(path22.join(xdg, "dove", "config.json"));
  if (root) {
    paths.push(path22.resolve(root, ".dove", "config.json"));
    paths.push(path22.resolve(root, ".dove", "config.local.json"));
  }
  return paths;
}
...
var safeId4 = { type: "string", pattern: "^[a-z0-9][a-z0-9._-]{0,127}$" };
...
var lessonRecordProps = {
  ...
  sourceIds: { type: "array", items: safeId4 },
  noteIds: { type: "array", items: safeId4 },
  artifactRefs: strings,
  appliesToArtifactRefs: strings,
  tags: strings,
  supersedesLessonId: safeId4
};
var researchTreeNodeUpdateSchema = {
  type: "object",
  properties: {
    nodeId: safeId4,
    parentNodeId: { anyOf: [safeId4, { type: "null" }] },
    ...
    blockedReasonCode: { anyOf: [safeId4, { type: "null" }] }
  },
  required: [
    "nodeId",
    "parentNodeId",
...
var missionReplayProps = {
  ...
  dependsOnMissionIds: { type: "array", items: safeId4 },
  supersedesMissionId: safeId4,
...
var baseToolDefinitions = [
  ...
  missionMutationTool(),
  queryTool("query_dove_mission", "Preview one minimal schema 9 mission contract.", { missionId: safeId4, goal: { type: "string", minLength: 1 }, scope: strings, outOfScope: strings, targetArtifacts: strings, expectedArtifacts: strings, completionCriteria: strings, evidenceRequirements: strings, dependsOnMissionIds: { type: "array", items: safeId4 }, supersedesMissionId: safeId4 }, ["goal"]),
  queryTool("query_dove_status", "Read minimal mission, receipt, source, evidence, review, and domain integrity without writes; missionId is optional, the only mission is selected when exactly one exists, and multiple missions require explicit selection.", { missionId: safeId4, detail: { type: "string", enum: ["compact", "full"] }, language: { type: "string", enum: ["zh", "en"] } }),
  mutationTool("ingest_execution_receipt", "Ingest an immutable hash-bound execution receipt.", { receiptId: safeId4, missionId: safeId4, contractDigest: { type: "string", pattern: "^[0-9a-f]{64}$" }, summary: { type: "string", minLength: 1 }, artifacts: { type: "array", minItems: 1, items: receiptArtifactSchema }, validations: { type: "array", items: receiptValidationSchema }, criteriaSatisfied: { type: "array", items: receiptCriterionSchema }, producedAt: { type: "string", minLength: 1 } }, ["receiptId", "missionId", "contractDigest", "summary", "artifacts", "validations", "criteriaSatisfied", "producedAt"]),
  mutationTool("close_host_outcome", "Record current host-produced mission artifacts using server-generated outcome evidence.", { missionId: safeId4, summary: { type: "string", minLength: 1 }, artifactPaths: strings, validationPaths: strings }, ["missionId", "summary", "artifactPaths"]),
  queryTool("assess_mission_completion", "Reassess mission completion from current evidence and review authority.", { missionId: safeId4 }, ["missionId"]),
  ...
  queryTool("query_sources", "Query mission-bound source candidates or rejections and current ineligibility.", { missionId: safeId4, sourceId: safeId4, lifecycle: { type: "string", enum: ["candidate", "rejected"] }, limit: { type: "number" } }, ["missionId"]),
  queryTool("query_dove_lessons", "Query explicit advisory lessons without writes, repair, refresh, automatic capture, automatic recall, transcript ingestion, or Trellis/runtime integration.", { lessonId: safeId4, missionId: safeId4, scope: lessonScopes, kind: lessonKinds, tags: strings, artifactRefs: strings, includeSuperseded: { type: "boolean" }, includeUnscoped: { type: "boolean" }, limit: { type: "number" } }),
  ...
  mutationTool("register_source", "Register one mission-bound source candidate. Registration never verifies it.", { missionId: safeId4, sourceId: safeId4, citationKey: { type: "string" }, title: { type: "string" }, authors: strings, year: { type: ["string", "number"] }, locator: { type: "string" }, sourceType: { type: "string" }, abstract: { type: "string" }, origin: { type: "string" }, capturePath: { type: "string" } }, ["missionId", "sourceId"]),
  mutationTool("verify_source", "Record a mission-bound source rejection. Public positive verification is unavailable.", { missionId: safeId4, sourceId: safeId4, method: { type: "string", minLength: 1 }, checkedMaterial: { type: "string", minLength: 1 }, auditEvidence: { type: "array", minItems: 1, items: { type: "object", properties: { reference: { type: "string", minLength: 1 }, kind: { type: "string", minLength: 1 }, observation: { type: "string", minLength: 1 } }, required: ["reference", "kind", "observation"], additionalProperties: false } } }, ["missionId", "sourceId", "method", "checkedMaterial", "auditEvidence"]),
  mutationTool("upsert_note", "Record substantive mission-bound synthesis from current mission artifacts; sourceIds remain ineligible until a trusted positive verifier exists.", { missionId: safeId4, noteId: safeId4, title: { type: "string" }, summary: { type: "string" }, quotes: strings, claims: strings, openQuestions: strings, sourceIds: { type: "array", items: safeId4 }, artifactRefs: strings }, ["missionId", "noteId"]),
  mutationTool("upsert_claims", "Record mission-bound evidence-backed claims.", { missionId: safeId4, claims: { type: "array", minItems: 1, items: { type: "object", properties: { claimId: safeId4, text: { type: "string", minLength: 1 }, sourceIds: { type: "array", items: safeId4 }, noteIds: { type: "array", items: safeId4 }, artifactRefs: strings, experimentResultIds: { type: "array", items: safeId4 }, gap: { type: "string" } }, required: ["claimId", "text"], additionalProperties: false } } }, ["missionId", "claims"]),
  mutationTool("run_experience_workflow", "Record experiment protocol, result evidence, audit, and optional claim bridge without scheduling execution.", { missionId: safeId4, experimentId: safeId4, title: { type: "string" }, goal: { type: "string", minLength: 1 }, hypothesis: { type: "string", minLength: 1 }, protocol: { type: "string", minLength: 1 }, successCriteria: { type: "array", minItems: 1, items: { type: "string", minLength: 1 } }, comparisonTargets: strings, result: { type: "string" }, resultEvidenceRefs: strings, auditFindings: strings, integrityFlags: strings, claimId: safeId4, bridgeReason: { type: "string" } }, ["missionId", "experimentId", "goal", "hypothesis", "protocol", "successCriteria"]),
  mutationTool("upsert_draft", "Write a real mission-bound draft body with evidence lineage.", { missionId: safeId4, draftId: safeId4, title: { type: "string" }, body: { type: "string", minLength: 1 }, summary: { type: "string" }, evidenceRefs: strings, artifactRefs: strings }, ["missionId", "draftId", "body"]),
  mutationTool("upsert_draft_metadata", "Write explicit metadata for an existing mission-bound draft.", { missionId: safeId4, draftId: safeId4, title: { type: "string" }, summary: { type: "string" }, evidenceRefs: strings, artifactRefs: strings }, ["missionId", "draftId"]),
  mutationTool("run_figure_workflow", "Prepare mission-bound figure materials and prompt, or import host-produced output with caption, provenance, QA, and final hash.", { missionId: safeId4, figureId: safeId4, intent: { type: "string", minLength: 1 }, purpose: { type: "string", minLength: 1 }, materials: { type: "array", minItems: 1, items: { type: "string" } }, prompt: { type: "string", minLength: 1 }, outputPath: { type: "string" }, outputSha256: { type: "string", pattern: "^[0-9a-f]{64}$" }, caption: { type: "string" }, qaFindings: strings }, ["missionId", "figureId", "intent", "purpose", "materials", "prompt"]),
  mutationTool("prepare_review_exchange", "Preflight or prepare one schema 9 policy-scoped mission review exchange without launching a reviewer.", { missionId: safeId4, policy: { type: "string", enum: ["local-preflight", "isolated-selected-artifacts", "final-plan-results-only", "external"] }, artifactPaths: strings, finalPlanPaths: strings, finalResultPaths: strings }, ["missionId", "policy"]),
  mutationTool("import_review_exchange", "Import one canonical hash-bound schema 9 handoff and report as non-authoritative mission artifacts.", { missionId: safeId4, exchangeId: safeId4, reviewId: safeId4 }, ["missionId", "exchangeId", "reviewId"]),
  queryTool("verify_review_coverage", "Verify current exact artifact review coverage while Reviewer authority remains fail closed without a trusted issuer.", { missionId: safeId4, artifactPaths: strings, requireAuthoritative: { type: "boolean" } }, ["missionId"]),
  mutationTool("normalize_rebuttal_issues", "Normalize author-side reviewer issues with finding and evidence lineage.", { missionId: safeId4, issues: { type: "array", minItems: 1, items: { type: "object", properties: { issueId: safeId4, summary: { type: "string", minLength: 1 }, findingRefs: strings, evidenceRefs: strings }, required: ["issueId", "summary"], additionalProperties: false } } }, ["missionId", "issues"]),
  mutationTool("build_rebuttal_strategy", "Record an author-side strategy for normalized mission-bound issues.", { missionId: safeId4, strategy: { type: "string", minLength: 1 } }, ["missionId", "strategy"]),
  mutationTool("build_rebuttal", "Record evidence-linked author-side rebuttal responses.", { missionId: safeId4, issues: { type: "array", items: { type: "object", properties: { issueId: safeId4, summary: { type: "string", minLength: 1 }, findingRefs: strings, evidenceRefs: strings }, required: ["issueId", "summary", "findingRefs", "evidenceRefs"], additionalProperties: false } }, strategy: { type: "string" }, responses: { type: "array", minItems: 1, items: { type: "object", properties: { issueId: safeId4, response: { type: "string", minLength: 1 }, evidenceRefs: { type: "array", minItems: 1, items: { type: "string" } } }, required: ["issueId", "response", "evidenceRefs"], additionalProperties: false } } }, ["missionId", "responses"]),
  mutationTool("create_version_snapshot", "Snapshot current mission artifacts with optional supersedes lineage.", { missionId: safeId4, versionId: safeId4, label: { type: "string" }, artifactRefs: { type: "array", minItems: 1, items: { type: "string" } }, supersedesVersionId: safeId4 }, ["missionId", "versionId", "artifactRefs"]),
  queryTool("compare_versions", "Compare two current mission-bound artifact snapshots immediately without writes and reject stale inputs.", { missionId: safeId4, fromVersionId: safeId4, toVersionId: safeId4 }, ["missionId", "fromVersionId", "toVersionId"])
];
...
var MUTATING_TOOL_NAMES = new Set(baseToolDefinitions.map((tool) => tool.name).filter((name) => !READ_ONLY_TOOL_NAMES.has(name)));
...
var CHECKPOINT_TOOL_NAMES = /* @__PURE__ */ new Set([
  "init_dove_goal",
  "create_dove_mission",
...
function safeText(value) {
  ...
  return value.replace(/(?:^|[\s"'(])\.dove(?:-archive)?(?:\/[A-Za-z0-9._/-]+)?/gu, " an internal Dove artifact").replace(/\/(?:home|tmp|var|private|Users)\/[A-Za-z0-9._/@+-]+(?:\/[A-Za-z0-9._/@+-]+)*/gu, "the selected path").replace(/\b[0-9a-f]{64}\b/giu, "the validated fingerprint").replace(/\b(?:proposal(?:Digest|Workspace|Version|Token)|confirmArgs|confirm|exactReplay|resultMode|mutationMode|MutationContext|confirmed|(?:workspace|mission|source|note|claim|experiment|figure|review|exchange|version|lesson|receipt|criterion)Ids?|contractDigest|sourceTreeDigest|archiveTarget)\b/gu, "validated internal state").replace(/\b(?:source|note|claim|experiment-result|review|lesson|receipt):[a-z0-9._-]+\b/giu, "the selected evidence").replace(/\bUnknown (?:mission|source|note|claim|experiment|figure|review|exchange|version|lesson|receipt|criterion):\s*[a-z0-9._-]+\b/giu, "The selected item was not found").replace(/\b(?:Mission|Source|Note|Claim|Experiment|Figure|Review|Exchange|Version|Lesson|Receipt|Criterion)\s+[a-z0-9._-]+(?=\s+(?:belongs|does|is|has|requires|cannot|contains|was)\b)/giu, "The selected item").replace(/\b(?:workspace|mission|source|note|claim|experiment|figure|review|exchange|version|lesson|receipt|criterion)-[a-z0-9._-]+\b/giu, "the selected item");
}
function safeReason(value) {
  if (typeof value === "string" && PUBLIC_REASON_MESSAGES[value]) return PUBLIC_REASON_MESSAGES[value];
  const text = safeText(value);
  return typeof text === "string" && /\b(?:schema|contract|receipt|digest|hash|fingerprint|binding|ledger|internal state)\b/iu.test(text) ? "Recorded evidence is unavailable or no longer current." : text;
}
...
function publicArtifacts(items) {
  ...
  return items.flatMap((item) => {
    const path23 = publicPath(typeof item === "string" ? item : item?.path ?? item?.reference);
    if (!path23) return [];
    const result = { path: path23 };
    if (isPlainObject3(item)) {
      for (const field of ["kind", "current", "covered", "status"]) if (item[field] !== void 0) result[field] = item[field];
...
function publicItems(items, fields) {
  if (!Array.isArray(items)) return [];
  return items.filter(isPlainObject3).map((item) => Object.fromEntries(fields.flatMap((field) => {
    if (item[field] === void 0) return [];
    if (field === "reason") return [[field, safeReason(item[field])]];
    if (field === "eligibility" && isPlainObject3(item[field])) return [[field, { eligible: item[field].eligible === true, reason: safeReason(item[field].reason) }]];
...
function publicResult(name, data) {
  if (!isPlainObject3(data)) return data;
  const result = {};
  for (const field of ["status", "operation", "detailsAvailable", "zeroWrite", "complete", "policy", "inputBoundary"]) {
    if (data[field] !== void 0) result[field] = data[field];
  }
  const message = safeMessage(name, data);
  if (message) result.message = message;
  if (isPlainObject3(data.approval)) result.approval = {
  ...
  if (isPlainObject3(next)) result.continuation = {
    label: safeText(next.label),
    why: safeText(next.why),
    requiredActions: safeStrings(next.requiredActions)
  };
  if (name === "query_dove_mission" && isPlainObject3(data.mission)) result.mission = {
  ...
  if (name === "query_dove_status" && isPlainObject3(data.currentContext)) result.current = {
  ...
  if (name === "query_dove_status" && isPlainObject3(data.needsAttention)) {
    ...
    if (isPlainObject3(data.needsAttention.stableGaps)) result.gaps = {
  ...
  if (isPlainObject3(data.diff)) result.changes = { addedCount: data.diff.addedNodes?.length ?? 0, completedCount: data.diff.completedNodes?.length ?? 0, blockedCount: data.diff.blockedNodes?.length ?? 0, unchangedCount: data.diff.unchangedNodes?.length ?? 0 };
  if (["ingest_execution_receipt", "close_host_outcome"].includes(name) && isPlainObject3(data.receipt)) {
  ...
  if (name === "import_review_exchange" && isPlainObject3(data.review)) result.review = {
  ...
  if (name === "compare_versions" && isPlainObject3(data.comparison)) result.changes = {
  ...
  if (isPlainObject3(data.authority)) result.authority = { authoritative: data.authority.authoritative === true, reason: safeReason(data.authority.reason) };
  return result;
}
...
function dispatchData(root, name, args) {
  switch (name) {
    ...
    case "ingest_execution_receipt":
      return ingestExecutionReceipt(root, args);
    ...
    case "search_network":
      return searchNetwork(root, args);
    case "query_network_search_providers":
      return queryNetworkSearchProviders(root, args);
    case "query_sources":
      return querySources(root, args);
    case "query_dove_lessons":
      return queryDoveLessons(root, args);
    case "record_dove_lesson":
      return recordDoveLesson(root, args);
    case "register_source":
      return registerSource(root, args);
    case "verify_source":
      return verifySource(root, args);
    case "upsert_note":
      return upsertNote(root, args);
    case "upsert_claims":
      return upsertClaims(root, args);
    case "run_experience_workflow":
      return runExperienceWorkflow(root, args);
    case "upsert_draft":
      return upsertDraft(root, args);
    case "upsert_draft_metadata":
      return upsertDraftMetadata(root, args);
    case "run_figure_workflow":
      return runFigureWorkflow(root, args);
    case "prepare_review_exchange":
      return prepareReviewExchange(root, args);
    case "import_review_exchange":
      return importReviewExchange(root, args);
    case "verify_review_coverage":
      return verifyReviewCoverage(root, args);
    case "normalize_rebuttal_issues":
      return normalizeRebuttalIssues(root, args);
    case "build_rebuttal_strategy":
      return buildRebuttalStrategy(root, args);
    case "build_rebuttal":
      return buildRebuttal(root, args);
    case "create_version_snapshot":
      return createVersionSnapshot(root, args);
    case "compare_versions":
      return compareVersions(root, args);
    default:
      throw new Error(`Unknown tool: ${name}`);
  }
}
function assertAllowed2(name, args) {
...
function applyCheckpoint(root, name, args, proposal) {
  ...
  if (name !== "prepare_review_exchange" && !isPlainObject3(confirmArgs2)) throw new Error(`${name} did not return private exact replay data.`);
  return invokeMutation(root, name, () => {
    ...
    return dispatchData(root, name, confirmArgs2);
  });
}
function recordLesson(root, args) {
  const proposal = recordDoveLesson(root, { ...args, mutationMode: "direct-process" });
  if (!isPlainObject3(proposal.confirmation?.confirmArgs)) throw new Error("record_dove_lesson did not return private exact replay data.");
  ...
  if (interaction === "checkpoint") {
    ...
    return Promise.resolve(options.requestCheckpointApproval(approval)).then((value) => {
      ...
      if (applied.data && typeof applied.data.then === "function") {
        return applied.data.then((data) => publicResult(name, resolveExecutionReceiptPostCommit(root, data, { committed: applied.committed }))).catch(() => {
          throw checkpointApplyError();
        });
      }
...
function normalizeHostPathInputs(root, name, args) {
  ...
  switch (name) {
    ...
    case "run_figure_workflow":
      ...
      if (result.outputPath !== void 0) result.outputPath = hostFile(root, result.outputPath, "outputPath");
      break;
    case "prepare_review_exchange":
      result.artifactPaths = hostFiles(root, result.artifactPaths, "artifactPaths");
    ...
    case "query_dove_mission":
    case "create_dove_mission":
      if (result.targetArtifacts !== void 0) result.targetArtifacts = hostArtifacts(root, result.targetArtifacts, "targetArtifacts");
      if (result.expectedArtifacts !== void 0) result.expectedArtifacts = hostArtifacts(root, result.expectedArtifacts, "expectedArtifacts");
...
function dispatchTool(root, name, args = {}, options = {}) {
  try {
    assertAllowed2(name, args);
    assertMcpInputSchema(name, args, TOOL_INPUT_SCHEMAS.get(name));
    const normalizedArgs2 = normalizeHostPathInputs(root, name, args);
    const result = executeTool(root, name, normalizedArgs2, options);
    return result && typeof result.then === "function" ? result.then((value) => textResult(value)).catch((error) => textResult(publicErrorMessage(name, error), true)) : textResult(result);
  } catch (error) {
...
${body}`);
  ...
  async function handleMessage(message) {
    ...
    if (id !== void 0) sendError(id, -32601, `Method not found: ${method}`);
  }
