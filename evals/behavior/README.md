...
进程失败、公开主会话 result 错误、预算超额、缺失终态或任一 hard check 失败，命令以非零状态退出。权限拒绝单独保存在 `permissionDenials`，不自动改变任务结果：随后通过获准路径完成仍可记录完成，但被拒绝的动作不能声称成功，缺少工件或未完成判断仍按实际证据报告。退出码 0 本身不代表科研成功；预算耗尽 subtype 即使费用未高于上限也会失败。费用只取公开主会话终态的 `total_cost_usd`，不把分模型费用、工具材料或子代理字段当作总费用。未提供有效总费用时保留 `null`，不伪称已核验费用。

`expectedBehavior` 是人评材料线索，不是隐式 hard gate。案例选择的材料、显式 snapshot 路径以及 hard expectation 路径进入前后快照。快照保留存在性、类型、大小和字节摘要，不复制宿主私有对话。只有 `hardExpectations` 计算机器检查结果；不强制每例相同路径、禁止访问列表、三个 rubric 或固定语料数量。纯改写、只读判断、Source 核验等案例保留与用户请求直接相关的零写入检查，其他案例不继承统一的 DOCTOR/Review/Run 禁令。路径解析只为观察工件，不是权限执行器或通用安全审计。
