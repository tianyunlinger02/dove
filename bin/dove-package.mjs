...
var projectOption = value("--project");
var outputOptions = [boolean("--json"), value("--format")];
var businessOutputOptions = [...outputOptions, value("--language")];
var mutationOptions = [value("--mutation-mode"), ...businessOutputOptions];
var missionOptions = [
  projectOption,
  value("--operation"),
  ...
  value("--mode"),
  value("--goal"),
  value("--requirement", { repeatable: true }),
  value("--assumption", { repeatable: true }),
  ...
  value("--depends-on-mission-number", { repeatable: true }),
  value("--parent-mission-number"),
  value("--branch-kind"),
  value("--branch-reason"),
  value("--stop-parent-reason"),
  value("--handoff-artifact", { repeatable: true }),
  value("--requested-disposition"),
  value("--synthesis"),
  value("--hypothesis-json", { repeatable: true }),
  value("--route-json", { repeatable: true }),
  value("--open-question-json", { repeatable: true }),
  value("--evidence-ref", { repeatable: true }),
  value("--reason-code", { repeatable: true }),
  value("--next-action-json"),
  value("--mutation-mode"),
  ...businessOutputOptions
];
...
// src/cli/project-integration-output.mjs
...
// src/cli/terminal-output.mjs
var DOVE_PIXEL_ART = Object.freeze([
  "  \u2588\u2588\u2593\u2593                          \u2588\u2593\u2593\u2593",
  " \u2593\u2588\u2588\u2588\u2588\u2588                      \u2588\u2588\u2588\u2588\u2593\u2588\u2588",
  " \u2593\u2588\u2588\u2588\u2588\u2588\u2593                   \u2593\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588",
  " \u2593\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2593                \u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588",
...
function terminalColorEnabled(stream, env = process.env) {
  const noColor = env?.NO_COLOR;
  return isInteractiveTerminal(stream) && !(typeof noColor === "string" && noColor.length > 0);
}
function terminalStyle(text11, style, options = {}) {
  if (options.color !== true) return text11;
  const code = ANSI[style];
  if (!code) throw new Error(`Unknown Dove terminal style: ${style}.`);
  return `${code}${text11}${ANSI.reset}`;
}
function renderDovePixelArt(options = {}) {
...
function renderDoveHome(options = {}) {
  ...
  const state2 = options.state ?? (options.projectInitialized === true ? "current" : "uninitialized");
  const nextCommand = options.nextCommand ?? (state2 === "uninitialized" ? "dove" : "/dove:workspace");
  const stateLabel = {
...
// src/cli/project-integration-output.mjs
var INTERNAL_FIELD_NAMES = /* @__PURE__ */ new Set([
  "writtenPaths",
  "removedPaths",
  "changedPaths",
  "transactionState",
...
function terminalSafeText(value2) {
  return String(value2).replace(/[\x00-\x1f\x7f-\x9f]/gu, "?");
}
...
function headingFor(command3, status) {
  ...
  if (command3 === "sync" && status === "unchanged") return "Dove \u9879\u76EE\u96C6\u6210\u5DF2\u662F\u6700\u65B0";
  if (command3 === "sync" && status === "synchronized") return "Dove \u9879\u76EE\u96C6\u6210\u5DF2\u5237\u65B0";
  throw new Error(`Unsupported Dove integration presentation: ${command3}/${status}.`);
}
function setupLines(command3, status) {
  if (command3 === "init" && status === "already-initialized") {
  ...
  return status === "unchanged" ? ["\u2713 \u5DE5\u4F5C\u5165\u53E3\u3001Dove-only MCP \u6279\u51C6\u548C\u81EA\u7136\u8BED\u8A00\u4EFB\u52A1\u5165\u53E3\u5747\u5DF2\u662F\u6700\u65B0"] : ["\u2713 \u5DE5\u4F5C\u5165\u53E3\u3001Dove-only MCP \u6279\u51C6\u548C\u81EA\u7136\u8BED\u8A00\u4EFB\u52A1\u5165\u53E3\u5DF2\u5B89\u5168\u5237\u65B0"];
}
function renderProjectIntegrationResult(command3, result, options = {}) {
  assertIntegrationResult(result);
  if (!(/* @__PURE__ */ new Set(["init", "sync"])).has(command3)) throw new Error(`Unsupported Dove integration command: ${command3}.`);
  ...
  const interactive = isInteractiveTerminal(stream);
  const color = terminalColorEnabled(stream, env);
  const heading = headingFor(command3, result.status);
  const projectName = terminalSafeText(path.basename(result.target) || result.target);
  const lines = [];
  if (command3 === "init" && ["initialized", "already-initialized"].includes(result.status) && interactive) {
  ...
  if (command3 === "init" && result.status === "already-initialized") {
    ...
    lines.push(
      command3 === "init" ? "\u79D1\u7814\u4E3B\u7EBF\u5C1A\u672A\u5EFA\u7ACB\u3002\u8FDB\u5165 Claude Code \u540E\u663E\u5F0F\u8FD0\u884C /dove:workspace\u3002" : "\u79D1\u7814\u8BB0\u5F55\u672A\u88AB\u4FEE\u6539\u3002\u91CD\u65B0\u8FDB\u5165 Claude Code \u540E\u53EF\u76F4\u63A5\u4F7F\u7528\u66F4\u65B0\u540E\u7684 Dove\u3002"
    );
  }
  lines.push("");
...
function classifyLessonsIntent(prompt) {
  ...
  const normalized3 = prompt.normalize("NFKC").trim();
  if (!normalized3 || normalized3.startsWith("/") || LESSONS_QUESTION.test(normalized3)) return null;
  const body = requestBody(normalized3);
...
function isHighConfidenceAmbientWorkPrompt(prompt) {
  ...
  const normalized3 = prompt.normalize("NFKC").trim();
  if (!normalized3 || normalized3.startsWith("/")) return false;
  if (CONVERSATIONAL_ONLY.some((pattern) => pattern.test(normalized3))) return false;
  if (STATUS_ONLY.some((pattern) => pattern.test(normalized3))) return false;
  const body = requestBody(normalized3);
...
function sameKeys(value2, keys) {
  return plainObject(value2) && Object.keys(value2).sort().join(",") === [...keys].sort().join(",");
}
function isExactManagedHook(value2) {
  if (!sameKeys(value2, ["hooks"]) || !Array.isArray(value2.hooks) || value2.hooks.length !== 1) return false;
  const hook = value2.hooks[0];
...
function referencesManagedHook(value2) {
  if (!plainObject(value2) || !Array.isArray(value2.hooks)) return false;
  return value2.hooks.some((hook) => plainObject(hook) && typeof hook.command === "string" && (hook.command.includes("dove hook user-prompt-submit") || hook.command.includes("dove-user-prompt-submit-package.mjs")));
}
...
function normalizeDoveResponseLanguage(value2, fallback = DEFAULT_DOVE_RESPONSE_LANGUAGE, options = {}) {
  const normalizedFallback = DOVE_RESPONSE_LANGUAGES.includes(fallback) ? fallback : DEFAULT_DOVE_RESPONSE_LANGUAGE;
  if (typeof value2 !== "string" || !value2.trim()) return normalizedFallback;
  const normalized3 = value2.trim().toLowerCase();
  if (DOVE_RESPONSE_LANGUAGES.includes(normalized3)) return normalized3;
  if (options.strict === true) {
    throw new Error(`Unsupported Dove response language: ${value2}. Supported values: ${DOVE_RESPONSE_LANGUAGES.join(", ")}.`);
  }
...
function normalizeRelativePath(relativePath, label = "Filesystem path") {
  ...
  const normalized3 = path2.posix.normalize(relativePath.replace(/\\/gu, "/"));
  if (path2.isAbsolute(relativePath) || normalized3 === "." || normalized3 === ".." || normalized3.startsWith("../") || normalized3.includes("\0")) {
...
var AnchoredFilesystem = class {
  ...
  withParent(relativePath, callback) {
    ...
    const parentRelative = path2.posix.dirname(normalized3);
    const parent = this.openDirectory(parentRelative === "." ? null : parentRelative);
    const name = path2.posix.basename(normalized3);
    try {
      return callback({ normalized: normalized3, parent, name, handlePath: path2.posix.join(parent.handlePath, name) });
    } finally {
  ...
  openFile(relativePath, flags, mode2) {
    return this.withParent(relativePath, ({ handlePath }) => requiredFunction(this.fsOps, "openSync")(handlePath, flags | this.constants.O_NOFOLLOW, mode2));
  }
  ...
  writeNewFile(relativePath, content, options = {}) {
    const mode2 = options.mode ?? 384;
    const fd = this.openFile(relativePath, this.constants.O_WRONLY | this.constants.O_CREAT | this.constants.O_EXCL, mode2);
    try {
  ...
  chmod(relativePath, mode2) {
    const fd = this.openFile(relativePath, this.constants.O_RDONLY);
    try {
      requiredFunction(this.fsOps, "fchmodSync")(fd, mode2);
    } finally {
      requiredFunction(this.fsOps, "closeSync")(fd);
    }
  }
  ...
  rename(fromRelativePath, toRelativePath) {
    ...
    try {
      const sourcePath2 = path2.posix.join(fromParent.handlePath, path2.posix.basename(from));
...
function resolveCanonicalContainedWrite(root, candidatePath, options = {}) {
  ...
  const requestedPath = path3.isAbsolute(candidatePath) ? path3.resolve(candidatePath) : path3.resolve(resolvedRoot, candidatePath);
  const requestedRelative = path3.relative(resolvedRoot, requestedPath);
  ...
  for (const component of requestedRelative.split(path3.sep)) {
    currentPath = path3.join(currentPath, component);
    let stat;
    try {
  ...
  const canonicalRelative = path3.relative(canonicalRoot, canonicalAncestor);
  if (pathEscapesRoot(canonicalRelative)) {
    throw new Error(`${label} must stay inside the canonical root: ${candidatePath}`);
  }
  return {
    root: canonicalRoot,
    relativePath: requestedRelative.split(path3.sep).join("/"),
    fullPath: path3.join(canonicalRoot, requestedRelative)
  };
}
...
function normalizeRelativePath2(relativePath) {
  ...
  const normalized3 = path4.posix.normalize(relativePath.replace(/\\/g, "/"));
  if (path4.isAbsolute(relativePath) || normalized3 === "." || normalized3.startsWith("../") || normalized3 === "..") {
...
var MutationContext = class {
  ...
  makeAnchoredDirectory(anchor, relativePath, createdDirectories) {
    const normalized3 = path4.posix.normalize(relativePath || ".");
...
function artifactEvidenceRole(relativePath) {
  ...
  const value2 = normalized3.normalizedPath;
...
function inspectDeclaredPath(root, rawPath, options = {}) {
  ...
  const fullPath = path5.resolve(rootPath, normalized3.normalizedPath);
...
function stableValue(value2) {
  ...
  if (value2 && typeof value2 === "object") return Object.fromEntries(Object.entries(value2).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, stableValue(item)]));
  return value2;
}
...
function safeId(value2, label) {
  if (typeof value2 !== "string" || !SAFE_ID.test(value2)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return value2;
}
...
function canonicalPath(value2) {
  const normalized3 = normalizeProjectRelativePath(value2);
  if (!normalized3.ok || normalized3.normalizedPath !== value2) throw new Error("Artifact handoff path must be a canonical safe project-relative path.");
  return value2;
}
function normalized(value2) {
  const fromMissionId = safeId(value2.fromMissionId, "Artifact handoff fromMissionId");
  const toMissionId = safeId(value2.toMissionId, "Artifact handoff toMissionId");
  if (fromMissionId === toMissionId) throw new Error("Artifact handoff must transfer authority to a different mission.");
  return {
    workspaceId: safeId(value2.workspaceId, "Artifact handoff workspaceId"),
    path: canonicalPath(value2.path),
    fromMissionId,
    toMissionId,
    fromReceiptId: safeId(value2.fromReceiptId, "Artifact handoff fromReceiptId"),
    fromSha256: hash(value2.fromSha256, "Artifact handoff fromSha256"),
    reason: text(value2.reason, "Artifact handoff reason"),
    createdAt: exactIso(value2.createdAt, "Artifact handoff createdAt")
  };
}
function digest(value2) {
  return sha2562(serialize({ schemaVersion: ARTIFACT_HANDOFF_SCHEMA_VERSION, ...normalized(value2) }));
}
function id(value2, handoffDigest) {
  return `artifact-handoff-${sha2562(serialize({ path: value2.path, fromMissionId: value2.fromMissionId, toMissionId: value2.toMissionId, handoffDigest })).slice(0, 24)}`;
}
function createArtifactHandoff(value2) {
  const content = normalized(value2);
...
function validateArtifactHandoff(value2, options = {}) {
  const label = options.label ?? "Artifact handoff";
  sealed(value2, FIELDS, label);
  if (value2.schemaVersion !== ARTIFACT_HANDOFF_SCHEMA_VERSION) throw new Error(`${label} has an unsupported schemaVersion.`);
  const content = normalized(value2);
  const handoffDigest = digest(content);
  const handoffId = id(content, handoffDigest);
  if (value2.handoffDigest !== handoffDigest || value2.handoffId !== handoffId) throw new Error(`${label} does not match its canonical content.`);
...
function validateArtifactHandoffs(values, options = {}) {
  ...
  for (const value2 of values) {
    const handoff = validateArtifactHandoff(value2, options);
...
function artifactHandoffPath(handoffId) {
  return path6.posix.join(ARTIFACT_PATHS.artifactHandoffsDir, `${safeId(handoffId, "Artifact handoff id")}.json`);
}
...
function assertExecutionFactText(value2, label = "Execution fact") {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  const statement = value2.trim();
...
function createExecutionFact(value2, label = "Execution fact") {
  const statement = typeof value2 === "string" ? assertExecutionFactText(value2, label) : assertExecutionFactText(value2?.statement, `${label}.statement`);
  const factId = typeof value2 === "string" || value2?.factId === void 0 ? executionFactId(statement) : value2.factId;
  ...
  if (value2 && typeof value2 === "object" && !Array.isArray(value2)) {
    const unknown = Object.keys(value2).filter((field) => !EXECUTION_FACT_FIELDS.includes(field));
...
function readStoredExecutionFact(value2, label = "Execution fact") {
  return createExecutionFact(value2, label);
}
...
function stableValue2(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue2);
  if (value2 && typeof value2 === "object") return Object.fromEntries(Object.entries(value2).filter(([, item]) => item !== void 0).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, stableValue2(item)]));
  return value2;
}
function stableMissionSerialize(value2) {
  return JSON.stringify(stableValue2(value2));
}
function plain2(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function sealed2(value2, fields, label) {
  plain2(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
}
function text2(value2, label) {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  return value2.trim();
}
function safeId2(value2, label) {
  const normalized3 = text2(value2, label);
...
function strings(value2, label) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const result = value2.map((item, index) => text2(item, `${label}[${index}]`));
...
function normalizeArtifacts(value2) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error("artifacts must be an array.");
  const result = value2.map((item, index) => {
...
function normalizeCriteria(value2) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error("completionCriteria must be an array.");
  const result = value2.map((item, index) => {
...
function normalizeEvidenceReference(value2, label) {
  const requirement = text2(value2, label);
...
function normalizeEvidenceRequirements(value2) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error("evidenceRequirements must be an array.");
  const result = value2.map((item, index) => {
...
function normalizeMissionMode(value2) {
  const mode2 = text2(value2, "Mission mode");
  if (!MODE_SET.has(mode2)) throw new Error(`Mission mode must be one of: ${MISSION_MODES.join(", ")}.`);
  return mode2;
}
function normalizeMissionContractContent(value2 = {}) {
  plain2(value2, "Mission contract");
  const content = {
    mode: normalizeMissionMode(value2.mode),
    goal: text2(value2.goal, "Dove mission goal"),
    requirements: strings(value2.requirements, "requirements"),
    assumptions: strings(value2.assumptions, "assumptions"),
    scope: strings(value2.scope, "scope"),
    outOfScope: strings(value2.outOfScope, "outOfScope"),
    artifacts: normalizeArtifacts(value2.artifacts),
    completionCriteria: normalizeCriteria(value2.completionCriteria),
    evidenceRequirements: normalizeEvidenceRequirements(value2.evidenceRequirements),
    dependsOnMissionIds: strings(value2.dependsOnMissionIds, "dependsOnMissionIds").map((item, index) => safeId2(item, `dependsOnMissionIds[${index}]`))
  };
  const parentMissionId = value2.parentMissionId === void 0 ? null : safeId2(value2.parentMissionId, "parentMissionId");
  const branchKind = value2.branchKind === void 0 ? null : text2(value2.branchKind, "branchKind");
  const branchReason = value2.branchReason === void 0 ? null : text2(value2.branchReason, "branchReason");
  if (parentMissionId) {
...
function exactHash(value2, label) {
  const normalized3 = exactString(value2, label);
  if (!HASH3.test(normalized3)) throw new Error(`${label} must be a lowercase SHA-256 digest.`);
  return normalized3;
}
function exactSafeId(value2, label) {
  const normalized3 = exactString(value2, label);
  if (!SAFE_ID3.test(normalized3)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return normalized3;
}
function assertObject(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function assertFields(value2, fields, label) {
  assertObject(value2, label);
  const allowed2 = new Set(fields);
  const unknown = Object.keys(value2).filter((field) => !allowed2.has(field));
  if (unknown.length > 0) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
}
function exactExitStatus(value2, label) {
  if (value2 === null) return null;
  if (!Number.isSafeInteger(value2) || value2 < 0 || value2 > 255) throw new Error(`${label} must be an integer from 0 through 255, or null when no exit status was observed.`);
  return value2;
}
function createValidationRecord(value2, options = {}) {
  const label = options.label ?? "validation";
  assertFields(value2, VALIDATION_FIELDS, label);
  const kind = exactString(value2.kind, `${label}.kind`);
  if (!KIND_SET.has(kind)) throw new Error(`${label}.kind must be one of: ${VALIDATION_KINDS.join(", ")}.`);
  const result = exactString(value2.result, `${label}.result`);
  if (!RESULT_SET.has(result)) throw new Error(`${label}.result must be one of: ${VALIDATION_RESULTS.join(", ")}.`);
  const level = exactString(value2.level, `${label}.level`);
  if (!LEVEL_SET.has(level)) throw new Error(`${label}.level must be one of: ${VALIDATION_LEVELS.join(", ")}.`);
  const producerKind = exactString(value2.producerKind, `${label}.producerKind`);
  if (!PRODUCER_KIND_SET.has(producerKind)) throw new Error(`${label}.producerKind must be one of: ${VALIDATION_PRODUCER_KINDS.join(", ")}.`);
  const producerOperation = exactSafeId(value2.producerOperation, `${label}.producerOperation`);
  const observedExitStatus = exactExitStatus(value2.observedExitStatus, `${label}.observedExitStatus`);
  const targetReference = exactString(value2.targetReference, `${label}.targetReference`);
  const targetHash = exactHash(value2.targetHash, `${label}.targetHash`);
  const reference = exactString(value2.reference, `${label}.reference`);
  const outputHash = exactHash(value2.outputHash, `${label}.outputHash`);
...
function stableValue3(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue3);
  if (value2 && typeof value2 === "object") {
    return Object.fromEntries(Object.entries(value2).sort(([left], [right]) => left.localeCompare(right)).map(([key, item]) => [key, stableValue3(item)]));
  }
  return value2;
}
function sha2564(value2) {
  return crypto5.createHash("sha256").update(JSON.stringify(stableValue3(value2))).digest("hex");
}
function ordinaryHostOutcomeCallbackDigest(value2) {
  return sha2564({
    attemptId: value2.attemptId,
    missionId: value2.missionId,
    contractDigest: value2.contractDigest,
    summary: value2.summary,
    mode: value2.mode,
    status: value2.status,
    facts: value2.facts,
    artifacts: value2.artifacts.map(({ path: artifactPath, sha256: artifactSha256 }) => ({ path: artifactPath, sha256: artifactSha256 })),
    validations: value2.validations.map(({ kind, result, level, producerKind, producerOperation, observedExitStatus, targetReference, targetHash, reference, outputHash }) => ({ kind, result, level, producerKind, producerOperation, observedExitStatus, targetReference, targetHash, reference, outputHash }))
  });
}
...
function canonicalPath3(value2, label) {
  const normalized3 = normalizeProjectRelativePath(value2);
  if (!normalized3.ok) throw new Error(`${label} has an unsafe path: ${normalized3.reason}.`);
  if (value2 !== normalized3.normalizedPath) throw new Error(`${label} must use a canonical project-relative path.`);
  return value2;
}
function canonicalStringArray(value2, label, options = {}) {
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const items = value2.map((item, index) => exactString2(item, `${label}[${index}]`));
...
function validateProducer(value2, label) {
  ...
  if (!PRODUCER_KIND_SET2.has(value2.kind)) throw new Error(`${label}.kind is unsupported.`);
  const actionId = safeId3(value2.actionId, `${label}.actionId`);
  if (value2.kind === "public-execution" && !["ingest-execution-receipt", "close-host-outcome"].includes(actionId)) {
...
function validateStoredReceipt(value2, context) {
  ...
  safeId3(value2.workspaceId, `${label}.workspaceId`);
  const receiptId = safeId3(value2.receiptId, `${label}.receiptId`);
  ...
  const missionId2 = safeId3(value2.missionId, `${label}.missionId`);
  const mission = missions.get(missionId2);
  if (!mission) throw new Error(`${label} references unknown mission ${missionId2}.`);
  hash3(value2.contractDigest, `${label}.contractDigest`);
  if (value2.contractDigest !== mission.contractDigest) throw new Error(`${label}.contractDigest does not match mission ${missionId2}.`);
  exactString2(value2.summary, `${label}.summary`);
  exactIso3(value2.producedAt, `${label}.producedAt`);
  exactIso3(value2.recordedAt, `${label}.recordedAt`);
  ...
  const ordinaryHostOutcome = value2.ordinaryHostOutcome;
  if (ordinaryHostOutcome !== void 0) {
    ...
    if (ordinaryHostOutcome.mode === "observation-only" && (value2.artifacts.length > 0 || value2.validations.length > 0)) {
      throw new Error(`${label}.ordinaryHostOutcome observation-only mode cannot own artifacts or claim validations.`);
    }
    if (ordinaryHostOutcome.mode === "artifact-backed" && value2.artifacts.length === 0) throw new Error(`${label}.ordinaryHostOutcome artifact-backed mode requires at least one artifact.`);
  }
  const researchOutcome = value2.researchOutcome;
  if (researchOutcome !== void 0) {
    ...
    const expectedResearchDigest = sha2564({
      ...
      artifacts: value2.artifacts.map(({ path: artifactPath, sha256: artifactSha256 }) => ({ path: artifactPath, sha256: artifactSha256 })),
      validations: value2.validations.map(({ reference, outputHash }) => ({ path: reference, sha256: outputHash })),
    ...
    if (value2.producer.kind !== "dove-internal" || value2.producer.actionId !== "record-research-outcome") throw new Error(`${label}.researchOutcome requires the record-research-outcome producer.`);
  }
  ...
  if (value2.artifacts.length === 0 && value2.validations.length === 0 && value2.criteriaSatisfied.length === 0 && ordinaryHostOutcome === void 0 && researchOutcome === void 0) {
  ...
  value2.validations = value2.validations.map((validation, index) => {
    ...
    const target2 = normalized3.targetReference.slice(separator + 1);
    if (!["artifact", "validation"].includes(targetKind) || !target2) throw new Error(`${itemLabel}.targetReference must be an exact typed file binding.`);
    canonicalPath3(target2, `${itemLabel}.targetReference`);
    return normalized3;
  });
  ...
  for (const [index, criterion] of value2.criteriaSatisfied.entries()) {
    ...
    for (const [referenceIndex, reference] of references.entries()) {
      ...
      if ((kind === "artifact" || kind === "validation") && canonicalPath3(target2, `${itemLabel}.evidenceRefs[${referenceIndex}]`) !== target2) {
  ...
  if (ordinaryHostOutcome !== void 0) {
    const currentDigest = ordinaryHostOutcomeCallbackDigest({
      ...
      artifacts: value2.artifacts,
      validations: value2.validations
    });
...
function stableValue4(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue4);
  if (value2 && typeof value2 === "object") return Object.fromEntries(Object.entries(value2).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, stableValue4(item)]));
  return value2;
}
function serialize2(value2) {
  return JSON.stringify(stableValue4(value2));
}
function sha2565(value2) {
  return crypto6.createHash("sha256").update(value2).digest("hex");
}
function plain3(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function sealed3(value2, fields, label) {
  plain3(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
function safeId4(value2, label) {
  if (typeof value2 !== "string" || !SAFE_ID5.test(value2)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return value2;
}
function hash4(value2, label) {
  if (typeof value2 !== "string" || !HASH5.test(value2)) throw new Error(`${label} must be a lowercase SHA-256 digest.`);
  return value2;
}
function text3(value2, label) {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  return value2.trim();
}
function exactIso4(value2, label) {
  if (typeof value2 !== "string" || !Number.isFinite(Date.parse(value2)) || new Date(Date.parse(value2)).toISOString() !== value2) throw new Error(`${label} must be an exact ISO-8601 timestamp.`);
  return value2;
}
function strings2(value2, label) {
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const result = value2.map((item, index) => text3(item, `${label}[${index}]`));
  if (new Set(result).size !== result.length) throw new Error(`${label} must not contain duplicates.`);
  return result;
}
function normalized2(value2) {
  const status = text3(value2.status, "Mission transition status");
  if (!STATUS_SET.has(status)) throw new Error("Mission transition status is unsupported.");
  const trigger = text3(value2.trigger, "Mission transition trigger");
  if (!TRIGGER_SET.has(trigger)) throw new Error("Mission transition trigger is unsupported.");
  return {
    workspaceId: safeId4(value2.workspaceId, "Mission transition workspaceId"),
    missionId: safeId4(value2.missionId, "Mission transition missionId"),
    contractDigest: hash4(value2.contractDigest, "Mission transition contractDigest"),
    workspaceRevisionId: safeId4(value2.workspaceRevisionId, "Mission transition workspaceRevisionId"),
    status,
    reason: text3(value2.reason, "Mission transition reason"),
    evidenceRefs: strings2(value2.evidenceRefs ?? [], "Mission transition evidenceRefs"),
    trigger,
    createdAt: exactIso4(value2.createdAt, "Mission transition createdAt")
  };
}
function digest2(value2) {
  return sha2565(serialize2({ schemaVersion: MISSION_TRANSITION_SCHEMA_VERSION, ...normalized2(value2) }));
}
...
function createMissionTransition(value2) {
  const content = normalized2(value2);
...
function validateMissionTransition(value2, options = {}) {
  const label = options.label ?? "Mission transition";
  sealed3(value2, FIELDS2, label);
  if (value2.schemaVersion !== MISSION_TRANSITION_SCHEMA_VERSION) throw new Error(`${label} has an unsupported schemaVersion.`);
  const content = normalized2(value2);
  const transitionDigest = digest2(content);
  const expectedId = transitionId(content.missionId, transitionDigest);
  if (value2.transitionDigest !== transitionDigest || value2.transitionId !== expectedId) throw new Error(`${label} does not match its canonical content.`);
  if (options.workspaceId !== void 0 && content.workspaceId !== options.workspaceId) throw new Error(`${label}.workspaceId does not match the manifest.`);
...
function validateMissionTransitions(values, options = {}) {
  ...
  for (const value2 of values) {
    const transition = validateMissionTransition(value2, options);
...
function missionTransitionPath(transitionIdValue) {
  return path8.posix.join(ARTIFACT_PATHS.missionTransitionsDir, `${safeId4(transitionIdValue, "Mission transition id")}.json`);
}
...
function missionEntries(value2) {
  ...
  return value2.map((entry, index) => {
    ...
    const filename = typeof entry.filename === "string" ? path9.posix.basename(entry.filename) : "";
...
function sha2566(value2) {
  return crypto7.createHash("sha256").update(value2).digest("hex");
}
function stableValue5(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue5);
  if (value2 && typeof value2 === "object") return Object.fromEntries(Object.entries(value2).filter(([, item]) => item !== void 0).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, stableValue5(item)]));
  return value2;
}
function stableResearchDecisionSerialize(value2) {
  return JSON.stringify(stableValue5(value2));
}
function plain4(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function sealed4(value2, fields, label) {
  plain4(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
}
function exact(value2, fields, label) {
  sealed4(value2, fields, label);
  const missing = [...fields].filter((field) => !Object.hasOwn(value2, field));
  if (missing.length) throw new Error(`${label} requires fields: ${missing.join(", ")}.`);
}
function text4(value2, label) {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  return value2.trim();
}
function safeId5(value2, label) {
  const normalized3 = text4(value2, label);
  if (!SAFE_ID6.test(normalized3)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return normalized3;
}
function hash5(value2, label) {
  const normalized3 = text4(value2, label);
  if (!HASH6.test(normalized3)) throw new Error(`${label} must be a lowercase SHA-256 digest.`);
  return normalized3;
}
function exactIso5(value2, label) {
  if (typeof value2 !== "string" || !ISO.test(value2) || !Number.isFinite(Date.parse(value2)) || new Date(Date.parse(value2)).toISOString() !== value2) throw new Error(`${label} must be an exact ISO-8601 timestamp.`);
  return value2;
}
function array(value2, label) {
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  return value2;
}
function strings3(value2, label, minItems = 0) {
  const result = array(value2, label).map((item, index) => text4(item, `${label}[${index}]`));
  if (result.length < minItems) throw new Error(`${label} must contain at least ${minItems} item(s).`);
  if (new Set(result).size !== result.length) throw new Error(`${label} must not contain duplicates.`);
  return result;
}
function ids(value2, label) {
  return strings3(value2, label).map((item, index) => safeId5(item, `${label}[${index}]`));
}
function enumValue(value2, allowed2, label) {
  const normalized3 = text4(value2, label);
...
function budget(value2, label) {
  plain4(value2, label);
  const result = {};
  for (const [dimension, amount] of Object.entries(value2)) {
...
function normalizeHypothesis(value2, index) {
  const label = `hypotheses[${index}]`;
  exact(value2, HYPOTHESIS_FIELDS, label);
  return { hypothesisId: safeId5(value2.hypothesisId, `${label}.hypothesisId`), statement: text4(value2.statement, `${label}.statement`), assessment: enumValue(value2.assessment, ASSESSMENT_SET, `${label}.assessment`), supportingEvidence: strings3(value2.supportingEvidence, `${label}.supportingEvidence`), counterEvidence: strings3(value2.counterEvidence, `${label}.counterEvidence`), falsificationCondition: text4(value2.falsificationCondition, `${label}.falsificationCondition`) };
}
function normalizeRoute(value2, index) {
  const label = `routes[${index}]`;
  exact(value2, ROUTE_FIELDS, label);
  return { routeId: safeId5(value2.routeId, `${label}.routeId`), summary: text4(value2.summary, `${label}.summary`), disposition: enumValue(value2.disposition, ROUTE_DISPOSITION_SET, `${label}.disposition`), rationale: text4(value2.rationale, `${label}.rationale`) };
}
function normalizeQuestion(value2, index) {
  const label = `openQuestions[${index}]`;
  exact(value2, QUESTION_FIELDS, label);
  return { questionId: safeId5(value2.questionId, `${label}.questionId`), question: text4(value2.question, `${label}.question`) };
}
function actionDigest(value2) {
  const { actionDigest: _actionDigest, ...content } = value2;
  return sha2566(stableResearchDecisionSerialize(content));
}
function normalizeResearchDecisionAction(value2, options = {}) {
  const label = options.label ?? "Research decision action";
  exact(value2, ACTION_FIELDS, label);
  const action = { actionId: safeId5(value2.actionId, `${label}.actionId`), kind: enumValue(value2.kind, ACTION_KIND_SET, `${label}.kind`), description: text4(value2.description, `${label}.description`), rationale: text4(value2.rationale, `${label}.rationale`), targetHypothesisOrQuestionIds: ids(value2.targetHypothesisOrQuestionIds, `${label}.targetHypothesisOrQuestionIds`), successConditions: strings3(value2.successConditions, `${label}.successConditions`, 1), stopConditions: strings3(value2.stopConditions, `${label}.stopConditions`, 1), expectedEvidence: strings3(value2.expectedEvidence, `${label}.expectedEvidence`, 1), budget: budget(value2.budget, `${label}.budget`) };
  const suppliedDigest = hash5(value2.actionDigest, `${label}.actionDigest`);
...
function createResearchDecisionAction(value2) {
...
function normalizeResearchDecisionContent(value2 = {}) {
  exact(value2, CONTENT_FIELDS, "Research decision content");
  const hypotheses = array(value2.hypotheses, "hypotheses").map(normalizeHypothesis);
  const routes = array(value2.routes, "routes").map(normalizeRoute);
  const openQuestions = array(value2.openQuestions, "openQuestions").map(normalizeQuestion);
  unique(hypotheses, "hypothesisId", "Hypothesis");
  unique(routes, "routeId", "Route");
  unique(openQuestions, "questionId", "Open question");
  const evidenceRefs2 = strings3(value2.evidenceRefs, "evidenceRefs");
  const consumedReceiptIds = ids(value2.consumedReceiptIds, "consumedReceiptIds");
  const disposition = enumValue(value2.disposition, DISPOSITION_SET, "disposition");
  const reasonCodes = ids(value2.reasonCodes, "reasonCodes");
  const nextAction = value2.nextAction === null ? null : normalizeResearchDecisionAction(value2.nextAction, { label: "nextAction" });
  ...
  return { synthesis: text4(value2.synthesis, "synthesis"), hypotheses, routes, openQuestions, evidenceRefs: evidenceRefs2, consumedReceiptIds, disposition, reasonCodes, nextAction };
}
function envelope(value2, label) {
  const missionId2 = safeId5(value2.missionId, `${label}.missionId`);
  const contractDigest = hash5(value2.contractDigest, `${label}.contractDigest`);
  if (!Number.isSafeInteger(value2.revision) || value2.revision < 1) throw new Error(`${label}.revision must be a positive safe integer.`);
  const predecessorDecisionId = value2.predecessorDecisionId === null ? null : safeId5(value2.predecessorDecisionId, `${label}.predecessorDecisionId`);
  const predecessorDecisionDigest = value2.predecessorDecisionDigest === null ? null : hash5(value2.predecessorDecisionDigest, `${label}.predecessorDecisionDigest`);
  if (value2.revision === 1 !== (predecessorDecisionId === null && predecessorDecisionDigest === null)) throw new Error(`${label} predecessor binding does not match revision.`);
  return { missionId: missionId2, contractDigest, revision: value2.revision, predecessorDecisionId, predecessorDecisionDigest, createdAt: exactIso5(value2.createdAt, `${label}.createdAt`), content: normalizeResearchDecisionContent(value2.content ?? Object.fromEntries(RESEARCH_DECISION_CONTENT_FIELDS.map((field) => [field, value2[field]]))) };
}
function researchDecisionDigest(value2) {
  const normalized3 = envelope(value2, "Research decision digest input");
...
function createResearchDecision(args2 = {}) {
  exact(args2, CREATE_FIELDS, "createResearchDecision");
  const normalized3 = envelope(args2, "Research decision");
...
function validatePersistedResearchDecision(value2, options = {}) {
  exact(value2, PERSISTED_FIELDS, options.label ?? "Research decision");
  if (value2.schemaVersion !== RESEARCH_DECISION_SCHEMA_VERSION) throw new Error(`${options.label ?? "Research decision"} has an unsupported schemaVersion.`);
  const normalized3 = envelope(value2, options.label ?? "Research decision");
  const computedDigest = researchDecisionDigest({ ...normalized3, content: normalized3.content });
  if (value2.decisionDigest !== computedDigest || value2.decisionId !== researchDecisionId(normalized3.missionId, normalized3.revision, computedDigest)) throw new Error(`${options.label ?? "Research decision"} does not match its canonical content.`);
...
function appendResearchDecision(decisions, args2 = {}) {
  exact(args2, APPEND_FIELDS, "appendResearchDecision");
  const chain = validateResearchDecisionChain(decisions, { missionId: args2.missionId, contractDigest: args2.contractDigest });
  const head = chain.at(-1) ?? null;
  if ((args2.predecessorDecisionId ?? null) !== (head?.decisionId ?? null) || (args2.predecessorDecisionDigest ?? null) !== (head?.decisionDigest ?? null)) throw new Error("Research decision append rejected a stale predecessor.");
  const decision = createResearchDecision({ missionId: args2.missionId, contractDigest: args2.contractDigest, revision: (head?.revision ?? 0) + 1, predecessorDecisionId: head?.decisionId ?? null, predecessorDecisionDigest: head?.decisionDigest ?? null, createdAt: args2.createdAt, content: args2.content });
  return validateResearchDecisionChain([...chain, decision]);
}
function researchDecisionNarrativeDirective(value2) {
  const decision = validatePersistedResearchDecision(value2);
...
function stable(value2) {
  if (Array.isArray(value2)) return value2.map(stable);
  if (value2 && typeof value2 === "object") return Object.fromEntries(Object.entries(value2).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, stable(item)]));
  return value2;
}
function evidenceDigest(value2) {
  return crypto8.createHash("sha256").update(`${JSON.stringify(stable(value2))}
`).digest("hex");
}
...
function sealed5(value2, fields, label) {
  object(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
  return value2;
}
function text5(value2, label) {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  return value2.trim();
}
function id2(value2, label) {
  const normalized3 = text5(value2, label);
...
function strings4(value2, label, min = 0) {
  if (!Array.isArray(value2) || value2.some((item) => typeof item !== "string" || !item.trim())) throw new Error(`${label} must be an array of non-empty strings.`);
  const normalized3 = value2.map((item) => item.trim());
...
function enumeration(value2, values, label) {
  const normalized3 = text5(value2, label);
  if (!values.includes(normalized3)) throw new Error(`${label} is unsupported.`);
  return normalized3;
}
function number(value2, label) {
  if (typeof value2 !== "number" || !Number.isFinite(value2)) throw new Error(`${label} must be a finite number.`);
  return value2;
}
...
function normalizeExperimentProtocol(value2, label = "protocol") {
  sealed5(value2, PROTOCOL_FIELDS, label);
  return {
    question: text5(value2.question, `${label}.question`),
    hypothesis: text5(value2.hypothesis, `${label}.hypothesis`),
    procedure: strings4(value2.procedure, `${label}.procedure`, 1),
    inputs: strings4(value2.inputs, `${label}.inputs`, 1),
    comparisons: strings4(value2.comparisons, `${label}.comparisons`),
    metrics: strings4(value2.metrics, `${label}.metrics`, 1),
    successConditions: strings4(value2.successConditions, `${label}.successConditions`, 1),
    stopConditions: strings4(value2.stopConditions, `${label}.stopConditions`, 1),
    constraints: strings4(value2.constraints, `${label}.constraints`),
    expectedArtifacts: strings4(value2.expectedArtifacts, `${label}.expectedArtifacts`, 1),
    frozenAt: timestamp(value2.frozenAt, `${label}.frozenAt`)
  };
}
...
function normalizeExperimentResult(value2, label = "result") {
  sealed5(value2, RESULT_FIELDS, label);
  sealed5(value2.denominator, DENOMINATOR_FIELDS, `${label}.denominator`);
  const denominator = Object.fromEntries([...DENOMINATOR_FIELDS].map((field) => [field, number(value2.denominator[field], `${label}.denominator.${field}`)]));
  ...
  if (!Array.isArray(value2.failures)) throw new Error(`${label}.failures must be an array.`);
  const failures = value2.failures.map((item, index) => {
  ...
  if (!Array.isArray(value2.measurements)) throw new Error(`${label}.measurements must be an array.`);
  const measurements = value2.measurements.map((item, index) => {
  ...
  const status = enumeration(value2.status, EXPERIMENT_RESULT_STATUSES, `${label}.status`);
  const artifactRefs = strings4(value2.artifactRefs, `${label}.artifactRefs`);
  const validationRefs = strings4(value2.validationRefs, `${label}.validationRefs`);
  ...
  return {
    status,
    outcome: text5(value2.outcome, `${label}.outcome`),
    measurements,
    artifactRefs,
    validationRefs,
    denominator,
    failures,
    deviations: strings4(value2.deviations, `${label}.deviations`),
    limitations: strings4(value2.limitations, `${label}.limitations`, 1),
    recordedAt: timestamp(value2.recordedAt, `${label}.recordedAt`)
  };
}
...
function normalizeClaimContract(value2, label = "claim") {
  sealed5(value2, CLAIM_FIELDS, label);
  if (!Array.isArray(value2.experimentEvidence)) throw new Error(`${label}.experimentEvidence must be an array.`);
  const experimentEvidence = value2.experimentEvidence.map((item, index) => {
  ...
  const uncertainty = strings4(value2.uncertainty, `${label}.uncertainty`);
  const unsupportedExtensions = strings4(value2.unsupportedExtensions, `${label}.unsupportedExtensions`);
  const currentAssessment = enumeration(value2.currentAssessment, CLAIM_ASSESSMENTS, `${label}.currentAssessment`);
...
function stableValue6(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue6);
  if (value2 && typeof value2 === "object") {
    return Object.fromEntries(Object.entries(value2).sort(([left], [right]) => left.localeCompare(right)).map(([key, item]) => [key, stableValue6(item)]));
  }
  return value2;
}
function stableWorkspaceRevisionSerialize(value2) {
  return JSON.stringify(stableValue6(value2));
}
function sha2567(value2) {
  return crypto9.createHash("sha256").update(value2).digest("hex");
}
...
function safeId6(value2, label) {
  if (typeof value2 !== "string" || !SAFE_ID8.test(value2)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return value2;
}
function hash6(value2, label) {
  if (typeof value2 !== "string" || !HASH7.test(value2)) throw new Error(`${label} must be a lowercase SHA-256 digest.`);
  return value2;
}
function text6(value2, label) {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  return value2.trim();
}
function exactIso6(value2, label) {
...
function normalizedContent(value2) {
  const workspaceId = safeId6(value2.workspaceId, "Workspace revision workspaceId");
  if (!Number.isSafeInteger(value2.revision) || value2.revision < 1) throw new Error("Workspace revision number must be a positive safe integer.");
  const previousRevisionId = value2.previousRevisionId === null ? null : safeId6(value2.previousRevisionId, "Workspace revision previousRevisionId");
  const previousRevisionDigest = value2.previousRevisionDigest === null ? null : hash6(value2.previousRevisionDigest, "Workspace revision previousRevisionDigest");
  if (value2.revision === 1 && (previousRevisionId !== null || previousRevisionDigest !== null)) throw new Error("Workspace revision 1 requires null predecessor fields.");
  if (value2.revision > 1 && (previousRevisionId === null || previousRevisionDigest === null)) throw new Error(`Workspace revision ${value2.revision} requires predecessor fields.`);
  return {
    workspaceId,
    revision: value2.revision,
    previousRevisionId,
    previousRevisionDigest,
    mainline: text6(value2.mainline, "Workspace revision mainline"),
    changeReason: text6(value2.changeReason, "Workspace revision changeReason"),
    createdAt: exactIso6(value2.createdAt, "Workspace revision createdAt")
  };
}
function workspaceRevisionDigest(value2) {
  const content = normalizedContent(value2);
...
function createWorkspaceRevision(value2) {
  const content = normalizedContent(value2);
...
function validateWorkspaceRevision(value2, options = {}) {
  const label = options.label ?? "Workspace revision";
  assertSealed2(value2, FIELDS3, label);
  if (value2.schemaVersion !== WORKSPACE_REVISION_SCHEMA_VERSION) throw new Error(`${label} has an unsupported schemaVersion.`);
  const content = normalizedContent(value2);
  const revisionDigest = workspaceRevisionDigest(content);
  const revisionId = workspaceRevisionId(content.workspaceId, content.revision, revisionDigest);
  if (value2.revisionDigest !== revisionDigest) throw new Error(`${label}.revisionDigest does not match its canonical content.`);
  if (value2.revisionId !== revisionId) throw new Error(`${label}.revisionId does not match its canonical content.`);
...
function validateSchema12WorkspaceRevision(value2, options = {}) {
  const label = options.label ?? "Schema 12 workspace revision";
  assertSealed2(value2, SCHEMA_1_FIELDS, label);
  if (value2.schemaVersion !== 1) throw new Error(`${label} has an unsupported schemaVersion.`);
  const workspaceId = safeId6(value2.workspaceId, `${label}.workspaceId`);
  if (!Number.isSafeInteger(value2.revision) || value2.revision < 1) throw new Error(`${label}.revision must be a positive safe integer.`);
  const previousRevisionId = value2.previousRevisionId === null ? null : safeId6(value2.previousRevisionId, `${label}.previousRevisionId`);
  const previousRevisionDigest = value2.previousRevisionDigest === null ? null : hash6(value2.previousRevisionDigest, `${label}.previousRevisionDigest`);
  if (value2.revision === 1 && (previousRevisionId !== null || previousRevisionDigest !== null)) throw new Error(`${label} revision 1 requires null predecessor fields.`);
  if (value2.revision > 1 && (previousRevisionId === null || previousRevisionDigest === null)) throw new Error(`${label} revision ${value2.revision} requires predecessor fields.`);
  const content = {
    workspaceId,
    revision: value2.revision,
    previousRevisionId,
    previousRevisionDigest,
    projectGoal: text6(value2.projectGoal, `${label}.projectGoal`),
    researchMainline: text6(value2.researchMainline, `${label}.researchMainline`),
    changeReason: text6(value2.changeReason, `${label}.changeReason`),
    createdAt: exactIso6(value2.createdAt, `${label}.createdAt`)
  };
  const revisionDigest = sha2567(stableWorkspaceRevisionSerialize({ schemaVersion: 1, ...content }));
  const revisionId = workspaceRevisionId(workspaceId, value2.revision, revisionDigest);
  if (value2.revisionDigest !== revisionDigest || value2.revisionId !== revisionId) throw new Error(`${label} does not match its canonical schema 12 content.`);
...
function validateWorkspaceRevisionChain(values, project, options = {}) {
  ...
  const revisions = values.map((value2) => validateRevision(value2, { workspaceId: project.workspaceId }));
  const byId = /* @__PURE__ */ new Map();
...
function workspaceRevisionPath(revisionId) {
  ...
  return path10.posix.join(ARTIFACT_PATHS.workspaceRevisionsDir, `${revisionId}.json`);
}
...
function sha2568(value2) {
  return crypto10.createHash("sha256").update(value2).digest("hex");
}
function stableValue7(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue7);
  if (value2 && typeof value2 === "object") {
    return Object.fromEntries(Object.entries(value2).sort(([left], [right]) => left.localeCompare(right)).map(([key, item]) => [key, stableValue7(item)]));
  }
  return value2;
}
function stableWorkspaceSerialize(value2) {
  return JSON.stringify(stableValue7(value2));
}
function workspaceDigest(value2) {
  return sha2568(stableWorkspaceSerialize(value2));
}
function canonicalWorkspacePath(root) {
  return fs6.realpathSync.native(path11.resolve(root));
}
...
function exactIso7(value2, label) {
  if (typeof value2 !== "string" || !ISO_TIMESTAMP2.test(value2) || !Number.isFinite(Date.parse(value2)) || new Date(Date.parse(value2)).toISOString() !== value2) {
    throw new Error(`${label} must be an exact ISO-8601 timestamp.`);
  }
  return value2;
}
function safeId7(value2, label) {
  if (typeof value2 !== "string" || !SAFE_ID9.test(value2)) {
...
function validateDoveManifest(value2, options = {}) {
  assertSealed3(value2, MANIFEST_FIELDS, "Dove manifest");
  const expectedSchemaVersion = options.expectedSchemaVersion ?? DOVE_WORKSPACE_SCHEMA_VERSION;
  if (value2.schemaVersion !== expectedSchemaVersion) {
    throw new Error(`Dove manifest schemaVersion ${value2.schemaVersion ?? "missing"} is unsupported; expected ${expectedSchemaVersion}.`);
  }
  ...
  safeId7(value2.workspaceId, "Dove manifest workspaceId");
  exactIso7(value2.createdAt, "Dove manifest createdAt");
...
function validateDoveProject(value2, manifest) {
  assertSealed3(value2, PROJECT_FIELDS, "Dove project identity");
  ...
  safeId7(value2.workspaceId, "Dove project workspaceId");
  ...
  safeId7(value2.currentRevisionId, "Dove project currentRevisionId");
  hash7(value2.currentRevisionDigest, "Dove project currentRevisionDigest");
  exactIso7(value2.createdAt, "Dove project createdAt");
  exactIso7(value2.updatedAt, "Dove project updatedAt");
  if (value2.createdAt !== manifest.createdAt) {
    throw new Error("Dove project createdAt must match the manifest createdAt.");
  }
  return value2;
}
function hash7(value2, label) {
  if (typeof value2 !== "string" || !HASH8.test(value2)) throw new Error(`${label} must be a lowercase SHA-256 digest.`);
  return value2;
}
...
function validateLessonsMarkdown(value2, label = "Dove Lessons document") {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be non-empty Markdown.`);
  if (value2.includes("\0")) throw new Error(`${label} must not contain null bytes.`);
  if (!value2.endsWith("\n")) throw new Error(`${label} must end with a newline.`);
  const headings = [...value2.matchAll(/^##\s+(.+?)\s*$/gmu)].map((match) => match[1]);
...
function evidenceReferenceOwner(reference, receiptLedger) {
  if (reference.startsWith("artifact:")) {
    const target2 = reference.slice("artifact:".length);
    return receiptLedger.currentOwnership.find((item) => item.path === target2) ?? null;
  }
  if (reference.startsWith("validation:")) {
    const target2 = reference.slice("validation:".length);
    for (const receipt of receiptLedger.receipts.toReversed()) {
      const validation = receipt.validations.find((item) => item.reference === target2);
...
function validateCurrentEvidenceReferences(root, references, missionId2, receiptLedger, missionGraph, label, sources = /* @__PURE__ */ new Map()) {
  ...
  for (const [index, reference] of references.entries()) {
    ...
    const target2 = reference.slice(reference.indexOf(":") + 1);
    const fullPath = path11.join(root, target2);
...
function readExperimentRecords(root, missions, missionGraph, receiptLedger, sources) {
  const directory = path11.join(root, ARTIFACT_PATHS.experimentsDir);
  ...
  for (const entry of fs6.readdirSync(directory, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    const relativePath = path11.posix.join(ARTIFACT_PATHS.experimentsDir, entry.name);
    ...
    const value2 = readJsonStrict2(path11.join(root, relativePath), relativePath);
    if (value2.schemaVersion !== EXPERIMENT_RECORD_SCHEMA_VERSION) throw new Error(`${relativePath} has an unsupported schemaVersion.`);
    assertSealed3(value2, suffix === "plan" ? EXPERIMENT_PLAN_FIELDS : EXPERIMENT_RESULT_FIELDS, relativePath);
    const experimentId = safeId7(value2.experimentId, `${relativePath}.experimentId`);
    const missionId2 = safeId7(value2.missionId, `${relativePath}.missionId`);
    ...
    if (!owner || owner.missionId !== missionId2 || owner.sha256 !== sha2568(fs6.readFileSync(path11.join(root, relativePath)))) throw new Error(`${relativePath} is not current hash-matching mission-owned Experiment material.`);
    if (suffix === "plan") {
      nonEmptyString(value2.title, `${relativePath}.title`);
      const protocol = normalizeExperimentProtocol(value2.protocol, `${relativePath}.protocol`);
      if (value2.protocolDigest !== evidenceDigest(protocol)) throw new Error(`${relativePath}.protocolDigest does not bind the frozen protocol.`);
      exactIso7(value2.updatedAt, `${relativePath}.updatedAt`);
    } else {
      if (safeId7(value2.resultId, `${relativePath}.resultId`) !== experimentId) throw new Error(`${relativePath}.resultId must match experimentId.`);
      const { resultDigest, schemaVersion: _schemaVersion, resultId: _resultId, experimentId: _experimentId, missionId: _missionId, protocolDigest: _protocolDigest, ...contract } = value2;
      normalizeExperimentResult(contract, relativePath);
      if (resultDigest !== evidenceDigest({ schemaVersion: value2.schemaVersion, resultId: value2.resultId, experimentId: value2.experimentId, missionId: value2.missionId, protocolDigest: value2.protocolDigest, ...contract })) throw new Error(`${relativePath}.resultDigest does not bind the exact result.`);
      validateCurrentEvidenceReferences(root, value2.artifactRefs, missionId2, receiptLedger, missionGraph, `${relativePath}.artifactRefs`, sources);
      validateCurrentEvidenceReferences(root, value2.validationRefs, missionId2, receiptLedger, missionGraph, `${relativePath}.validationRefs`, sources);
      for (const [index, failure] of value2.failures.entries()) validateCurrentEvidenceReferences(root, failure.evidenceRefs, missionId2, receiptLedger, missionGraph, `${relativePath}.failures[${index}].evidenceRefs`, sources);
    }
...
function currentOwnedJson(root, relativePath, missionId2, receiptLedger, fields, label) {
  ...
  const value2 = readJsonStrict2(fullPath, relativePath);
  assertSealed3(value2, fields, label);
  return value2;
}
function readSourceRecords(root, missions, receiptLedger) {
  const directory = path11.join(root, ARTIFACT_PATHS.sourcesDir);
  ...
  for (const entry of fs6.readdirSync(directory, { withFileTypes: true })) {
    ...
    const relativePath = path11.posix.join(ARTIFACT_PATHS.sourcesDir, entry.name);
    ...
    const peek = readJsonStrict2(path11.join(root, relativePath), relativePath);
    ...
    const materialFile = path11.join(root, materialPath);
...
function readClaimRecords(root, missions, missionGraph, receiptLedger, experiments, sources) {
  ...
  for (const entry of fs6.readdirSync(path11.join(root, ARTIFACT_PATHS.claimsDir), { withFileTypes: true })) {
    const relativePath = path11.posix.join(ARTIFACT_PATHS.claimsDir, entry.name);
...
function inspectDoveWorkspaceVersion(root) {
  ...
  try {
    ...
    const problems = [
      ...
      ...CURRENT_SCHEMA_FORBIDDEN_LEGACY_PATHS.filter((relativePath) => fs6.existsSync(path11.join(workspace, relativePath))).map((relativePath) => `${relativePath} is a retained legacy artifact and must not coexist with current schema ${DOVE_WORKSPACE_SCHEMA_VERSION}`)
    ].filter(Boolean);
    ...
    const workspaceRevisionValues = validateJsonDirectory(workspace, ARTIFACT_PATHS.workspaceRevisionsDir, manifest, (value2, currentManifest, label) => validateWorkspaceRevision(value2, {
      label,
    ...
    const missionTransitionValues = validateJsonDirectory(workspace, ARTIFACT_PATHS.missionTransitionsDir, manifest, (value2, currentManifest, label) => ({ value: value2, currentManifest, label }));
    const missionTransitions = validateMissionTransitions(missionTransitionValues.map(({ value: value2 }) => value2), { workspaceId: manifest.workspaceId, missions });
    const artifactHandoffValues = validateJsonDirectory(workspace, ARTIFACT_PATHS.artifactHandoffsDir, manifest, (value2, currentManifest, label) => ({ value: value2, currentManifest, label }));
    const artifactHandoffs = validateArtifactHandoffs(artifactHandoffValues.map(({ value: value2 }) => value2), { workspaceId: manifest.workspaceId, missions });
    const researchDecisionValues = validateJsonDirectory(workspace, ARTIFACT_PATHS.researchDecisionsDir, manifest, (value2, _manifest, label) => {
      const decision = validatePersistedResearchDecision(value2, { label });
      if (path11.posix.basename(label) !== `${decision.decisionId}.json`) throw new Error(`${label} filename must match decisionId ${decision.decisionId}.`);
    ...
    const lessonsDocument = fs6.readFileSync(path11.join(workspace, ARTIFACT_PATHS.lessonsDocument), "utf8");
...
function stableId(prefix, value2) {
  return `${prefix}-${crypto11.createHash("sha256").update(stableResearchDecisionSerialize(value2)).digest("hex").slice(0, 20)}`;
}
...
function safeId8(value2, label) {
  if (typeof value2 !== "string" || !SAFE_ID10.test(value2)) {
    throw new Error(`${label} must be a safe lowercase identifier.`);
  }
  return value2;
}
function researchDecisionPath(decisionId) {
  return path13.posix.join(ARTIFACT_PATHS.researchDecisionsDir, `${safeId8(decisionId, "decisionId")}.json`);
}
...
function appendResearchDecision2(root, args2 = {}) {
  assertGovernanceMutationRegistered("append-research-decision", "guarded");
  assertSealed4(args2, APPEND_FIELDS2, "appendResearchDecision");
  const context = currentMutationContext(root);
  ...
  const missionId2 = safeId8(args2.missionId, "missionId");
  ...
  context.requireCommitPrecondition(path13.posix.join(ARTIFACT_PATHS.missionsDir, `${mission.missionId}.json`));
  ...
  const nextChain = appendResearchDecision(currentChain, {
    ...
    contractDigest: mission.contractDigest,
    predecessorDecisionId: args2.predecessorDecisionId,
    predecessorDecisionDigest: args2.predecessorDecisionDigest,
  ...
  if (context.fileExists(relativePath) || fs8.existsSync(path13.resolve(root, relativePath))) {
...
function assertReviewMissionBinding(workspace, mission, value2) {
  if (typeof value2 !== "string" || !BINDING_PATTERN.test(value2) || value2 !== reviewMissionBinding(workspace, mission)) {
...
function stable2(value2) {
  if (Array.isArray(value2)) return value2.map(stable2);
  if (value2 && typeof value2 === "object") return Object.fromEntries(Object.entries(value2).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => [key, stable2(item)]));
  return value2;
}
function sha2569(value2) {
  return crypto13.createHash("sha256").update(JSON.stringify(stable2(value2))).digest("hex");
}
function plain5(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function sealed6(value2, fields, label) {
  plain5(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
function text7(value2, label) {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  return value2.trim();
}
function safeId9(value2, label) {
  const result = text7(value2, label);
  if (!SAFE_ID11.test(result)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return result;
}
function hash8(value2, label) {
  const result = text7(value2, label);
  if (!HASH9.test(result)) throw new Error(`${label} must be a lowercase SHA-256 digest.`);
  return result;
}
function exactIso8(value2, label) {
  if (typeof value2 !== "string" || !ISO2.test(value2) || new Date(Date.parse(value2)).toISOString() !== value2) throw new Error(`${label} must be an exact ISO-8601 timestamp.`);
  return value2;
}
function outcomeIso(value2, label) {
  if (typeof value2 !== "string" || !OUTCOME_TIME.test(value2)) throw new Error(`${label} must be an ISO-8601 UTC timestamp with seconds and optional milliseconds.`);
  const result = new Date(Date.parse(value2)).toISOString();
  if (result !== (value2.includes(".") ? value2 : value2.replace("Z", ".000Z"))) throw new Error(`${label} must be an ISO-8601 UTC timestamp.`);
  return result;
}
function strings5(value2, label) {
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const result = value2.map((item, index) => text7(item, `${label}[${index}]`));
  if (new Set(result).size !== result.length) throw new Error(`${label} must not contain duplicates.`);
  return result;
}
function budget2(value2, label, allowEmpty = false) {
  plain5(value2, label);
  const result = {};
  for (const [dimension, amount] of Object.entries(value2)) {
    if (!Number.isSafeInteger(amount) || amount < 0) throw new Error(`${label}.${dimension} must be a non-negative safe integer.`);
...
function normalizeEnvelope(value2) {
  sealed6(value2, ENVELOPE_FIELDS, "Research handoff");
  if (value2.schemaVersion !== RESEARCH_HANDOFF_SCHEMA_VERSION) throw new Error("Research handoff has an unsupported schemaVersion.");
  const result = { schemaVersion: RESEARCH_HANDOFF_SCHEMA_VERSION, envelopeId: safeId9(value2.envelopeId, "Research handoff envelopeId"), missionId: safeId9(value2.missionId, "Research handoff missionId"), contractDigest: hash8(value2.contractDigest, "Research handoff contractDigest"), decisionDigest: hash8(value2.decisionDigest, "Research handoff decisionDigest"), actionId: safeId9(value2.actionId, "Research handoff actionId"), actionDigest: hash8(value2.actionDigest, "Research handoff actionDigest"), budget: budget2(value2.budget, "Research handoff budget"), expectedEvidence: strings5(value2.expectedEvidence, "Research handoff expectedEvidence"), issuedAt: exactIso8(value2.issuedAt, "Research handoff issuedAt"), expiresAt: exactIso8(value2.expiresAt, "Research handoff expiresAt"), supersedesEnvelopeId: value2.supersedesEnvelopeId === null ? null : safeId9(value2.supersedesEnvelopeId, "Research handoff supersedesEnvelopeId"), seal: hash8(value2.seal, "Research handoff seal") };
...
function createResearchHandoff(value2, options = {}) {
  const decision = validatePersistedResearchDecision(value2, { label: "Research handoff decision" });
...
function validateResearchHandoff(value2, current = {}) {
  const envelope2 = normalizeEnvelope(value2);
...
function assertAllowed(args2) {
  assertPlainObject5(args2, "Dove workspace arguments");
...
function operationFrom(args2) {
  const operation = args2.operation ?? "initialize";
...
function exactReplayInput(args2) {
  const operation = operationFrom(args2);
  return {
    operation,
    mainline: text8(args2.mainline, "Dove workspace research mainline"),
    changeReason: text8(args2.changeReason, "Dove workspace change reason"),
    ...
    currentRevisionId: optionalReplayString(args2.currentRevisionId, "currentRevisionId"),
    currentRevisionDigest: optionalReplayString(args2.currentRevisionDigest, "currentRevisionDigest"),
    activeMissionIds: Array.isArray(args2.activeMissionIds) ? [...args2.activeMissionIds] : []
  };
}
function assertArchiveTargetSafe(workspace, archiveTarget) {
  const archiveParent = path14.join(workspace, ".dove-archive");
  if (path14.dirname(archiveTarget) !== archiveParent) throw new Error("Dove archive target must be the deterministic workspace-local .dove-archive target.");
...
function initializeProposal(root, args2, workspace, replay) {
  ...
  const mainline = text8(args2.mainline, "Dove workspace research mainline");
  const changeReason = typeof args2.changeReason === "string" && args2.changeReason.trim() ? args2.changeReason.trim() : "Establish the initial research mainline.";
  const mutationMode3 = mutationModeFor(workspace, args2);
  if (archiveReset && mutationMode3 === "patch-plan") throw new Error("Dove workspace archive-reset requires direct-process transaction semantics.");
...
function reviseProposal(root, args2, workspace, replay) {
  if (args2.archiveReset === true) throw new Error("revise-mainline does not accept archiveReset.");
  ...
  const mainline = text8(args2.mainline, "Dove workspace research mainline");
  const changeReason = text8(args2.changeReason, "Dove workspace mainline change reason");
...
function buildProposal(root, args2 = {}) {
  ...
  const operation = operationFrom(args2);
  const proposal = operation === DOVE_REVISE_MAINLINE_OPERATION ? reviseProposal(root, args2, workspace, replay) : initializeProposal(root, args2, workspace, replay);
...
function proposalOperations(proposal) {
  ...
  return proposal.envelope.archiveReset ? [{ type: "atomic-directory-rename", from: ".dove", to: path14.relative(proposal.envelope.workspace, proposal.envelope.archiveTarget).split(path14.sep).join("/") }, ...initialize] : initialize;
}
function mutationPaths(proposal) {
  ...
  return [
    ...proposal.envelope.archiveReset ? [".dove", path14.relative(proposal.envelope.workspace, proposal.envelope.archiveTarget).split(path14.sep).join("/")] : [],
...
function stageInitialization(context, proposal) {
  if (proposal.envelope.archiveReset) context.replaceDirectory(".dove", { archiveTarget: path14.relative(proposal.envelope.workspace, proposal.envelope.archiveTarget).split(path14.sep).join("/") });
...
function initDoveWorkspace(root, args2 = {}) {
  assertAllowed(args2);
...
function sha25610(value2) {
  return crypto14.createHash("sha256").update(value2).digest("hex");
}
function text9(value2, fallback = null) {
  if (typeof value2 !== "string") return fallback;
  const result = value2.trim();
  return result || fallback;
}
function plain6(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function allowed(value2, fields, label) {
  plain6(value2, `${label} arguments`);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
...
function canonicalWorkspace(root) {
  return fs10.realpathSync.native(path15.resolve(root));
}
function slugify(value2) {
  return String(value2 ?? "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48) || "mission";
}
function missionId(value2, goal) {
  const id3 = text9(value2, `mission-${slugify(goal)}-${sha25610(goal).slice(0, 10)}`);
  if (!SAFE_ID12.test(id3)) throw new Error("missionId must be a safe lowercase identifier.");
  return id3;
}
function missionPath(id3) {
  return path15.posix.join(ARTIFACT_PATHS.missionsDir, `${id3}.json`);
}
...
function nextCreatedAt(workspace) {
  const now = nowIso();
  const latest = [...workspace.missions.values()].reduce((value2, mission) => mission.createdAt > value2 ? mission.createdAt : value2, "");
  return latest && now <= latest ? addMilliseconds(latest, 1) : now;
}
function replayTimestamp(args2, field, fallback, confirmed, label) {
  const supplied = text9(args2[field], null);
...
function mutationMode(root, args2) {
  const explicit = Object.hasOwn(args2, "mutationMode") ? normalizeMutationMode(args2.mutationMode) : null;
  const active = currentMutationContext(root)?.mutationMode ?? null;
...
function exists(root, relativePath) {
  return currentMutationContext(root)?.fileExists(relativePath) ?? fs10.existsSync(path15.join(root, relativePath));
}
function artifactIdentity(root, rawPath) {
  ...
  const fullPath = path15.join(root, relativePath);
...
function normalizeHandoffPaths(value2) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error("handoffArtifactPaths must be an array.");
  const result = value2.map((item, index) => {
...
function branchRecords(workspace, content, args2, createdAt) {
  if (!content.parentMissionId) {
    if (args2.stopParentReason !== void 0 || args2.handoffArtifactPaths !== void 0) throw new Error("Root mission creation must not declare parent handoff fields.");
  ...
  const ordinaryChild = args2.operation === "create-child";
  if (ordinaryChild) {
    if (args2.stopParentReason !== void 0 || args2.handoffArtifactPaths !== void 0) throw new Error("Ordinary child mission creation must not stop its parent or transfer artifact ownership.");
    return { transition: null, artifactHandoffs: [] };
  }
  const existingTransition = workspace.missionTransitions.get(parent.missionId) ?? null;
  const reason = text9(args2.stopParentReason, null);
  ...
  const artifactHandoffs = normalizeHandoffPaths(args2.handoffArtifactPaths).map((artifactPath) => {
...
function buildProposal2(root, args2) {
  ...
  const content = normalizeMissionContractContent(Object.fromEntries(Object.entries(args2).filter(([field]) => !CREATE_EXTRA_FIELDS.has(field) && !REPLAY_FIELDS.has(field))));
  const id3 = missionId(args2.missionId, content.goal);
  if (content.parentMissionId === id3) throw new Error("Mission must not be its own parent.");
  content.missionId = id3;
  if (args2.confirmed && (args2.workspaceId !== opened.manifest.workspaceId || args2.workspaceRevisionId !== revision.revisionId || args2.workspaceRevisionDigest !== revision.revisionDigest)) throw new Error("Mission replay no longer matches the current workspace revision.");
  const createdAt = replayTimestamp(args2, "createdAt", nextCreatedAt(opened), args2.confirmed === true, "mission createdAt");
  ...
  const branch = branchRecords(opened, persisted, args2, createdAt);
  let researchDecision = null;
  let researchHandoff = null;
  if (content.mode === "research") {
    const decisionCreatedAt = replayTimestamp(args2, "decisionCreatedAt", createdAt, args2.confirmed === true, "research decision createdAt");
    const handoffIssuedAt = replayTimestamp(args2, "handoffIssuedAt", decisionCreatedAt, args2.confirmed === true, "research handoff issuedAt");
    const handoffExpiresAt = replayTimestamp(args2, "handoffExpiresAt", addMilliseconds(handoffIssuedAt, FOUR_HOURS_MS), args2.confirmed === true, "research handoff expiresAt");
  ...
  const mode2 = mutationMode(root, args2);
  const creationOperation = args2.operation ?? (content.parentMissionId ? "branch" : "create-root");
  const envelope2 = { proposalVersion: MISSION_PROPOSAL_VERSION, workspace: workspacePath, mutationMode: mode2, workspaceSchemaVersion: DOVE_WORKSPACE_SCHEMA_VERSION, workspaceId: opened.manifest.workspaceId, workspaceRevisionId: revision.revisionId, workspaceRevisionDigest: revision.revisionDigest, creationOperation, targetArtifactIdentities: targetIdentities, mission: persisted, ...researchDecision ? { researchDecision, researchHandoff } : {}, parentTransitionId: branch.transition?.transitionId ?? null, artifactHandoffIds: branch.artifactHandoffs.map((item) => item.handoffId) };
  return { workspace: workspacePath, opened, revision, mutationMode: mode2, creationOperation, content, mission: persisted, researchDecision, researchHandoff, targetIdentities, branch, proposalDigest: sha25610(stableMissionSerialize(envelope2)) };
}
...
function normalizeSkillId(value2) {
  const skill = text9(value2, null);
  if (!SKILL_ID_SET.has(skill)) throw new Error(`Skill must be one of: ${DOVE_RESEARCH_SKILL_IDS.join(", ")}.`);
  return skill;
}
function normalizeContextArtifactPaths(value2) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error("contextArtifactPaths must be an array.");
  const paths2 = value2.map((item, index) => {
    const normalized3 = normalizeProjectRelativePath(item);
...
function skillParentMission(workspace, args2) {
  const explicitParentId = text9(args2.parentMissionId, null);
  const contextPaths = normalizeContextArtifactPaths(args2.contextArtifactPaths);
...
function startDoveSkillMission(root, args2 = {}) {
  assertGovernanceMutationRegistered("create-dove-mission", "guarded");
  allowed(args2, SKILL_START_FIELDS, "Dove skill start");
  if (args2.operation !== "start-skill") throw new Error("Dove skill start requires operation=start-skill.");
  if (!currentMutationContext(root)) throw new Error("Dove skill start requires an active MutationContext.");
  const skill = normalizeSkillId(args2.skill);
  const goal = text9(args2.goal, null);
  ...
  const parent = skillParentMission(workspace, args2);
  const contextPaths = normalizeContextArtifactPaths(args2.contextArtifactPaths);
  const contractArgs = Object.fromEntries(Object.entries(args2).filter(([field]) => !["operation", "skill", "parentMissionId", "contextArtifactPaths"].includes(field)));
...
function previewDoveMissionContract(root, args2 = {}) {
  allowed(args2, new Set(MISSION_CONTRACT_INPUT_FIELDS), "Dove mission preview");
...
function createDoveMission(root, args2 = {}) {
  assertGovernanceMutationRegistered("create-dove-mission", "guarded");
  const operation = args2.operation ?? (args2.parentMissionId ? "branch" : "create-root");
  ...
  allowed(args2, /* @__PURE__ */ new Set([...MISSION_CONTRACT_INPUT_FIELDS, ...REPLAY_FIELDS, ...CREATE_EXTRA_FIELDS]), "create_dove_mission");
  const proposal = buildProposal2(root, args2);
  if (!args2.confirmed) return { status: "needs-confirmation", mission: proposal.mission, contractDigest: proposal.mission.contractDigest, approval: { required: true, noChangesApplied: true, summary: `Dove can save this immutable mission: ${proposal.mission.goal}`, effects: ["Bind the mission directly to the current workspace revision.", "Save one mission-owned delivery and evidence contract."], question: "Create this mission checkpoint?" }, confirmation: { required: true, exactReplay: true, confirmArgs: confirmation(proposal) }, mutation: { mutationMode: proposal.mutationMode, writesApplied: false, paths: [] } };
  ...
  if (args2.proposalVersion !== MISSION_PROPOSAL_VERSION || args2.proposalWorkspace !== proposal.workspace || args2.proposalDigest !== proposal.proposalDigest) throw new Error("The selected mission proposal no longer matches current state.");
...
function createAmbientDoveMission(root, args2 = {}) {
  assertGovernanceMutationRegistered("create-ambient-dove-mission", "guarded");
  allowed(args2, AMBIENT_FIELDS, "create_ambient_dove_mission");
  if (!MISSION_MODES.includes(args2.mode)) throw new Error(`Ambient mission creation requires explicit mode: ${MISSION_MODES.join(" or ")}.`);
  if (args2.changesWorkspaceMainline !== false || !text9(args2.mainlineAlignment, null)) throw new Error("Ambient mission creation requires current-mainline alignment and no mainline change.");
  const proposal = createDoveMission(root, { ...Object.fromEntries(Object.entries(args2).filter(([field]) => !["mainlineAlignment", "changesWorkspaceMainline"].includes(field))), missionId: newMissionId(), operation: "create-root", mutationMode: "direct-process" });
  return createDoveMission(root, proposal.confirmation.confirmArgs);
}
function manageDoveWorkspace(root, args2 = {}) {
  assertGovernanceMutationRegistered("manage-dove-workspace", "guarded");
  allowed(args2, WORKSPACE_FIELDS, "Dove workspace");
  if (!["set-mainline", "initialize", "revise-mainline"].includes(args2.operation)) return initDoveWorkspace(root, args2);
  if (args2.operation !== "set-mainline") return initDoveWorkspace(root, args2);
  const projectBrief = text9(args2.projectBrief, null);
  const mainline = text9(args2.mainline, null);
...
function sha256Buffer(value2) {
  return crypto15.createHash("sha256").update(value2).digest("hex");
}
...
function snapshotArtifactBuffer(root, relativePath, label = "artifact") {
  ...
  if (mutationContext && typeof mutationContext.readFileSnapshot === "function") {
    ...
    content = fs11.readFileSync(path16.resolve(root, canonicalPath4));
  }
...
function stableSnapshotSetHash(snapshots = []) {
  const canonical = [...snapshots].map(({ path: artifactPath, sizeBytes, sha256: sha25616 }) => ({ path: artifactPath, sizeBytes, sha256: sha25616 })).sort((left, right) => left.path.localeCompare(right.path));
...
function resolveReviewArtifactSnapshots(root, missionId2, relativePaths, label = "reviewed artifacts", options = {}) {
  ...
  for (const [index, relativePath] of relativePaths.entries()) {
    ...
    const snapshot = {
      ...
      sha256: sha256File(path16.resolve(root, canonicalPath4))
    };
...
function verifyReviewSnapshotSet(root, preparedSnapshots, expectedSetHash) {
  ...
  for (const prepared of normalized3.snapshots) {
    ...
    const current = { path: canonicalPath4, sizeBytes: inspection.sizeBytes, sha256: sha256File(path16.resolve(root, canonicalPath4)) };
...
function domainSafeId(value2, label) {
  const normalized3 = typeof value2 === "string" ? value2.trim() : "";
...
function domainStringArray(value2, label, options = {}) {
  const source = value2 === void 0 ? [] : value2;
...
function domainSha256(value2) {
  return crypto16.createHash("sha256").update(value2).digest("hex");
}
function readCurrentMission(root, missionId2, operation = "Domain workflow") {
  ...
  const relativePath = path17.posix.join(ARTIFACT_PATHS.missionsDir, `${normalizedMissionId}.json`);
  const fullPath = path17.resolve(root, relativePath);
...
function currentFileHash(root, relativePath, label) {
  ...
  return { path: canonicalPath4, sha256: sha256File(path17.resolve(root, canonicalPath4)) };
}
...
function normalizeWrite(root, missionId2, item, index, options = {}) {
  ...
  if (Buffer.isBuffer(item.content) && isPatchPlanMode(root) && path17.extname(relativePath).toLowerCase() !== ".svg") {
...
function stageConsolidatedDomainMutation(root, options = {}) {
  ...
  const receiptPath2 = path17.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`);
...
function finalizeDomainArtifacts(root, options = {}) {
  ...
  const artifacts = writes.map(({ path: artifactPath, kind, sha256: sha25616 }) => ({ path: artifactPath, kind, sha256: sha25616 }));
...
function text10(value2) {
  return typeof value2 === "string" ? value2.trim().replace(/\s+/gu, " ") : "";
}
function sealed7(value2, fields, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
  return value2;
...
function sourcePath(id3) {
  return path18.posix.join(".dove/sources", `${id3}.json`);
}
...
function readSourceFiles(root) {
  ...
  return fs13.readdirSync(path18.resolve(root, ".dove/sources"), { withFileTypes: true }).filter((entry) => entry.name !== "materials").map((entry) => {
    const relativePath = path18.posix.join(".dove/sources", entry.name);
...
function newSource(args2, material, contractDigest, existing) {
  const title = text10(args2.title);
  const locator = text10(args2.locator);
  if (!title && !locator) throw new Error("register_source requires a real title or locator.");
  const capturedAt = (/* @__PURE__ */ new Date()).toISOString();
  return {
    schemaVersion: SOURCE_SCHEMA_VERSION,
    sourceId: domainSafeId(args2.sourceId, "sourceId"),
    missionId: domainSafeId(args2.missionId, "missionId"),
    contractDigest,
    citationKey: text10(args2.citationKey) || null,
    title: title || null,
    authors: domainStringArray(args2.authors, "authors"),
    year: args2.year === void 0 || args2.year === null || String(args2.year).trim() === "" ? null : String(args2.year).trim(),
    locator: locator || null,
    sourceType: text10(args2.sourceType) || null,
    abstract: text10(args2.abstract) || null,
    origin: text10(args2.origin) || null,
...
function registerSource(root, args2 = {}) {
  ...
  const id3 = domainSafeId(args2.sourceId, "sourceId");
  const relativePath = sourcePath(id3);
  const existing = fs13.existsSync(path18.resolve(root, relativePath)) ? readJson(root, relativePath, null) : null;
  if (existing && existing.missionId !== mission.missionId) throw new Error(`Source ${id3} belongs to another mission.`);
  const captured = materialSnapshot(root, args2.capturePath, true);
  const materialPath = path18.posix.join(".dove/sources/materials", `${id3}${path18.extname(captured.path).toLowerCase() || ".bin"}`);
  const source = newSource(args2, { path: materialPath, sizeBytes: captured.sizeBytes, sha256: captured.sha256 }, mission.contractDigest, existing);
...
function verifySource(root, args2 = {}) {
  ...
  const next = { ...source, lifecycle: "rejected", currentDecision: { decision: "rejected", method: domainNonEmptyText(args2.method, "method"), checkedMaterial: domainNonEmptyText(args2.checkedMaterial, "checkedMaterial"), auditEvidence: auditEvidence(args2.auditEvidence), decidedAt: (/* @__PURE__ */ new Date()).toISOString() } };
...
function querySources(root, args2 = {}) {
  ...
  const { workspace, mission } = readCurrentMission(root, args2.missionId, "Source query");
  const sourceId = text10(args2.sourceId);
  const lifecycle = text10(args2.lifecycle).toLowerCase();
...
function missionPath2(missionId2) {
  return path19.posix.join(ARTIFACT_PATHS.missionsDir, `${missionId2}.json`);
}
function sealed8(value2, allowed2) {
  return value2 && typeof value2 === "object" && !Array.isArray(value2) && Object.keys(value2).every((key) => allowed2.has(key));
}
...
function typedEvidenceEligibility(root, missionId2, reference) {
  if (reference.startsWith("source:")) {
    ...
    const evaluation = evaluateSourceReferences(root, [value2], missionId2)[0] ?? { eligible: false, reason: "unknown-source" };
...
function assessValidation(root, validation) {
  ...
  try {
    ...
    const target2 = currentHashedFile(root, targetPath, normalized3.targetHash);
    if (!target2.current) return { path: normalized3.reference, current: false, result: normalized3.result, level: normalized3.level, producerKind: normalized3.producerKind, completionEligible: false, reason: "validation-target-drift", recordedSha256: normalized3.outputHash, targetReference: normalized3.targetReference, targetHash: normalized3.targetHash };
...
function assessMissionFromWorkspace(root, workspace, missionId2, state2) {
  if (state2.memo.has(missionId2)) return state2.memo.get(missionId2);
  if (state2.visiting.has(missionId2)) throw new Error(`Mission dependency assessment contains a cycle at ${missionId2}.`);
  const mission = workspace.missions.get(missionId2);
  if (!mission) throw new Error(`Mission does not exist: ${missionId2}.`);
  state2.visiting.add(missionId2);
  try {
    ...
    state2.memo.set(missionId2, assessment);
    return assessment;
  } finally {
    state2.visiting.delete(missionId2);
  }
}
...
function assessMissionCompletion(root, args2 = {}) {
  ...
  const missionId2 = typeof args2.missionId === "string" ? args2.missionId.trim() : "";
...
function completeMissionIfEligible(root, args2 = {}) {
  if (!args2 || typeof args2 !== "object" || Array.isArray(args2)) throw new Error("Mission completion transition arguments must be a plain object.");
  const unknown = Object.keys(args2).filter((field) => !["missionId", "createdAt"].includes(field));
  ...
  const state2 = assessmentState(workspace);
  const assessments = new Map([...workspace.missions.keys()].map((candidateMissionId) => [
    candidateMissionId,
    assessMissionFromWorkspace(root, workspace, candidateMissionId, state2)
  ]));
  const createdAt = args2.createdAt ?? nowIso();
...
function assertPlainRecord(value2, label) {
  if (value2 === null || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
  const prototype = Object.getPrototypeOf(value2);
  if (prototype !== Object.prototype && prototype !== null) throw new Error(`${label} must be a plain object.`);
  for (const key of Reflect.ownKeys(value2)) {
...
function assertSealedRecord(value2, fields, label) {
  assertPlainRecord(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length > 0) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
}
function assertJsonArray(value2, label) {
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const unexpected = Reflect.ownKeys(value2).filter((key) => typeof key !== "string" || key !== "length" && !/^(?:0|[1-9][0-9]*)$/u.test(key));
  if (unexpected.length > 0) throw new Error(`${label} must not contain custom fields.`);
}
function normalizeResearchNarrativeText(value2, label = "Research narrative text") {
  if (typeof value2 !== "string" || !value2.trim()) throw new Error(`${label} must be a non-empty string.`);
  const normalized3 = value2.trim().replace(/\s+/gu, " ");
...
function safeResearchNarrativeSourceText(value2) {
  try {
    return normalizeResearchNarrativeText(value2);
  } catch {
    return null;
  }
}
function normalizeTexts(value2, label) {
  assertJsonArray(value2, label);
  return Object.freeze([...new Set(value2.map((item, index) => normalizeResearchNarrativeText(item, `${label}[${index}]`)))]);
}
function normalizeObjects(value2, fields, label, normalizeItem) {
  assertJsonArray(value2, label);
  return Object.freeze(value2.map((item, index) => {
...
function normalizeResearchNarrative(value2) {
  assertSealedRecord(value2, NARRATIVE_FIELDS, "ResearchNarrative");
  for (const field of NARRATIVE_FIELDS) {
    if (!["nextStep", "stopReason"].includes(field) && !Object.hasOwn(value2, field)) throw new Error(`ResearchNarrative requires $.${field}.`);
  }
  const nextStep = value2.nextStep == null ? null : normalizeResearchNarrativeText(value2.nextStep, "ResearchNarrative.nextStep");
  const stopReason = value2.stopReason == null ? null : normalizeResearchNarrativeText(value2.stopReason, "ResearchNarrative.stopReason");
  if (nextStep === null === (stopReason === null)) throw new Error("ResearchNarrative requires exactly one of nextStep or stopReason.");
  return Object.freeze({
    researchDirection: normalizeResearchNarrativeText(value2.researchDirection, "ResearchNarrative.researchDirection"),
    currentUnderstanding: normalizeTexts(value2.currentUnderstanding, "ResearchNarrative.currentUnderstanding"),
    evidence: normalizeTexts(value2.evidence, "ResearchNarrative.evidence"),
    unknowns: normalizeTexts(value2.unknowns, "ResearchNarrative.unknowns"),
    currentValueJudgment: normalizeResearchNarrativeText(value2.currentValueJudgment, "ResearchNarrative.currentValueJudgment"),
    nextStep,
    stopReason,
    rejectedDirections: normalizeObjects(value2.rejectedDirections, REJECTED_DIRECTION_FIELDS, "ResearchNarrative.rejectedDirections", (item, label) => ({
    ...
    applicableLessons: normalizeObjects(value2.applicableLessons, APPLICABLE_LESSON_FIELDS, "ResearchNarrative.applicableLessons", (item, label) => ({
...
function buildResearchNarrativeFromDecision(value2, options = {}) {
  assertSealedRecord(options, BUILD_FROM_DECISION_OPTION_FIELDS, "ResearchNarrative decision options");
  const decision = validatePersistedResearchDecision(value2, { label: "Research narrative decision" });
...
function renderResearchNarrative(value2, options = {}) {
  ...
  const narrative = normalizeResearchNarrative(value2);
...
function assertAllowedFields(value2, allowed2, label) {
  assertPlainObject6(value2, label);
  const unknown = Object.keys(value2).filter((field) => !allowed2.has(field));
  if (unknown.length > 0) {
...
function safeId10(value2, label) {
  const id3 = nonEmptyString2(value2, label);
...
function hostOutcomeStrings(value2, label, normalize = nonEmptyString2) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const items = value2.map((item, index) => normalize(item, `${label}[${index}]`));
...
function hostOutcomePaths(value2, label) {
  return hostOutcomeStrings(value2, label);
}
function hostOutcomeFacts(value2, mission) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error("facts must be an array.");
  const criteria = missionCompletionCriteria(mission);
  const normalized3 = value2.map((item, index) => {
...
function closeHostOutcome(root, args2 = {}) {
  assertGovernanceMutationRegistered("close-host-outcome", "guarded");
  assertAllowedFields(args2, HOST_OUTCOME_FIELDS, "close_host_outcome");
  const mutationContext = currentMutationContext(root);
  if (!mutationContext) throw new Error("close_host_outcome requires an active MutationContext.");
  const missionId2 = safeId10(args2.missionId, "missionId");
  const status = nonEmptyString2(args2.status, "status");
  if (!HOST_OUTCOME_STATUS_SET.has(status)) throw new Error(`status must be one of: ${ORDINARY_HOST_OUTCOME_STATUSES.join(", ")}.`);
  const summary = assertExecutionFactText(args2.summary, "summary");
  const artifactPaths = hostOutcomePaths(args2.artifactPaths, "artifactPaths");
  const validationPaths = hostOutcomePaths(args2.validationPaths, "validationPaths");
  ...
  const factDeclarations = hostOutcomeFacts(args2.facts, mission);
  ...
  const attemptId = safeId10(args2.attemptId, "attemptId");
...
${attemptId}`).digest("hex").slice(0, 24)}`;
  ...
  if (existing) {
    ...
    const replayArtifacts = existing.artifacts.map(({ path: artifactPath, sha256: sha25616 }) => ({ path: artifactPath, sha256: sha25616 }));
  ...
  const mode2 = eligibleArtifacts.length > 0 ? "artifact-backed" : "observation-only";
  ...
  const receiptValidations = mode2 === "artifact-backed" ? validations.map((validation) => createValidationRecord({
    kind: "validation-log",
    result: "incomplete",
    level: "static",
  ...
  const baseReceipt = {
    ...
    ordinaryHostOutcome: { attemptId, mode: mode2, status, facts, callbackDigest: callbackDigest2 }
  };
...
function plainObject2(value2) {
  return Boolean(value2) && typeof value2 === "object" && !Array.isArray(value2);
}
function normalizedString(value2) {
...
function configPaths(root, env) {
  ...
  paths2.push(path21.join(xdg, "dove", "config.json"));
  if (root) {
    paths2.push(path21.resolve(root, ".dove", "config.json"));
    paths2.push(path21.resolve(root, ".dove", "config.local.json"));
  }
  return paths2;
}
function readConfig(filePath2) {
  ...
  try {
    const value2 = JSON.parse(fs15.readFileSync(filePath2, "utf8"));
    if (!plainObject2(value2)) throw new Error("top level must be an object");
    return value2;
  } catch (error) {
...
function merge(left, right) {
  ...
  for (const [key, value2] of Object.entries(right ?? {})) {
    output[key] = plainObject2(output[key]) && plainObject2(value2) ? merge(output[key], value2) : structuredClone(value2);
  }
  return output;
}
function loadDoveConfig(root, env = process.env) {
  ...
  for (const filePath2 of configPaths(root, env)) {
    const value2 = readConfig(filePath2);
    if (value2) source = merge(source, value2);
  }
...
function loadExplicitDoveLanguageConfig(root, env = process.env) {
  ...
  for (const filePath2 of configPaths(root, env)) {
    const value2 = readConfig(filePath2);
    if (value2 && value2.language !== void 0) language = value2.language;
  }
...
function explicitLanguage(...sources) {
  for (const source of sources) {
    ...
    const value2 = source.language;
    if (typeof value2 === "string" && value2.trim()) return value2;
  }
  return null;
}
function resolveDoveResponseLanguage(root, args2 = {}, options = {}) {
  const requested = explicitLanguage(args2, args2.settings);
...
function decodeBinding(value2) {
  const binding = domainNonEmptyText(value2, "binding");
  let payload;
  try {
...
function currentLessons(root, operation) {
  ...
  const currentHash = sha25611(markdown);
  return { workspace, markdown, currentHash };
}
function readDoveLessons(root, args2 = {}) {
  assertSealedDomainArgs(args2, READ_FIELDS2, "read_dove_lessons");
...
function updateDoveLessons(root, args2 = {}) {
  assertSealedDomainArgs(args2, UPDATE_FIELDS, "update_dove_lessons");
  const context = currentMutationContext(root);
  if (!context) throw new Error("Lessons update requires an active MutationContext.");
  const { binding, payload } = decodeBinding(args2.binding);
  const markdown = typeof args2.markdown === "string" ? args2.markdown : "";
...
function assertPlainRecord2(value2, label) {
  if (value2 === null || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
  const prototype = Object.getPrototypeOf(value2);
  if (prototype !== Object.prototype && prototype !== null) throw new Error(`${label} must be a plain object.`);
  for (const key of Reflect.ownKeys(value2)) {
...
function assertSealedRecord2(value2, allowed2, label) {
  assertPlainRecord2(value2, label);
  const unknown = Object.keys(value2).filter((field) => !allowed2.has(field));
  if (unknown.length > 0) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
}
function assertJsonArray2(value2, label) {
  if (!Array.isArray(value2)) throw new Error(`${label} must be an array.`);
  const unexpected = Reflect.ownKeys(value2).filter((key) => typeof key !== "string" || key !== "length" && !/^(?:0|[1-9][0-9]*)$/u.test(key));
  if (unexpected.length > 0) throw new Error(`${label} must not contain custom fields.`);
}
function assertBoundedLength(value2, limit, label) {
  if (value2.length > limit) throw new Error(`${label} must contain at most ${limit} items.`);
}
function normalizeTexts2(value2, label, limit) {
  assertJsonArray2(value2, label);
  assertBoundedLength(value2, limit, label);
  const items = value2.map((item, index) => normalizeResearchNarrativeText(item, `${label}[${index}]`));
  if (new Set(items).size !== items.length) throw new Error(`${label} must not contain duplicates.`);
  return Object.freeze(items);
}
function normalizeObjects2(value2, fields, label, limit, normalizeItem) {
  assertJsonArray2(value2, label);
  assertBoundedLength(value2, limit, label);
  return Object.freeze(value2.map((item, index) => {
    const itemLabel = `${label}[${index}]`;
...
function normalizeProjectResearchNarrative(value2) {
  assertSealedRecord2(value2, PROJECT_NARRATIVE_FIELDS, "ProjectResearchNarrative");
  for (const field of PROJECT_NARRATIVE_FIELDS) {
    if (!Object.hasOwn(value2, field)) throw new Error(`ProjectResearchNarrative requires $.${field}.`);
  }
  return Object.freeze({
    mainline: normalizeResearchNarrativeText(value2.mainline, "ProjectResearchNarrative.mainline"),
    activeDirections: normalizeObjects2(value2.activeDirections, ACTIVE_DIRECTION_FIELDS, "ProjectResearchNarrative.activeDirections", PROJECT_NARRATIVE_LIMITS.activeDirections, (item, label) => ({
      direction: normalizeResearchNarrativeText(item.direction, `${label}.direction`),
      currentUnderstanding: normalizeResearchNarrativeText(item.currentUnderstanding, `${label}.currentUnderstanding`)
    })),
    evidence: normalizeTexts2(value2.evidence, "ProjectResearchNarrative.evidence", PROJECT_NARRATIVE_LIMITS.evidence),
    unknowns: normalizeTexts2(value2.unknowns, "ProjectResearchNarrative.unknowns", PROJECT_NARRATIVE_LIMITS.unknowns),
    rejectedDirections: normalizeObjects2(value2.rejectedDirections, REJECTED_DIRECTION_FIELDS2, "ProjectResearchNarrative.rejectedDirections", PROJECT_NARRATIVE_LIMITS.rejectedDirections, (item, label) => ({
      direction: normalizeResearchNarrativeText(item.direction, `${label}.direction`),
      reason: normalizeResearchNarrativeText(item.reason, `${label}.reason`)
    })),
    historicalWorkWithoutJudgment: normalizeTexts2(value2.historicalWorkWithoutJudgment, "ProjectResearchNarrative.historicalWorkWithoutJudgment", PROJECT_NARRATIVE_LIMITS.historicalWorkWithoutJudgment),
    currentValueJudgment: normalizeResearchNarrativeText(value2.currentValueJudgment, "ProjectResearchNarrative.currentValueJudgment"),
    recommendation: normalizeResearchNarrativeText(value2.recommendation, "ProjectResearchNarrative.recommendation"),
    recommendationReason: normalizeResearchNarrativeText(value2.recommendationReason, "ProjectResearchNarrative.recommendationReason"),
    applicableLessons: normalizeObjects2(value2.applicableLessons, APPLICABLE_LESSON_FIELDS2, "ProjectResearchNarrative.applicableLessons", PROJECT_NARRATIVE_LIMITS.applicableLessons, (item, label) => ({
      lesson: normalizeResearchNarrativeText(item.lesson, `${label}.lesson`),
      application: normalizeResearchNarrativeText(item.application, `${label}.application`)
    }))
  });
}
function safeText(value2, fallback) {
  return safeResearchNarrativeSourceText(value2) ?? fallback;
}
function projectEvidenceText(value2) {
  const safe = safeResearchNarrativeSourceText(value2);
...
function sentence(value2) {
  return /[.!?。！？]$/u.test(value2) ? value2 : `${value2}\u3002`;
}
...
function renderProjectResearchNarrative(value2, options = {}) {
  ...
  const narrative = normalizeProjectResearchNarrative(value2);
...
function sealed9(value2, fields, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length > 0) throw new Error(`${label} does not accept unknown fields: ${unknown.map((field) => `$.${field}`).join(", ")}.`);
  return value2;
}
function exactIso9(value2, label) {
  if (typeof value2 !== "string" || !Number.isFinite(Date.parse(value2)) || new Date(Date.parse(value2)).toISOString() !== value2) {
    throw new Error(`${label} must be an exact ISO-8601 timestamp.`);
  }
  return value2;
}
function optionalCanonicalText(value2, label) {
  if (value2 === void 0) return void 0;
  if (typeof value2 !== "string" || !value2.trim() || value2 !== value2.trim()) throw new Error(`${label} must be a canonical non-empty string when supplied.`);
  return value2;
}
...
function reviewPath(reviewId) {
  return path22.posix.join(ARTIFACT_PATHS.reviewsDir, `${reviewId}.json`);
}
function reportPath(reviewId) {
  return path22.posix.join(ARTIFACT_PATHS.reviewsDir, `${reviewId}.report.md`);
}
function canonicalReviewRecordPath(value2, label) {
  const candidate = domainNonEmptyText(value2, label);
  const fileName = path22.posix.basename(candidate);
  if (!/^review-[a-f0-9]{24}\.json$/u.test(fileName) || candidate !== path22.posix.join(ARTIFACT_PATHS.reviewsDir, fileName)) {
...
function receiptPath(receiptId) {
  return path22.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`);
}
...
function normalizeScopeBinding(value2) {
  sealed9(value2, SCOPE_BINDING_FIELDS, "scopeBinding");
  if (value2.schemaVersion !== REVIEW_RECORD_SCHEMA_VERSION) throw new Error(`scopeBinding.schemaVersion must be ${REVIEW_RECORD_SCHEMA_VERSION}.`);
  const missionId2 = domainSafeId(value2.missionId, "scopeBinding.missionId");
  if (!HASH11.test(String(value2.contractDigest ?? ""))) throw new Error("scopeBinding.contractDigest must be a lowercase SHA-256 hash.");
  const host = requireNativeReviewerHost(value2.hostKind);
  if (typeof value2.reviewMissionBinding !== "string") throw new Error("scopeBinding.reviewMissionBinding must be the original opaque Review Mission binding.");
  const snapshots = normalizeReviewSnapshots(value2.reviewedArtifacts, "scopeBinding.reviewedArtifacts");
  if (!snapshots.ok) throw new Error(snapshots.reason);
  const setHash = stableSnapshotSetHash(snapshots.snapshots);
  if (value2.reviewedArtifactSetSha256 !== setHash) throw new Error("scopeBinding reviewed artifact set hash is invalid.");
  return {
    schemaVersion: REVIEW_RECORD_SCHEMA_VERSION,
    missionId: missionId2,
    contractDigest: value2.contractDigest,
    reviewMissionBinding: value2.reviewMissionBinding,
    hostKind: host.id,
    reviewedArtifacts: snapshots.snapshots,
    reviewedArtifactSetSha256: setHash
  };
}
function prepareScope(root, args2, operation) {
  const { workspace, mission } = readCurrentMission(root, args2.missionId, operation);
  assertMissionAcceptsWrites(workspace, mission);
  const host = requireNativeReviewerHost(args2.hostKind);
  const reviewMissionBindingValue = assertReviewMissionBinding(workspace, mission, args2.reviewMissionBinding);
  const artifactPaths = domainStringArray(args2.artifactPaths, "artifactPaths", { minItems: 1 });
...
function scopeReviewRecord(root, args2 = {}) {
  assertSealedDomainArgs(args2, SCOPE_FIELDS, "review scope");
  const prepared = prepareScope(root, args2, "Review scope");
...
function normalizeProvenance(value2, binding) {
  sealed9(value2, PROVENANCE_FIELDS, "provenance");
  const host = requireNativeReviewerHost(value2.hostKind);
  if (host.id !== binding.hostKind) throw new Error("provenance.hostKind does not match the original review scope binding.");
  return {
    hostKind: host.id,
    reviewedAt: exactIso9(value2.reviewedAt, "provenance.reviewedAt"),
    ...value2.provider === void 0 ? {} : { provider: optionalCanonicalText(value2.provider, "provenance.provider") },
    ...value2.model === void 0 ? {} : { model: optionalCanonicalText(value2.model, "provenance.model") }
  };
}
function normalizeArchiveInput(root, args2) {
  const { workspace, mission } = readCurrentMission(root, args2.missionId, "Review archive");
  const binding = normalizeScopeBinding(args2.scopeBinding);
  ...
  const status = domainNonEmptyText(args2.status, "status").toLowerCase();
  const verdict = domainNonEmptyText(args2.verdict, "verdict").toLowerCase();
  ...
  const findings = normalizeFindings(args2.findings, binding.reviewedArtifacts.map((item) => item.path));
  const actionItems = domainStringArray(args2.actionItems, "actionItems");
  ...
  const report = domainNonEmptyText(args2.report, "report");
  const provenance = normalizeProvenance(args2.provenance, binding);
  return {
    workspace,
    mission,
    binding,
...
function readJsonFile(root, relativePath, label) {
  try {
    return JSON.parse(fs16.readFileSync(path22.resolve(root, relativePath), "utf8"));
  } catch (error) {
...
function exactReplay(root, input, paths2, digest3) {
  const presence = [paths2.record, paths2.report, paths2.receipt].map((relativePath) => fs16.existsSync(path22.resolve(root, relativePath)));
...
function archiveReviewRecord(root, args2 = {}) {
  assertSealedDomainArgs(args2, ARCHIVE_FIELDS, "review archive");
  const input = normalizeArchiveInput(root, args2);
...
function assessRecord(root, workspace, requestedMissionId, relativePath) {
  ...
  try {
    ...
    const reportFullPath = path22.resolve(root, record.reportPath ?? "");
...
function queryReviewRecords(root, args2 = {}) {
  assertSealedDomainArgs(args2, /* @__PURE__ */ new Set(["missionId"]), "review record query");
  const { workspace, mission } = readCurrentMission(root, args2.missionId, "Review record query");
  const directory = path22.resolve(root, ARTIFACT_PATHS.reviewsDir);
  const assessments = fs16.existsSync(directory) ? fs16.readdirSync(directory, { withFileTypes: true }).filter((entry) => entry.isFile() && /^review-[a-f0-9]{24}\.json$/u.test(entry.name)).map((entry) => assessRecord(root, workspace, mission.missionId, path22.posix.join(ARTIFACT_PATHS.reviewsDir, entry.name))).filter((item) => item.record && missionCanReadMission(workspace.missionGraph, mission.missionId, item.record.missionId)) : [];
  return {
    status: "ok",
    zeroWrite: true,
    authority: "not-established",
...
function resolveCurrentReviewFinding(root, args2 = {}) {
  assertSealedDomainArgs(args2, /* @__PURE__ */ new Set(["missionId", "reviewPath", "findingId"]), "review finding resolution");
  const reviewPathValue = canonicalReviewRecordPath(args2.reviewPath, "reviewPath");
  const findingId = domainSafeId(args2.findingId, "findingId");
  const { workspace, mission } = readCurrentMission(root, args2.missionId, "Review finding resolution");
...
function filePath(directory, id3, suffix) {
  return path23.posix.join(directory, `${id3}.${suffix}.json`);
}
...
function upsertClaims(root, args2 = {}) {
  assertSealedDomainArgs(args2, CLAIM_FIELDS3, "upsert_claims");
  const { mission } = readCurrentMission(root, args2.missionId, "Claim workflow");
  if (!Array.isArray(args2.claims) || args2.claims.length === 0) throw new Error("upsert_claims requires at least one claim.");
  const records = [];
  const writes = args2.claims.map((item, index) => {
    ...
    const artifactRefs = domainStringArray(item.artifactRefs, `${label}.artifactRefs`).map((value2) => `artifact:${value2}`);
    const validationRefs = domainStringArray(item.validationRefs, `${label}.validationRefs`).map((value2) => `validation:${value2}`);
    const resolvedEvidence = evidenceRefs(root, mission.missionId, [...sourceIds.map((value2) => `source:${value2}`), ...artifactRefs, ...validationRefs], `${label}.evidenceRefs`);
    ...
    records.push(claim);
    return {
      path: path23.posix.join(".dove/claims", `${claimId}.json`),
      kind: "data",
...
function runExperienceWorkflow(root, args2 = {}) {
  ...
  const protocol = normalizeExperimentProtocol(args2.protocol);
  const protocolDigest = evidenceDigest(protocol);
  const planPath = filePath(".dove/experiments", experimentId, "plan");
  if (!fs17.existsSync(path23.resolve(root, planPath))) {
    if (args2.result !== void 0) throw new Error("Freeze the formal experiment protocol in a prior transaction before recording a result.");
    const plan2 = { schemaVersion: EXPERIMENT_RECORD_SCHEMA_VERSION, experimentId, missionId: mission.missionId, title: typeof args2.title === "string" && args2.title.trim() ? args2.title.trim() : experimentId, protocol, protocolDigest, updatedAt: protocol.frozenAt };
...
`, derivedReferences: [] }] }), plan: plan2, result: null, hostBoundary: { executesExperiment: false, mintsIndependentAuthority: false } };
  ...
  if (args2.result === void 0) throw new Error("A frozen experiment protocol already exists; recording now requires a result.");
  const resultPath = filePath(".dove/experiments", experimentId, "result");
  if (fs17.existsSync(path23.resolve(root, resultPath))) throw new Error("Experiment result is immutable once recorded; use a new experimentId for another run.");
  const normalized3 = normalizeExperimentResult({
    ...args2.result,
    artifactRefs: evidenceRefs(root, mission.missionId, args2.result.artifactRefs.map((value2) => `artifact:${value2}`), "result.artifactRefs"),
    validationRefs: evidenceRefs(root, mission.missionId, args2.result.validationRefs.map((value2) => `validation:${value2}`), "result.validationRefs"),
    failures: args2.result.failures.map((failure, index) => ({ ...failure, evidenceRefs: evidenceRefs(root, mission.missionId, failure.evidenceRefs, `result.failures[${index}].evidenceRefs`) }))
  });
...
function archiveArtifact(root, actionId, label, args2, fields, prepareSpecializedFields = () => ({})) {
  assertSealedDomainArgs(args2, fields, label);
  const { mission } = readCurrentMission(root, args2.missionId, label);
  const [artifact] = resolveMissionArtifactReferences(root, mission.missionId, [args2.artifactPath], `${label}.artifactPath`);
  ...
  const references = resolveMissionArtifactReferences(root, mission.missionId, args2.referencePaths ?? [], `${label}.referencePaths`);
  const qa = domainStringArray(args2.qa, `${label}.qa`);
  const findings = domainStringArray(args2.findings, `${label}.findings`);
  const specializedFields = prepareSpecializedFields(mission);
  const staged = stageConsolidatedDomainMutation(root, {
    actionId,
    missionId: mission.missionId,
    operation: label,
    receiptId: `receipt-${actionId}-${crypto19.randomUUID()}`,
...
function recordDoveDraft(root, args2 = {}) {
  return archiveArtifact(root, "record-dove-draft", "Draft archive", args2, ARCHIVE_FIELDS2);
}
function recordDoveFigure(root, args2 = {}) {
  return archiveArtifact(root, "record-dove-figure", "Figure archive", args2, FIGURE_ARCHIVE_FIELDS, () => ({
    caption: domainNonEmptyText(args2.caption, "Figure archive.caption")
  }));
}
...
function recordDoveRebuttal(root, args2 = {}) {
  return archiveArtifact(root, "record-dove-rebuttal", "Rebuttal archive", args2, REBUTTAL_ARCHIVE_FIELDS, (mission) => ({
    findingRefs: domainStringArray(args2.findingRefs, "Rebuttal archive.findingRefs", { minItems: 1 }).map((reference, index) => currentFindingReference(root, mission.missionId, reference, `Rebuttal archive.findingRefs[${index}]`)),
...
function queryDomainIntegrity(root, missionId2 = null) {
  ...
  const stale = domainArtifacts.filter((item) => !fs17.existsSync(path23.resolve(root, item.path)));
...
function readCurrentMissions(root, options = {}) {
  ...
  const missionsRoot = path24.resolve(root, ARTIFACT_PATHS.missionsDir);
  const missions = fs18.readdirSync(missionsRoot, { withFileTypes: true }).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => {
    const relativePath = path24.posix.join(ARTIFACT_PATHS.missionsDir, entry.name);
    let mission;
    try {
      mission = JSON.parse(fs18.readFileSync(path24.resolve(root, relativePath), "utf8"));
    } catch (error) {
...
function resolveVisibleMissionSelector(root, args2 = {}) {
  const selected = resolveMissionNumber(root, args2.missionNumber, { operation: "Dove visible mission selection" });
  if (Object.hasOwn(args2, "missionGoal")) {
    const missionGoal = typeof args2.missionGoal === "string" ? args2.missionGoal.trim() : "";
...
function workspaceStatusGraph(root, workspace, missions, assessments, selectedMission = null) {
  ...
  const missionItems = displayedMissions.map((mission) => {
    ...
    return {
      ...
      dependencyDisplayIndices: mission.dependsOnMissionIds.map((missionId2) => missionDisplayById.get(missionId2)).filter((value2) => value2 !== void 0),
...
function statusGraphCollection(graph, name) {
  const value2 = graph?.[name];
  return value2 && typeof value2 === "object" && !Array.isArray(value2) ? { ...value2, items: Array.isArray(value2.items) ? value2.items : [] } : { totalCount: 0, truncated: false, items: [] };
}
...
function publicStatusScope(value2, count) {
  if (value2 === "only-mission" || value2 === "explicit") return "single workstream";
...
function statusOutputPaths(items, predicate = () => true) {
  ...
  return [...new Set(items.filter((item) => item && typeof item === "object" && predicate(item)).map((item) => item.path).filter((value2) => typeof value2 === "string" && value2.trim() && !/(?:^|\/)\.dove(?:-archive)?(?:\/|$)/u.test(value2)))];
}
function statusCondition(value2) {
  if (typeof value2 !== "string" || !value2.trim()) return null;
  if (value2.startsWith("artifact:")) {
    const artifactPath = value2.slice("artifact:".length);
  ...
  if (value2.startsWith("validation:")) {
    const validationPath = value2.slice("validation:".length);
  ...
  if (/^(?:source|note):/u.test(value2)) return "eligible current source evidence";
  return value2;
}
function statusWorkstreamItems(graph) {
  return statusGraphCollection(graph, "missions").items.filter((item) => item && typeof item === "object").map((item, index) => {
    ...
    const missingOutputs = [...new Set((Array.isArray(item.uncoveredOutputs) ? item.uncoveredOutputs : []).filter((value2) => typeof value2 === "string" && !/(?:^|\/)\.dove(?:-archive)?(?:\/|$)/u.test(value2)))];
    const uncoveredCriteria = [...new Set((Array.isArray(item.uncoveredCompletionCriteria) ? item.uncoveredCompletionCriteria : []).filter((value2) => typeof value2 === "string"))];
...
function emptyStatus(args2, detail) {
...
function queryDoveStatus(root, args2 = {}) {
  ...
  const unknown = Object.keys(args2).filter((field) => !allowed2.has(field));
  ...
  if (workspace.state === "absent") return emptyStatus(args2, detail);
  const hasMissionNumber = Object.hasOwn(args2, "missionNumber");
  const requestedMissionId = typeof args2.missionId === "string" ? args2.missionId.trim() : "";
  const selectedMission = requestedMissionId ? missions.find((mission) => mission.missionId === requestedMissionId) ?? null : hasMissionNumber ? resolveMissionNumber(root, args2.missionNumber, { operation: "Dove status mission selection" }) : missions.length === 1 ? missions[0] : null;
...
function queryDoveMission(root, args2 = {}) {
  const inspection = openDoveWorkspace(root, { allowAbsent: true, operation: "Dove mission preview" });
  if (inspection.state !== "absent") return previewDoveMissionContract(root, args2);
  if (!args2 || typeof args2 !== "object" || Array.isArray(args2)) throw new Error("Dove mission preview arguments must be a plain object.");
  ...
  const mode2 = typeof args2.mode === "string" ? args2.mode.trim() : "";
  if (!["ordinary", "research"].includes(mode2)) throw new Error("Dove mission preview requires explicit mode: ordinary or research.");
  ...
  const stringList2 = (value2, label) => {
    if (value2 === void 0) return [];
    if (!Array.isArray(value2) || value2.some((item) => typeof item !== "string" || !item.trim())) throw new Error(`${label} must be an array of non-empty strings.`);
    return [...new Set(value2.map((item) => item.trim()))];
  };
  const artifacts = Array.isArray(args2.artifacts) ? args2.artifacts.map((item, index) => {
  ...
  return {
    status: "proposal",
    mission: { mode: mode2, goal, requirements: stringList2(args2.requirements, "requirements"), assumptions: stringList2(args2.assumptions, "assumptions"), scope: stringList2(args2.scope, "scope"), outOfScope: stringList2(args2.outOfScope, "outOfScope"), artifacts, completionCriteria: stringList2(args2.completionCriteria, "completionCriteria"), evidenceRequirements: stringList2(args2.evidenceRequirements, "evidenceRequirements") },
...
function sealed10(value2, fields, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
function exactIso10(value2, label) {
  if (typeof value2 !== "string" || new Date(Date.parse(value2)).toISOString() !== value2) throw new Error(`${label} must be an exact ISO timestamp.`);
  return value2;
}
...
function prepareResearchDecisionReevaluation(root, args2 = {}) {
  ...
  const nextAction = args2.nextAction === null ? null : {
    ...args2.nextAction,
    actionId: researchStableId("research-action", missionId2, decisionRevision, 0, args2.nextAction.description),
    targetHypothesisOrQuestionIds: args2.nextAction.targets.map((target2) => {
      const resolved = targetIds.get(target2);
      if (!resolved) throw new Error(`nextAction.targets contains an unknown public target ${target2}.`);
      return resolved;
    })
  };
  if (nextAction) delete nextAction.targets;
  return {
    operation: args2.operation,
    missionId: missionId2,
    decisionRevision,
    requestedDisposition: args2.requestedDisposition,
    synthesis: args2.synthesis,
    hypotheses,
    routes,
    openQuestions,
    evidenceRefs: args2.evidenceRefs,
    consumedReceiptIds,
    reasonCodes: args2.reasonCodes,
    nextAction
  };
}
function reevaluateResearchDecision(root, args2 = {}) {
  assertGovernanceMutationRegistered("reevaluate-research-decision", "guarded");
  sealed10(args2, FIELDS4, "reevaluate-research-decision");
  if (args2.operation !== "reevaluate-research-decision") throw new Error("Research decision reevaluation requires its explicit operation.");
  const context = currentMutationContext(root);
  ...
  const { workspace, mission } = readCurrentMission(root, args2.missionId, "Research decision reevaluation");
  if (mission.mode !== "research") throw new Error("Research decision reevaluation requires a research mission.");
  assertMissionAcceptsWrites(workspace, mission);
  const current = workspace.currentResearchDecisions.get(mission.missionId);
  if (!current || current.revision !== args2.decisionRevision) throw new Error("The submitted research decision revision is stale.");
  const disposition = domainNonEmptyText(args2.requestedDisposition, "requestedDisposition");
  if (!DISPOSITIONS.has(disposition)) throw new Error("requestedDisposition is unsupported.");
  const createdAt = exactIso10(args2.createdAt ?? (/* @__PURE__ */ new Date()).toISOString(), "createdAt");
  if (Date.parse(createdAt) < Date.parse(current.createdAt)) throw new Error("createdAt must not precede the current decision.");
  const evidenceRefs2 = resolveEvidence(root, mission.missionId, args2.evidenceRefs);
  const consumedReceiptIds = domainStringArray(args2.consumedReceiptIds, "consumedReceiptIds").map((id3, index) => domainSafeId(id3, `consumedReceiptIds[${index}]`));
  ...
  const nextAction = args2.nextAction === null ? null : createResearchDecisionAction(args2.nextAction);
  const content = { synthesis: domainNonEmptyText(args2.synthesis, "synthesis"), hypotheses: args2.hypotheses, routes: args2.routes, openQuestions: args2.openQuestions, evidenceRefs: evidenceRefs2, consumedReceiptIds, disposition, reasonCodes: domainStringArray(args2.reasonCodes, "reasonCodes"), nextAction };
...
function resolvedPolicy(value2, context) {
  return typeof value2 === "function" ? value2(context) : value2;
}
function assertPolicy(value2, allowed2, label, operationId) {
  if (!allowed2.has(value2)) throw new Error(`Invalid ${label} for Dove operation ${operationId}: ${value2}`);
  return value2;
}
...
var toolOperations = [
  freezeOperation({
    ...
    interaction: (args2) => args2?.operation === "set-mainline" ? "write" : "checkpoint",
    checkpoint: (args2) => args2?.operation !== "set-mainline"
  }),
  freezeOperation({
    ...
    interaction: (args2) => args2?.operation === "query" ? "read" : ["start-skill", "reevaluate-research-decision"].includes(args2?.operation) ? "write" : "checkpoint",
    checkpoint: (args2) => ["create-root", "branch"].includes(args2?.operation),
  ...
  freezeOperation({
    ...
    interaction: (args2) => args2?.operation === "query" ? "read" : "write",
    routes: {
  ...
  freezeOperation({
    ...
    interaction: (args2) => args2?.operation === "scope" ? "read" : "write",
    checkpoint: false,
    closure: DOMAIN_CLOSURE,
    routes: {
  ...
  freezeOperation({
    ...
    interaction: (args2) => args2?.operation === "read" ? "read" : "write",
    routes: {
...
function operationRoute(operation, args2 = {}) {
  ...
  const operationName = typeof args2?.operation === "string" ? args2.operation : Object.keys(routes).length === 1 ? Object.keys(routes)[0] : null;
  return operationName ? routes[operationName] ?? null : null;
}
function operationTargetTool(operation, args2 = {}) {
  const selected = typeof operation === "string" ? operationById(operation) : operation;
  const route = operationRoute(selected, args2);
...
function operationInteraction(operation, args2 = {}) {
  const selected = typeof operation === "string" ? operationById(operation) : operation;
  const route = operationRoute(selected, args2);
  return assertPolicy(route?.interaction ?? resolvedPolicy(selected.interaction, args2), INTERACTIONS, "interaction", selected.id);
}
function operationRequiresCheckpoint(operation, args2 = {}) {
  const selected = typeof operation === "string" ? operationById(operation) : operation;
  const route = operationRoute(selected, args2);
  const checkpoint = route?.checkpoint ?? resolvedPolicy(selected.checkpoint, args2);
...
function operationPublicProjector(operation, args2 = {}) {
  const selected = typeof operation === "string" ? operationById(operation) : operation;
  const projector = operationRoute(selected, args2)?.publicProjector ?? selected.publicProjector;
  if (typeof projector !== "string" || !projector) throw new Error(`Dove operation ${selected.id} requires a public projector.`);
  return projector;
}
function commandOperationMetadata(operationId) {
...
function normalizeStatus(value2) {
  return typeof value2 === "string" ? value2.trim().toLowerCase() : "";
}
function assertEnum(value2, allowed2, label) {
  if (!allowed2.has(value2)) throw new Error(`Unsupported invocation ${label}: ${value2}`);
  return value2;
}
...
var PACKAGE_DOCUMENTATION_PATHS = [
  "README.md",
  "docs/README.md",
  "docs/INSTALL.md",
  "docs/USAGE.md",
...
var PROJECT_HOST_IDS2 = ["opencode", "codex", "cursor", "agents", "claude"];
...
var HOST_ADAPTER_LANGUAGE_LABEL = DEFAULT_DOVE_RESPONSE_LANGUAGE === "zh" ? "Chinese" : DEFAULT_DOVE_RESPONSE_LANGUAGE;
...
var ADAPTER_NOTES = Object.freeze({
...
var CALL_FLOW_CHANNEL_POLICY = Object.freeze({
...
var selector = (mission = "none", research = "none") => Object.freeze({ mission, research });
var step = (tool, required, instruction) => Object.freeze({ tool, required: Object.freeze(required), instruction });
var readSkillContext = () => step("query_dove_status", ["operation"], "Use operation=status to read the public workspace and lifecycle context. Select an existing work number only when the requested record or artifact identifies it exactly; otherwise ask one zero-write clarification and stop.");
var startSkill = (skill, boundary) => step("manage_dove_mission", ["operation", "skill", "goal"], `Use operation=start-skill and skill=${skill}. Refine one minimal goal from the request and current context. Command text is optional constraints; clarify only material ambiguity and do not impose a fixed template. Start one research Skill Mission before ${boundary}; an artifact-owning mission may be selected as its parent without stopping that parent. Preserve result.selector.missionNumber for every later mission-bound tool and use the supplied research outcome closure exactly once after host work.`);
var resumeSkill = (boundary) => step("query_dove_status", ["operation"], `Use operation=status and select the exact existing work number identified by the requested lifecycle record or artifact before ${boundary}. Do not call start-skill, do not guess the latest work, and ask one zero-write clarification if the owner is not unambiguous.`);
var mode = (id3, when, steps, { clarification = [] } = {}) => Object.freeze({ id: id3, when, steps: Object.freeze(steps), clarification: Object.freeze(clarification) });
var flow = ({ status, selectors = selector(), generatedFields = [], modes, examples }) => Object.freeze({
  status,
  selectors,
...
var COMMAND_CALL_FLOWS = Object.freeze({
  "dove.workspace": flow({
...
var surface = (id3, title, category, policy, summary, operationId, constraints, ux) => ({
  id: id3,
  title,
  domain: "generic",
  category,
  ...
  adapterNotes: [...ADAPTER_NOTES[id3] ?? []],
  callFlow: COMMAND_CALL_FLOWS[id3],
  ux
});
var commandSurfaces = [
...
var COMMAND_SURFACES = commandSurfaces.map((command3) => {
  const metadata = commandOperationMetadata(command3.operationId);
  return { ...command3, ...metadata };
});
...
function adapterPathForCommand(hostId, command3) {
  ...
  switch (hostId) {
    ...
    case "claude":
      return `.claude/commands/dove/${hostSlug}.md`;
    default:
...
function stringList(value2, field) {
  if (value2 === void 0) return [];
  if (!Array.isArray(value2)) throw new Error(`${DOVE_CLAUDE_LOCAL_SETTINGS_PATH} ${field} must be an array.`);
  if (value2.some((item) => typeof item !== "string" || !item || item !== item.trim())) {
    throw new Error(`${DOVE_CLAUDE_LOCAL_SETTINGS_PATH} ${field} must contain non-empty trimmed strings.`);
  }
  if (new Set(value2).size !== value2.length) {
...
function inspectDirectoryTreeDigest(root, relativePath, options = {}) {
  const fsOps = options.fsOps ?? fs19;
  const canonicalRoot = typeof fsOps.realpathSync.native === "function" ? fsOps.realpathSync.native(path25.resolve(root)) : fsOps.realpathSync(path25.resolve(root));
  const normalized3 = path25.posix.normalize(String(relativePath).replace(/\\/gu, "/"));
  if (normalized3 === "." || normalized3 === ".." || normalized3.startsWith("../") || path25.posix.isAbsolute(normalized3)) {
    throw new Error(`Directory tree digest path must stay inside its root: ${relativePath}.`);
  }
  const directory = path25.join(canonicalRoot, normalized3);
  const rootStat = fsOps.lstatSync(directory, { bigint: true });
  if (rootStat.isSymbolicLink() || !rootStat.isDirectory()) throw new Error(`Directory tree digest source must be a real directory: ${normalized3}.`);
  ...
  const visit = (absoluteDirectory, prefix) => {
    ...
    for (const name of names) {
      const childPath2 = prefix ? path25.posix.join(prefix, name) : name;
      const absoluteChild = path25.join(absoluteDirectory, name);
      const stat = fsOps.lstatSync(absoluteChild, { bigint: true });
      const metadata = { path: childPath2, ...treeMetadata(stat) };
      if (stat.isDirectory() && !stat.isSymbolicLink()) {
        entries.push({ ...metadata, kind: "directory" });
        visit(absoluteChild, childPath2);
      } else if (stat.isFile()) {
        entries.push({ ...metadata, kind: "file", sizeBytes: String(stat.size), sha256: sha25612(fsOps.readFileSync(absoluteChild)) });
      } else if (stat.isSymbolicLink()) {
        entries.push({ ...metadata, kind: "symlink", target: fsOps.readlinkSync(absoluteChild) });
      } else {
        throw new Error(`Unsupported filesystem entry inside ${normalized3}: ${childPath2}.`);
      }
    }
  };
  visit(directory, "");
  return sha25612(canonicalJson(entries));
}
function state(anchor, relativePath) {
  ...
  return { exists: true, type: "file", sha256: sha25612(anchor.readFile(relativePath)), mode: stat.mode & 4095 };
}
...
function parentDirectories(relativePath) {
  const directories = [];
  let current = path25.posix.dirname(relativePath);
  while (current !== ".") {
    directories.push(current);
    current = path25.posix.dirname(current);
  }
...
function writeFileSetTransaction(entries, options = {}) {
  ...
  const fsOps = options.fsOps ?? fs19;
  const transactionId = (options.transactionId ?? crypto21.randomUUID()).replace(/[^a-z0-9._-]/giu, "-");
  ...
  try {
    ...
    for (const entry of resolved) {
      ...
      if (entry.deleting && entry.previous.type === "directory") {
        ...
        const scheduledChildren = new Set(resolved.filter((candidate) => candidate.anchor === entry.anchor && candidate.deleting && path25.posix.dirname(candidate.relativePath) === entry.relativePath).map((candidate) => path25.posix.basename(candidate.relativePath)));
...
function duplicateKeyError(label, key, path35) {
  throw new Error(`${label} must not contain duplicate JSON object keys: ${path35 === "$" ? key : `${path35}.${key}`}.`);
}
function parseJsonWithoutDuplicateKeys(text11, label = "JSON input") {
  if (typeof text11 !== "string") throw new Error(`${label} must contain valid JSON.`);
  let index = 0;
  function skipWhitespace() {
    while (/\s/u.test(text11[index] ?? "")) index += 1;
  }
  function parseString() {
    if (text11[index] !== '"') throw new Error(`${label} must contain valid JSON.`);
    const start = index;
    index += 1;
    let escaped = false;
    while (index < text11.length) {
      ...
      if (character === '"') {
        return JSON.parse(text11.slice(start, index));
      }
  ...
  function parseNumber() {
    const match = text11.slice(index).match(/^-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?/u);
    if (!match) throw new Error(`${label} must contain valid JSON.`);
    index += match[0].length;
  }
  function parseArray(path35) {
    index += 1;
    skipWhitespace();
    if (text11[index] === "]") {
      index += 1;
  ...
  function parseObject(path35) {
    ...
    while (true) {
      ...
      if (keys.has(key)) duplicateKeyError(label, key, path35);
      keys.add(key);
      skipWhitespace();
      if (text11[index] !== ":") throw new Error(`${label} must contain valid JSON.`);
      index += 1;
      parseValue(path35 === "$" ? `$.${key}` : `${path35}.${key}`);
      skipWhitespace();
      if (text11[index] === "}") {
        index += 1;
        return;
  ...
  function parseValue(path35) {
    skipWhitespace();
    const character = text11[index];
    if (character === "{") parseObject(path35);
    else if (character === "[") parseArray(path35);
    else if (character === '"') parseString();
    else if (character === "-" || /\d/u.test(character ?? "")) parseNumber();
    else if (text11.startsWith("true", index)) index += 4;
    else if (text11.startsWith("false", index)) index += 5;
    else if (text11.startsWith("null", index)) index += 4;
  ...
  try {
    parseValue("$");
    skipWhitespace();
    if (index !== text11.length) throw new Error(`${label} must contain valid JSON.`);
    return JSON.parse(text11);
  } catch (error) {
...
function readSmallRegularFile(root, relativePath, fsOps, maxBytes = 2 * 1024 * 1024) {
  const absolutePath = path26.join(root, relativePath);
...
function legacyMarkerHits(root, fsOps) {
  return LEGACY_PROJECT_MARKER_PATHS.filter((relativePath) => {
    ...
    try {
      const value2 = parseJsonWithoutDuplicateKeys(content, `Legacy Dove marker ${relativePath}`);
      return value2 !== null && typeof value2 === "object" && !Array.isArray(value2) && value2.host === "claude";
    } catch {
      return false;
    }
  });
...
function registrationHits(root, fsOps) {
  ...
  if (mcp !== null) {
    try {
      const value2 = parseJsonWithoutDuplicateKeys(mcp, "Legacy Dove MCP configuration");
      const dove = value2?.mcpServers?.dove;
      const args2 = Array.isArray(dove?.args) ? dove.args : [];
      if (dove?.command === "node" && args2.some((argument) => typeof argument === "string" && DOVE_MCP_BUNDLE_PATH.test(argument))) {
  ...
  if (settings !== null) {
    try {
      const value2 = parseJsonWithoutDuplicateKeys(settings, "Legacy Dove Claude settings");
      if (DOVE_HOOK_COMMAND.test(JSON.stringify(value2?.hooks?.UserPromptSubmit ?? null))) hits.push(".claude/settings.json#/hooks/UserPromptSubmit");
    } catch {
    }
  }
  return hits;
...
function inspectLegacyProjectInstallation(root, options = {}) {
  const fsOps = options.fsOps ?? fs20;
...
var INSTALLATION_MANIFEST_PATH = ".dove-install/manifest.json";
...
var INSTALLATION_RUNTIME_PROTOCOL_VERSION = 1;
...
function assertPlainObject7(value2, label) {
  if (!plainObject4(value2)) throw new Error(`${label} must be a plain object.`);
}
function assertSealed5(value2, fields, label) {
...
function nonEmptyString3(value2, label) {
  if (typeof value2 !== "string" || !value2 || value2 !== value2.trim() || value2.includes("\0")) throw new Error(`${label} must be a non-empty trimmed string.`);
  return value2;
}
function exactPositiveInteger(value2, expected, label) {
  if (value2 !== expected) throw new Error(`${label} must equal ${expected}.`);
  return value2;
}
function exactIsoTimestamp(value2, label) {
  nonEmptyString3(value2, label);
  const milliseconds = Date.parse(value2);
  if (!Number.isFinite(milliseconds) || new Date(milliseconds).toISOString() !== value2) throw new Error(`${label} must be an exact ISO timestamp.`);
  return value2;
}
function canonicalProjectRelativePath(value2, label) {
  nonEmptyString3(value2, label);
  if (value2.includes("\\") || path27.posix.isAbsolute(value2) || /^[A-Za-z]:[\\/]/u.test(value2)) {
    throw new Error(`${label} must be a project-relative path without backslashes: ${value2}`);
  }
  const normalized3 = path27.posix.normalize(value2);
  if (normalized3 !== value2 || normalized3 === "." || normalized3 === ".." || normalized3.startsWith("../") || value2.includes("//") || value2.endsWith("/")) {
    throw new Error(`${label} must be one canonical project-relative path: ${value2}`);
  }
  if (value2 === ".dove" || value2.startsWith(".dove/")) throw new Error(`${label} must not manage Dove workspace state: ${value2}`);
  return value2;
}
function normalizeAllowedHosts(options) {
  const allowed2 = options.hostIds;
  if (!Array.isArray(allowed2) || allowed2.length === 0 || allowed2.some((hostId) => typeof hostId !== "string" || !hostId || hostId === "all")) {
    throw new Error("Project installation manifest validation requires concrete hostIds in registry order.");
  }
  if (new Set(allowed2).size !== allowed2.length) throw new Error("Project installation manifest hostIds must be unique.");
  return allowed2;
}
function validateHosts(hosts, allowedHosts) {
  ...
  for (const hostId of hosts) {
    nonEmptyString3(hostId, "Project installation manifest host");
  ...
  const normalized3 = allowedHosts.filter((hostId) => hosts.includes(hostId));
  if (normalized3.length !== hosts.length || normalized3.some((hostId, index) => hostId !== hosts[index])) {
...
function validateManagedEntry(entry, index) {
  ...
  assertSealed5(entry, MANAGED_FIELDS, label);
  canonicalProjectRelativePath(entry.path, `${label}.path`);
  nonEmptyString3(entry.owner, `${label}.owner`);
  ...
  if (entry.mode === "exclusive-file") {
    ...
    nonEmptyString3(entry.selector, `${label}.selector`);
  }
  if (typeof entry.digest !== "string" || !SHA2562.test(entry.digest)) throw new Error(`${label}.digest must be a lowercase 64-character SHA-256 digest.`);
  return entry;
}
...
function normalizeCreatedAt(value2) {
  if (value2 instanceof Date) return value2.toISOString();
  if (typeof value2 === "string") return exactIsoTimestamp(value2, "Project installation manifest timestamp");
...
function validateProjectInstallationManifest(value2, options = {}) {
  ...
  assertSealed5(value2, MANIFEST_FIELDS2, "Project installation manifest");
  exactPositiveInteger(value2.schemaVersion, INSTALLATION_MANIFEST_SCHEMA_VERSION, "Project installation manifest schemaVersion");
  if (!acceptedVersionPairs.some(([integrationVersion, ownershipVersion]) => value2.integrationVersion === integrationVersion && value2.ownershipVersion === ownershipVersion)) {
  ...
  nonEmptyString3(value2.installationId, "Project installation manifest installationId");
  if (!INSTALLATION_ID.test(value2.installationId)) throw new Error("Project installation manifest installationId must use installation-<uuid> format.");
  assertSealed5(value2.package, PACKAGE_FIELDS, "Project installation manifest package");
  nonEmptyString3(value2.package.name, "Project installation manifest package.name");
  nonEmptyString3(value2.package.version, "Project installation manifest package.version");
  if (!SEMVER.test(value2.package.version)) throw new Error("Project installation manifest package.version must be a semantic version.");
  assertSealed5(value2.runtime, RUNTIME_FIELDS, "Project installation manifest runtime");
  if (value2.runtime.mode !== "user-cli") throw new Error("Project installation manifest runtime.mode must be user-cli.");
  exactPositiveInteger(value2.runtime.protocolVersion, INSTALLATION_RUNTIME_PROTOCOL_VERSION, "Project installation manifest runtime.protocolVersion");
  validateHosts(value2.hosts, allowedHosts);
  validateManaged(value2.managed);
  const createdAt = exactIsoTimestamp(value2.createdAt, "Project installation manifest createdAt");
  const updatedAt = exactIsoTimestamp(value2.updatedAt, "Project installation manifest updatedAt");
...
function createProjectInstallationManifest(input = {}, options = {}) {
  ...
  const manifest = {
    ...
    installationId: input.installationId ?? `installation-${crypto22.randomUUID()}`,
    package: {
...
function serializeProjectInstallationManifest(value2, options = {}) {
  validateProjectInstallationManifest(value2, options);
  return `${JSON.stringify(value2, null, 2)}
`;
}
function isPreviousProjectInstallationManifest(value2) {
  return value2?.schemaVersion === INSTALLATION_MANIFEST_SCHEMA_VERSION && value2?.integrationVersion === PREVIOUS_INSTALLATION_INTEGRATION_VERSION && value2?.ownershipVersion === PREVIOUS_INSTALLATION_OWNERSHIP_VERSION;
}
function inspectManifestFile(root, fsOps) {
  const installationDirectory = path27.join(root, path27.posix.dirname(INSTALLATION_MANIFEST_PATH));
  ...
  const manifestPath = path27.join(root, INSTALLATION_MANIFEST_PATH);
  let stat;
  try {
...
function readProjectInstallationManifest(root, options = {}) {
  const fsOps = options.fsOps ?? fs21;
  const manifestPath = inspectManifestFile(root, fsOps);
  let parsed2;
  try {
    parsed2 = parseJsonWithoutDuplicateKeys(fsOps.readFileSync(manifestPath, "utf8"), "Dove project installation manifest");
    validateProjectInstallationManifest(parsed2, options.allowPrevious === true ? {
      ...options,
  ...
  return deepFreezeManifest(structuredClone(parsed2));
}

// src/core/project-root.mjs
import fs22 from "node:fs";
import path28 from "node:path";
var INSTALLATION_DIRECTORY = path28.posix.dirname(INSTALLATION_MANIFEST_PATH);
function realpathNative3(fsOps, targetPath) {
  return typeof fsOps.realpathSync?.native === "function" ? fsOps.realpathSync.native(targetPath) : fsOps.realpathSync(targetPath);
}
function canonicalExistingDirectory(value2, label, fsOps) {
  if (typeof value2 !== "string" || !value2.trim() || value2.includes("\0")) throw new Error(`${label} must name an existing directory.`);
  const resolved = path28.resolve(value2);
  let stat;
  try {
...
function installationStateAt(root, options) {
  const fsOps = options.fsOps ?? fs22;
  const directoryPath = path28.join(root, INSTALLATION_DIRECTORY);
  const manifestPath = path28.join(root, INSTALLATION_MANIFEST_PATH);
...
function gitRootFrom(start, fsOps) {
  for (const directory of parentDirectories2(start)) {
    const dotGit = path28.join(directory, ".git");
...
function resolveProjectRootForInit(project, options = {}) {
  ...
  const directories = parentDirectories2(candidate);
...
function inspectProjectRoot(start, options = {}) {
  ...
  try {
    canonicalStart = canonicalExistingDirectory(start ?? options.cwd ?? process.cwd(), "Dove project search start", options.fsOps ?? fs22);
...
var __dirname = path29.dirname(__filename);
var PACKAGE_ROOT = path29.resolve(__dirname, "..");
...
function dailyUseBullets(command3) {
  const dailyFlow = command3.ux?.dailyFlow;
...
function exampleBullets(command3, hostId = null) {
  ...
  return examples.map((example) => {
    const text11 = String(example).trim();
    return hostId === "opencode" ? text11.replace(/^\/dove:/u, "/dove.") : text11;
  }).filter(Boolean);
}
function toolBullets(command3) {
  const tools = unique2(command3.requiredTools ?? []);
...
function renderWorkflow(command3) {
  const modes = command3.callFlow?.modes;
...
function renderGuidance(command3) {
  const notes = Array.isArray(command3.adapterNotes) ? command3.adapterNotes.filter(Boolean) : [];
...
function renderBody(command3, heading, hostId = null) {
  ...
  const workflow = renderWorkflow(command3);
...
function generatedAdapterEntries() {
  return PROJECT_HOST_IDS2.flatMap((hostId) => COMMAND_SURFACES.map((command3) => ({
    hostId,
    command: command3,
...
function sha25613(content) {
  return crypto23.createHash("sha256").update(content).digest("hex");
}
function canonicalJson2(value2) {
  if (value2 === null || typeof value2 !== "object") return JSON.stringify(value2);
  if (Array.isArray(value2)) return `[${value2.map(canonicalJson2).join(",")}]`;
  return `{${Object.keys(value2).sort().map((key) => `${JSON.stringify(key)}:${canonicalJson2(value2[key])}`).join(",")}}`;
}
function semanticDigest(value2) {
  return sha25613(canonicalJson2(value2));
}
...
function exactTimestamp(value2) {
  if (value2 === void 0) return (/* @__PURE__ */ new Date()).toISOString();
  const timestamp3 = value2 instanceof Date ? value2.toISOString() : value2;
  if (typeof timestamp3 !== "string" || new Date(timestamp3).toISOString() !== timestamp3) {
...
function inspectRegularProjectFile(root, relativePath, fsOps) {
  ...
  for (let index = 0; index < components.length; index += 1) {
    current = path30.join(current, components[index]);
    ...
    return { exists: true, bytes, digest: sha25613(bytes), mode: stat.mode & 4095 };
  }
...
function referencesDoveHook(entry) {
  ...
  return entry.hooks.some((hook) => plainObject5(hook) && typeof hook.command === "string" && (hook.command.includes("dove hook user-prompt-submit") || hook.command.includes("dove-user-prompt-submit-package.mjs")));
}
...
var OVERLAY_SHARED_JSON_PATHS = Object.freeze([MCP_PATH, DOVE_CLAUDE_SETTINGS_PATH, DOVE_CLAUDE_LOCAL_SETTINGS_PATH, ".opencode.json"]);
var LEGACY_MARKER_PATH = LEGACY_PROJECT_MARKER_PATHS[0];
var LEGACY_RUNTIME_SIGNATURES = Object.freeze(Object.fromEntries(
  LEGACY_PROJECT_BUNDLE_PROBES.map((probe) => [probe.path, probe.signatures])
));
function resultFromTransaction(status, target2, hosts, manifest, transaction) {
  return {
    status,
    target: target2,
    hosts: [...hosts],
...
function initializeProjectIntegration(rootOrProject, options = {}) {
  const fsOps = options.fsOps ?? fs24;
...
// src/core/project-doctor.mjs
var MODULE_DIRECTORY = path31.dirname(fileURLToPath2(import.meta.url));
var DEFAULT_PACKAGE_ROOT = path31.resolve(MODULE_DIRECTORY, "../..");
...
function sha25614(content) {
  return crypto24.createHash("sha256").update(content).digest("hex");
}
function lstatOrNull4(targetPath) {
  try {
    return fs25.lstatSync(targetPath);
  } catch (error) {
    if (error?.code === "ENOENT") return null;
    throw error;
  }
...
function regularNonSymlink(targetPath) {
  try {
    const stat = fs25.lstatSync(targetPath);
...
function containedPackagePath(packageRoot, relativePath) {
  const absolutePath = path31.resolve(packageRoot, relativePath);
  const relative = path31.relative(packageRoot, absolutePath);
...
function normalizeExecutableInspection(value2) {
  if (typeof value2 === "string") return { found: true, path: value2, usable: true, state: "found", message: null };
  if (!plainObject6(value2)) return { found: false, path: null, usable: false, state: "invalid-result", message: "PATH executable inspection returned an invalid result." };
  const executablePath = typeof value2.path === "string" && value2.path ? value2.path : null;
  const found = value2.found === true || executablePath !== null;
  return {
    found,
    path: executablePath,
    usable: value2.usable === void 0 ? found : value2.usable === true,
    state: typeof value2.state === "string" && value2.state ? value2.state : found ? "found" : "unavailable",
    message: typeof value2.message === "string" ? value2.message : null
  };
}
function inspectUserCli(options) {
  const packageRoot = path31.resolve(options.packageRoot ?? DEFAULT_PACKAGE_ROOT);
  ...
  const executablePath = path31.resolve(options.executablePath ?? path31.join(packageRoot, "bin/dove-package.mjs"));
  const executableRelative = path31.relative(packageRoot, executablePath);
  const executableContained = executableRelative === "" || !executableRelative.startsWith("..") && !path31.isAbsolute(executableRelative);
  const executable = {
    path: executablePath,
  ...
  const staticRuntime = plainObject6(registry) ? { healthy: registry.healthy === true, state: registry.state ?? (registry.healthy === true ? "loadable" : "invalid"), message: registry.message ?? null } : { healthy: false, state: "invalid-result", message: "Static runtime inspection returned an invalid result." };
...
function decodePointerToken(value2) {
  return value2.replaceAll("~1", "/").replaceAll("~0", "~");
}
function referencesDovePromptHook(value2) {
  return plainObject6(value2) && Array.isArray(value2.hooks) && value2.hooks.some((hook) => plainObject6(hook) && typeof hook.command === "string" && (hook.command.includes("dove hook user-prompt-submit") || hook.command.includes("dove-user-prompt-submit-package.mjs")));
}
function extractJsonPointer(value2, selector2) {
  if (selector2 === "") return value2;
  if (selector2 === "/hooks/UserPromptSubmit[dove-user-prompt-submit]") {
    const entries = value2?.hooks?.UserPromptSubmit;
    if (!Array.isArray(entries)) throw new Error(`JSON fragment selector does not exist: ${selector2}.`);
    const matches = entries.filter(referencesDovePromptHook);
    if (matches.length !== 1) throw new Error(`JSON fragment selector must resolve exactly once: ${selector2}.`);
    return matches[0];
  }
  if (selector2 === DOVE_CLAUDE_MCP_APPROVAL_SELECTOR) {
    const approval = inspectClaudeMcpApprovalSettings(value2);
    if (!approval.approved) throw new Error(`JSON fragment selector does not exist: ${selector2}.`);
    return DOVE_MCP_SERVER_NAME;
  }
  if (typeof selector2 !== "string" || !selector2.startsWith("/")) throw new Error(`Unsupported JSON fragment selector: ${String(selector2)}.`);
  let current = value2;
  for (const rawToken of selector2.slice(1).split("/")) {
    const token = decodePointerToken(rawToken);
    if (Array.isArray(current)) {
      if (!/^(?:0|[1-9]\d*)$/u.test(token) || Number(token) >= current.length) throw new Error(`JSON fragment selector does not exist: ${selector2}.`);
      current = current[Number(token)];
    } else if (plainObject6(current) && Object.hasOwn(current, token)) {
      current = current[token];
    } else {
      throw new Error(`JSON fragment selector does not exist: ${selector2}.`);
    }
  }
  return current;
}
function stableValue8(value2) {
  if (Array.isArray(value2)) return value2.map(stableValue8);
  if (plainObject6(value2)) {
    return Object.fromEntries(Object.entries(value2).sort(([left], [right]) => left.localeCompare(right)).map(([key, item]) => [key, stableValue8(item)]));
  }
  return value2;
}
function managedContentDigest(root, entry) {
  const absolutePath = path31.join(root, entry.path);
  ...
  if (entry.mode === "exclusive-file") return { state: "current", digest: sha25614(fs25.readFileSync(absolutePath)), message: null };
  if (entry.mode === "json-fragment") {
    try {
      const value2 = parseJsonWithoutDuplicateKeys(fs25.readFileSync(absolutePath, "utf8"), entry.path);
      const fragment = extractJsonPointer(value2, entry.selector);
      return { state: "current", digest: sha25614(JSON.stringify(stableValue8(fragment))), message: null };
    } catch (error) {
...
function inspectIntegrationManifest(start, options) {
  ...
  let manifest;
  try {
    manifest = options.readInstallationManifest ? options.readInstallationManifest(project.root) : readProjectInstallationManifest(project.root, { fsOps: options.fsOps, hostIds: PROJECT_HOST_IDS, allowPrevious: true });
  } catch (error) {
...
function inspectManifestOwnership(project, manifest) {
  const managed = manifest.managed.map((entry) => {
    try {
      ...
      const healthy2 = inspected.state === "current" && SHA2563.test(entry.digest) && inspected.digest === entry.digest;
      return {
        path: entry.path,
        owner: entry.owner,
        mode: entry.mode,
...
function inspectWorkspace(root) {
  ...
  try {
    ...
    const mode2 = workspace.state === "absent" ? "absent" : workspace.healthy ? "current" : workspace.category === "invalid" ? "invalid" : "archive-reset-required";
    return {
...
function sameMcpServer(value2) {
  return plainObject6(value2) && Object.keys(value2).sort().join(",") === "args,command,type" && value2.type === INSTALLED_DOVE_MCP_SERVER.type && value2.command === INSTALLED_DOVE_MCP_SERVER.command && Array.isArray(value2.args) && value2.args.length === INSTALLED_DOVE_MCP_SERVER.args.length && value2.args.every((item, index) => item === INSTALLED_DOVE_MCP_SERVER.args[index]);
}
function inspectClaudeRegistration(root, integration) {
  ...
  try {
    const mcpPath = path31.join(root, DOVE_MCP_CONFIG_PATH);
    ...
    if (stat === null) missing.push(DOVE_MCP_CONFIG_PATH);
      ...
      const value2 = parseJsonWithoutDuplicateKeys(fs25.readFileSync(mcpPath, "utf8"), DOVE_MCP_CONFIG_PATH);
      mcp = sameMcpServer(value2?.mcpServers?.[DOVE_MCP_SERVER_NAME]) ? { healthy: true, state: "registered", message: null } : { healthy: false, state: "drifted", message: `${DOVE_MCP_CONFIG_PATH} does not contain the current Dove user-CLI registration.` };
  ...
  try {
    const approvalPath = path31.join(root, DOVE_CLAUDE_LOCAL_SETTINGS_PATH);
    ...
    if (stat === null) missing.push(DOVE_CLAUDE_LOCAL_SETTINGS_PATH);
      ...
      const settings = parseJsonWithoutDuplicateKeys(fs25.readFileSync(approvalPath, "utf8"), DOVE_CLAUDE_LOCAL_SETTINGS_PATH);
  ...
  for (const relativePath of [DOVE_CLAUDE_AMBIENT_RULE_PATH, DOVE_CLAUDE_AMBIENT_SKILL_PATH]) {
    if (!regularNonSymlink(path31.join(root, relativePath))) missing.push(relativePath);
  }
  let ambientMessage = null;
  try {
    const settingsPath = path31.join(root, DOVE_CLAUDE_SETTINGS_PATH);
    ...
    if (stat === null) missing.push(DOVE_CLAUDE_SETTINGS_PATH);
      ...
      const settings = parseJsonWithoutDuplicateKeys(fs25.readFileSync(settingsPath, "utf8"), DOVE_CLAUDE_SETTINGS_PATH);
...
function normalizeReadiness(value2) {
  if (!plainObject6(value2)) return { healthy: false, ready: false, state: "invalid-result", message: "Claude connection inspection returned an invalid result." };
  const state2 = typeof value2.state === "string" && value2.state ? value2.state : "unknown";
  const ready = state2 === "connected";
  return { healthy: ready, ready, state: state2, message: typeof value2.message === "string" ? value2.message : null };
}
function inspectReadiness(root, registration, options, integration = null) {
  ...
  try {
    const inspector = options.inspectClaudeConnection ?? ((target2) => inspectClaudeMcpConnection(target2, options));
...
function inspectProjectDoctor(start, options = {}) {
  ...
  const healthy = userCli.healthy && projectIntegration.healthy && workspaceState.healthy && hostRegistration.healthy && readiness.healthy && legacyCopiedRuntime.healthy;
  return {
    healthy,
    state: healthy ? "healthy" : "unhealthy",
    target: safeRoot ?? (typeof start === "string" ? path31.resolve(start) : null),
    userCli,
    projectIntegration,
    workspaceState,
    hostRegistration,
...
function deepFreezePublic(value2, seen = /* @__PURE__ */ new WeakSet()) {
  if (value2 === null || typeof value2 !== "object" || seen.has(value2)) return value2;
  seen.add(value2);
  for (const item of Array.isArray(value2) ? value2 : Object.values(value2)) deepFreezePublic(item, seen);
...
function safeStrings(value2, transform = safeText2) {
  return Array.isArray(value2) ? value2.filter((item) => typeof item === "string").map(transform) : [];
}
function publicFactStatements(value2) {
  if (!Array.isArray(value2)) return [];
  return value2.flatMap((item) => {
...
function publicResearchDeviation(value2) {
  if (PUBLIC_RESEARCH_DEVIATIONS[value2]) return PUBLIC_RESEARCH_DEVIATIONS[value2];
  if (typeof value2 === "string" && value2.startsWith("budget-exceeded:")) return "The reported execution exceeded a declared budget limit.";
  if (typeof value2 === "string" && value2.startsWith("undeclared-budget-dimension:")) return "The reported execution included an undeclared budget dimension.";
  if (typeof value2 === "string" && value2.startsWith("unexpected-evidence:")) return "The returned evidence did not match the requested research action.";
...
function publicResearchJudgment(value2) {
  return PUBLIC_RESEARCH_JUDGMENTS[value2] ?? "Dove must review the available evidence before deciding whether research should continue.";
}
function chineseResearchJudgment(value2) {
  return PUBLIC_RESEARCH_JUDGMENTS_ZH[value2] ?? "Dove \u9700\u8981\u5148\u68C0\u67E5\u73B0\u884C\u8BC1\u636E\uFF0C\u518D\u5224\u65AD\u662F\u5426\u7EE7\u7EED\u7814\u7A76\u3002";
}
function publicPath(value2) {
  return typeof value2 === "string" && value2.trim() && !/(?:^|\/)\.dove(?:-archive)?(?:\/|$)/u.test(value2) ? safeText2(value2) : null;
}
function publicArtifacts(items) {
  if (!Array.isArray(items)) return [];
  return items.flatMap((item) => {
    const path35 = publicPath(typeof item === "string" ? item : item?.path ?? item?.reference);
    if (!path35) return [];
    const result = { path: path35 };
...
function publicFieldValue(field, value2) {
  if (["message", "error", "summary", "details", "title", "snippet", "sourceName", "displayName", "kind", "access", "status", "locator", "sourceType", "abstract", "lifecycle", "scope"].includes(field)) {
    return typeof value2 === "string" ? safeText2(value2) : void 0;
  }
  if (["authors", "capabilities", "nextTimeGuidance", "tags"].includes(field)) return safeStrings(value2);
  if (["year", "resultCount", "fetchedCount"].includes(field)) return Number.isFinite(value2) ? value2 : void 0;
  if (field === "openAccess") return typeof value2 === "boolean" ? value2 : void 0;
  if (field === "url") return typeof value2 === "string" ? safeText2(value2) : void 0;
  if (field === "publishedAt") return typeof value2 === "string" ? safeText2(value2) : void 0;
  if (field === "eligibility" && isPlainObject(value2)) return { eligible: value2.eligible === true, reason: safeReason(value2.reason) };
  return void 0;
}
function publicItems(items, fields) {
  ...
  return items.filter(isPlainObject).map((item) => Object.fromEntries(fields.flatMap((field) => {
    ...
    const value2 = publicFieldValue(field, item[field]);
    return value2 === void 0 ? [] : [[field, value2]];
  })));
}
function publicOperationalIntegrity(value2, fallback = {}) {
  const source = isPlainObject(value2) ? value2 : {};
  return {
...
function graphCollection(graph, name) {
  const value2 = graph?.[name];
  return isPlainObject(value2) ? value2 : { totalCount: 0, truncated: false, items: [] };
}
...
function chineseReason(value2, category = null) {
  const safe = safeReason(value2);
...
function publicTechnicalAppendix(data) {
  ...
  const visibleNumber = (mapping, value2) => Number.isSafeInteger(value2) ? mapping.get(value2) ?? null : null;
  const visibleNumbers = (mapping, values) => Array.isArray(values) ? values.map((value2) => visibleNumber(mapping, value2)).filter(Number.isSafeInteger) : [];
  return {
    bounded: true,
...
function assertHumanReportPrivacy(report) {
  const visit = (value2, location = "report") => {
    if (Array.isArray(value2)) {
      value2.forEach((item, index) => visit(item, `${location}[${index}]`));
      return;
    }
    ...
    if (typeof value2 === "string" && /\b(?:resume[-_ ]?original|host[-_ ]?outcome|research[-_ ]?outcome|host[-_ ]?control|closureRequest|boundArgs|requiredOutcomeFields|exactly[-_ ]?once)\b/iu.test(value2)) {
...
function valueAtPath(value2, path35) {
  return path35.split(".").reduce((current, field) => isPlainObject(current) ? current[field] : void 0, value2);
}
...
function publicResult(name, data, invocation, options = {}) {
  ...
  const selector2 = options.selector;
  if (selector2 !== void 0 && (!isPlainObject(selector2) || Object.keys(selector2).length !== 1 || !Number.isSafeInteger(selector2.missionNumber) || selector2.missionNumber < 1)) {
    throw new Error("Public result selector requires exactly one positive missionNumber.");
  }
  return deepFreezePublic({
    report,
    ...selector2 ? { selector: { missionNumber: selector2.missionNumber } } : {},
...
function reportLine(value2, fallback = "None") {
  return typeof value2 === "string" && value2.trim() ? safeText2(value2).trim() : fallback;
}
...
function chineseAction(value2, fallback = "\u7EE7\u7EED\u5904\u7406\u4F18\u5148\u7EA7\u6700\u9AD8\u7684\u672A\u5B8C\u6210\u5DE5\u4F5C\uFF0C\u5E76\u8865\u5145\u5BF9\u5E94\u8BC1\u636E\u3002") {
  const text11 = reportLine(value2, "");
...
function plain7(value2, label) {
  if (!value2 || typeof value2 !== "object" || Array.isArray(value2)) throw new Error(`${label} must be a plain object.`);
}
function sealed11(value2, fields, label) {
  plain7(value2, label);
  const unknown = Object.keys(value2).filter((field) => !fields.has(field));
  if (unknown.length) throw new Error(`${label} does not accept unknown fields: ${unknown.join(", ")}.`);
}
function safeId11(value2, label) {
  if (typeof value2 !== "string" || !/^[a-z0-9][a-z0-9._-]{0,127}$/u.test(value2)) throw new Error(`${label} must be a safe lowercase identifier.`);
  return value2;
}
function sha25615(value2) {
  return crypto25.createHash("sha256").update(typeof value2 === "string" || Buffer.isBuffer(value2) ? value2 : stableWorkspaceSerialize(value2)).digest("hex");
}
...
function recordResearchOutcome(root, args2 = {}) {
  ...
  const missionId2 = safeId11(args2.missionId, "missionId");
  const attemptId = safeId11(args2.attemptId, "attemptId");
  ...
  if (!decision || decision.revision !== args2.decisionRevision || !decision.nextAction) throw new Error("The current research decision is stale or does not authorize an action.");
  ...
  const outcome = validateResearchHostOutcome(handoff, { status: args2.status, performedActionCount: args2.performedActionCount, actualUsage: args2.actualUsage, evidenceReturned: args2.evidenceReturned, facts: args2.facts, claims: [], startedAt: args2.startedAt, finishedAt: args2.finishedAt }, { missionId: decision.missionId, contractDigest: decision.contractDigest, decisionDigest: decision.decisionDigest, currentEnvelopeId: handoff.envelopeId, supersededEnvelopeIds: [], now });
  ...
  const artifacts = inspectPaths(root, args2.artifactPaths, "artifactPaths");
  const validationFiles = inspectPaths(root, args2.validationPaths, "validationPaths");
  ...
  const digest3 = callbackDigest(args2, decision, outcome, artifacts, validationFiles);
  ...
  const receiptId = `receipt-research-${sha25615({ missionId: missionId2, attemptId }).slice(0, 24)}`;
  ...
  context.requireCommitLock(".dove/.receipt-ledger-append.lock", { label: "Execution receipt ledger append lock" });
  assertReceiptAppendable(workspace.receiptLedger, receipt, { artifactHandoffs: workspace.artifactHandoffs });
  writeJson(root, path32.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`), receipt);
  return { status: "recorded", accepted: true, evidenceComplete: outcome.missingRequiredEvidence.length === 0, missingRequiredEvidence: outcome.missingRequiredEvidence, scopeDeviation: outcome.scopeDeviation, scopeDeviationReasons: outcome.scopeDeviationReasons, outcomeStatus: outcome.status, performedActionCount: outcome.performedActionCount, receipt, awaitingReevaluation: true, decision, writes: [path32.posix.join(ARTIFACT_PATHS.executionReceiptsDir, `${receiptId}.json`)] };
}

// src/core/host-path-normalizer.mjs
import path33 from "node:path";
function invalidPath(label, value2) {
  throw new Error(`${label} must name one canonical path inside the workspace: ${value2}`);
}
function normalizeHostWorkspacePath(root, value2, label = "Host path") {
  if (typeof value2 !== "string" || !value2 || value2 !== value2.trim() || value2.includes("\0") || value2.includes("\\")) invalidPath(label, value2);
  if (/^[A-Za-z][A-Za-z0-9+.-]*:/u.test(value2) && !/^[A-Za-z]:[\\/]/u.test(value2)) invalidPath(label, value2);
  const resolvedRoot = path33.resolve(root);
  if (path33.isAbsolute(value2)) {
    const normalizedAbsolute = path33.resolve(value2);
    if (normalizedAbsolute !== value2) invalidPath(label, value2);
    const relative = path33.relative(resolvedRoot, normalizedAbsolute);
    if (!relative || relative === ".." || relative.startsWith(`..${path33.sep}`) || path33.isAbsolute(relative)) invalidPath(label, value2);
    return relative.split(path33.sep).join("/");
  }
  if (/^[A-Za-z]:[\\/]/u.test(value2)) invalidPath(label, value2);
  const normalized3 = path33.posix.normalize(value2);
  if (normalized3 !== value2 || normalized3 === "." || normalized3 === ".." || normalized3.startsWith("../") || value2.includes("//") || value2.endsWith("/")) invalidPath(label, value2);
  return value2;
}
function normalizeHostWorkspaceArtifactPath(root, value2, label = "Host artifact path") {
  let relativePath;
  try {
    relativePath = normalizeHostWorkspacePath(root, value2, label);
  } catch (error) {
    throw new Error(`${label} must name one canonical regular file or future file inside the workspace: ${value2}`, { cause: error });
  }
...
function normalizeHostWorkspaceFilePath(root, value2, label = "Host file path") {
  let relativePath;
  try {
    relativePath = normalizeHostWorkspacePath(root, value2, label);
  } catch (error) {
    throw new Error(`${label} must name one canonical regular file inside the workspace: ${value2}`, { cause: error });
  }
  const anchor = openAnchoredFilesystem(root);
  try {
    anchor.inspectRegularFile(relativePath);
...
function valueTypeMatches(value2, type) {
  if (type === "null") {
    return value2 === null;
  }
  if (type === "array") {
...
function validateSchemaValue(value2, schema, inputPath) {
  if (!schema || typeof schema !== "object" || Array.isArray(schema)) {
    return null;
  }
  if (Array.isArray(schema.allOf)) {
    for (const branch of schema.allOf) {
      const error = validateSchemaValue(value2, branch, inputPath);
      if (error) {
        return error;
      }
    }
  ...
  if (schema.if) {
    const conditionError = validateSchemaValue(value2, schema.if, inputPath);
  ...
  if (Array.isArray(schema.anyOf)) {
    const errors = schema.anyOf.map((branch) => validateSchemaValue(value2, branch, inputPath));
    if (errors.every(Boolean)) {
      return errors.at(-1) ?? `${inputPath} does not match any allowed schema.`;
    }
  }
  if (Array.isArray(schema.oneOf)) {
    const matches = schema.oneOf.map((branch) => validateSchemaValue(value2, branch, inputPath)).filter((error) => !error).length;
    if (matches !== 1) {
      return `${inputPath} must match exactly one allowed schema.`;
    }
  }
  if (schema.not && !validateSchemaValue(value2, schema.not, inputPath)) {
    return `${inputPath} matches a forbidden schema.`;
  }
  if (schema.const !== void 0 && !Object.is(value2, schema.const)) {
    return `${inputPath} must equal ${JSON.stringify(schema.const)}.`;
  }
  if (Array.isArray(schema.enum) && !schema.enum.some((item) => Object.is(item, value2))) {
    return `${inputPath} must be one of: ${schema.enum.map((item) => JSON.stringify(item)).join(", ")}.`;
  }
  if (schema.type !== void 0) {
    const types = Array.isArray(schema.type) ? schema.type : [schema.type];
    if (!types.some((type) => valueTypeMatches(value2, type))) {
      return `${inputPath} must be ${expectedTypeLabel(schema.type)}.`;
    }
  }
  if (typeof value2 === "string") {
    if (typeof schema.minLength === "number" && value2.length < schema.minLength) {
      return `${inputPath} must contain at least ${schema.minLength} character(s).`;
    }
    if (typeof schema.maxLength === "number" && value2.length > schema.maxLength) {
      return `${inputPath} must contain at most ${schema.maxLength} character(s).`;
    }
    if (typeof schema.pattern === "string" && !new RegExp(schema.pattern, "u").test(value2)) {
      return `${inputPath} does not match the required pattern.`;
    }
  }
  if (typeof value2 === "number") {
    if (typeof schema.minimum === "number" && value2 < schema.minimum) {
      return `${inputPath} must be at least ${schema.minimum}.`;
    }
    if (typeof schema.maximum === "number" && value2 > schema.maximum) {
      return `${inputPath} must be at most ${schema.maximum}.`;
    }
  }
  if (Array.isArray(value2)) {
    if (typeof schema.minItems === "number" && value2.length < schema.minItems) {
      return `${inputPath} must contain at least ${schema.minItems} item(s).`;
    }
    if (typeof schema.maxItems === "number" && value2.length > schema.maxItems) {
      return `${inputPath} must contain at most ${schema.maxItems} item(s).`;
    }
    if (schema.items) {
      for (let index = 0; index < value2.length; index += 1) {
        const error = validateSchemaValue(value2[index], schema.items, `${inputPath}[${index}]`);
        if (error) {
          return error;
        }
      }
  ...
  if (isPlainObject2(value2)) {
    ...
    if (Array.isArray(schema.required)) {
      const missing = schema.required.find((key) => !Object.hasOwn(value2, key));
      if (missing) {
        return `${childPath(inputPath, missing)} is required.`;
      }
    }
    if (schema.additionalProperties === false) {
      const unknown = Object.keys(value2).find((key) => !Object.hasOwn(properties, key));
      if (unknown) {
    ...
    for (const [key, propertySchema] of Object.entries(properties)) {
      ...
      const error = validateSchemaValue(value2[key], propertySchema, childPath(inputPath, key));
      if (error) {
        return error;
      }
    }
...
function assertMcpInputSchema(name, args2, schema) {
  const error = validateSchemaValue(args2, schema, "$");
  if (error) {
...
function dispatchData(root, name, args2) {
  switch (name) {
    case "manage_dove_workspace":
      return manageDoveWorkspace(root, args2);
    case "create_dove_mission":
      return createDoveMission(root, args2);
    case "start_dove_skill_mission":
      return startDoveSkillMission(root, args2);
    case "reevaluate_research_decision":
      return reevaluateResearchDecision(root, args2);
    case "query_dove_mission":
      return queryDoveMission(root, args2);
    case "query_dove_status":
      return queryDoveStatus(root, args2);
    case "assess_mission_completion":
      return assessMissionCompletion(root, args2);
    case "close_host_outcome":
      return closeHostOutcome(root, args2);
    case "record_research_outcome":
      return recordResearchOutcome(root, args2);
    case "query_sources":
      return querySources(root, args2);
    case "read_dove_lessons":
      return readDoveLessons(root, args2);
    case "update_dove_lessons":
      return updateDoveLessons(root, args2);
    case "register_source":
      return registerSource(root, args2);
    case "verify_source":
      return verifySource(root, args2);
    case "upsert_claims":
      return upsertClaims(root, args2);
    case "run_experience_workflow":
      return runExperienceWorkflow(root, args2);
    case "record_dove_draft":
      return recordDoveDraft(root, args2);
    case "record_dove_figure":
      return recordDoveFigure(root, args2);
    case "scope_review_record":
      return scopeReviewRecord(root, args2);
    case "archive_review_record":
      return archiveReviewRecord(root, args2);
    case "record_dove_rebuttal":
      return recordDoveRebuttal(root, args2);
    default:
...
function missionRelationshipArgs(root, args2) {
  const result = { ...args2 };
...
function coreCheckpointArgs(root, name, args2) {
  if (name !== "create_dove_mission") return args2;
  if (typeof args2.goal !== "string" || !args2.goal) {
    throw new Error("Mission creation requires a goal.");
  }
  return { ...missionRelationshipArgs(root, args2), missionId: newMissionId() };
}
function checkpointProposal(root, name, args2) {
  return dispatchData(root, name, { ...coreCheckpointArgs(root, name, args2), mutationMode: "direct-process" });
}
function applyCheckpoint(root, name, args2, proposal) {
...
function normalizeApprovalAction(value2) {
  if (value2 === "accept" || value2 === "decline" || value2 === "cancel") return value2;
  throw new Error("Checkpoint approval returned an unsupported action.");
}
function publicCheckpointProposalError(name, args2, error) {
  const message = error instanceof Error ? error.message : String(error);
...
function routedInvocation(operation, args2) {
  const route = operationRoute(operation, args2);
  const targetName = operationTargetTool(operation, args2);
  const projectorName = operationPublicProjector(operation, args2);
  const targetArgs = { ...args2, ...route?.fixedArgs ?? {} };
...
function executeTool(root, name, args2, options = {}) {
  const operation = operationForTool(name);
  const interaction = operationInteraction(operation, args2);
  const { targetName, projectorName, targetArgs } = routedInvocation(operation, args2);
  const project = (data, projectionOptions = {}) => projectInvocation(root, projectorName, data, operation, { invocationArgs: args2, ...projectionOptions });
  if (name === "manage_dove_workspace" && args2.operation === "set-mainline") {
  ...
  if (operationRequiresCheckpoint(operation, args2)) {
    let proposal;
    try {
      proposal = checkpointProposal(root, targetName, targetArgs);
    } catch (error) {
      throw publicCheckpointProposalError(name, args2, error);
    }
    const approval = publicResult(projectorName, proposal, classifyInvocationOutcome(proposal, operation, args2), { operation }).report.approval;
    ...
    return Promise.resolve(options.requestCheckpointApproval(approval)).then((value2) => {
      const action = normalizeApprovalAction(value2);
      if (action !== "accept") {
  ...
  if (interaction === "read") {
    ...
    return data && typeof data.then === "function" ? data.then((value2) => project(value2)) : project(data);
  }
...
function hostFile(root, value2, label) {
  return normalizeHostWorkspaceFilePath(root, value2, label);
}
function hostFiles(root, values, label) {
  return Array.isArray(values) ? values.map((value2, index) => hostFile(root, value2, `${label}[${index}]`)) : values;
}
function hostArtifact(root, value2, label) {
  return normalizeHostWorkspaceArtifactPath(root, value2, label);
}
...
function typedEvidenceList(root, values, label) {
  return Array.isArray(values) ? values.map((value2, index) => typedEvidence2(root, value2, `${label}[${index}]`)) : values;
}
...
function missionEvidenceList(root, values, label) {
  return Array.isArray(values) ? values.map((value2, index) => missionEvidence(root, value2, `${label}[${index}]`)) : values;
}
...
function normalizeHostPathInputs(root, name, args2) {
  const result = structuredClone(args2);
  switch (name) {
    case "register_source":
    ...
    case "record_dove_rebuttal":
      ...
      if (Array.isArray(result.findingRefs)) result.findingRefs = result.findingRefs.map((value2, index) => findingReference(root, value2, `findingRefs[${index}]`));
      break;
    case "scope_review_record":
      result.artifactPaths = hostFiles(root, result.artifactPaths, "artifactPaths");
      break;
...
function dispatchTool(root, name, args2 = {}, options = {}) {
  let operation = null;
  ...
  try {
    ...
    return result && typeof result.then === "function" ? result.then((value2) => textResult(value2, { language })).catch((error) => {
...
function startServer(root = process3.cwd()) {
  ...
  function sendMessage(message) {
    ...
    if (responseFraming === "jsonl") {
      process3.stdout.write(`${body}
`);
      return;
    }
    process3.stdout.write(`Content-Length: ${Buffer.byteLength(body, "utf8")}\r
\r
${body}`);
  }
  ...
  function parseMessages() {
    while (buffer.length > 0) {
      ...
      const parsed2 = prefix.startsWith("{") ? parseJsonLineMessage() : parseContentLengthMessage();
      if (!parsed2) return;
    }
  }
  process3.stdin.on("data", (chunk) => {
    buffer = Buffer.concat([buffer, chunk]);
    parseMessages();
  });
  process3.stdin.on("end", () => {
...
var __dirname2 = path34.dirname(__filename2);
var PACKAGE_ROOT2 = path34.resolve(__dirname2, "..");
var PACKAGE = JSON.parse(fs26.readFileSync(path34.join(PACKAGE_ROOT2, "package.json"), "utf8"));
var PACKAGE_OPTIONS = { packageName: PACKAGE.name, packageVersion: PACKAGE.version };
var BUSINESS_COMMANDS = /* @__PURE__ */ new Set(["mission", "status", "lessons", "source", "experiment", "draft", "figure", "review", "rebuttal"]);
var KNOWN_COMMANDS = /* @__PURE__ */ new Set(["init", "sync", "doctor", "workspace", "mcp", "hook", ...BUSINESS_COMMANDS]);
function usage() {
  console.log(`dove
...
function optionalPositiveInteger(args2, flag) {
  if (!args2.includes(flag)) return void 0;
  const raw = readFlagValue(args2, flag);
  if (!/^\d+$/u.test(raw ?? "")) throw new Error(`${flag} must be an integer greater than or equal to 1.`);
  const value2 = Number(raw);
  if (!Number.isSafeInteger(value2) || value2 < 1) throw new Error(`${flag} must be an integer greater than or equal to 1.`);
  return value2;
}
function missionIdForNumber(root, args2) {
  const missionNumber = optionalPositiveInteger(args2, "--mission-number");
  if (missionNumber === void 0) return void 0;
...
function mutationMode2(args2, fallback = "patch-plan") {
...
function resolveTarget(rawTarget2) {
  return path34.resolve(process4.cwd(), rawTarget2 || ".");
}
function projectFlag(args2) {
  return optionalFlagValue(args2, "--project");
}
function projectStart(rawTarget2, args2, { allowPositional = true } = {}) {
  const explicitProject = projectFlag(args2);
  if (rawTarget2 !== void 0 && !allowPositional) throw new Error("This command does not accept a positional project target.");
  if (rawTarget2 !== void 0 && explicitProject !== void 0) throw new Error("A positional project target cannot be combined with --project.");
  return resolveTarget(explicitProject ?? rawTarget2 ?? ".");
}
function resolveBusinessTarget(rawTarget2, args2) {
  return resolveInstalledProjectRoot(projectStart(rawTarget2, args2));
}
function selectedHosts(args2) {
  const raw = readFlagValues(args2, "--host");
...
function writeIntegrationResult(command3, result, args2) {
  if (wantsJson(args2)) {
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  console.log(renderProjectIntegrationResult(command3, result, {
    stream: process4.stdout,
    env: process4.env
  }));
}
...
function runMutation(target2, actionId, args2, callback, fallback = "patch-plan") {
  ...
  const result = runWithMutationContext(target2, { actionId, mutationMode: mutationMode2(args2, fallback), hostId: "cli" }, () => callback(cleanArgs));
  return resolveExecutionReceiptPostCommit(target2, result);
}
function workspaceArgs(action, args2) {
  const goal = optionalFlagValue(args2, "--goal");
  const mainline = optionalFlagValue(args2, "--mainline");
  const changeReason = optionalFlagValue(args2, "--change-reason");
  if (action === "init") {
    if (args2.includes("--archive") || changeReason !== void 0) throw new Error("dove workspace init does not accept --archive or --change-reason.");
    return definedObject({ operation: "initialize", goal, mainline, mutationMode: mutationMode2(args2, "direct-process") });
  }
  if (action === "revise-mainline") {
    if (args2.includes("--archive")) throw new Error("dove workspace revise-mainline does not accept --archive.");
    return definedObject({ operation: "revise-mainline", goal, mainline, changeReason, mutationMode: mutationMode2(args2, "direct-process") });
  }
  if (action === "migrate") {
    if (goal !== void 0 || mainline !== void 0 || changeReason !== void 0 || args2.includes("--archive")) throw new Error("dove workspace migrate accepts no goal, mainline, change reason, or archive fields.");
    return { operation: "migrate-workspace", mutationMode: mutationMode2(args2, "direct-process") };
  }
  if (action === "reset") {
    if (!args2.includes("--archive")) throw new Error("dove workspace reset requires --archive.");
    return definedObject({ operation: "initialize", goal, mainline, archiveReset: true, mutationMode: mutationMode2(args2, "direct-process") });
  }
...
function missionArgs(args2) {
  ...
  const result = definedObject({
    operation,
    mutationMode: mutationMode2(args2, "direct-process"),
    missionNumber: optionalPositiveInteger(args2, "--mission-number"),
    missionGoal: optionalFlagValue(args2, "--mission-goal"),
    mode: optionalFlagValue(args2, "--mode"),
    goal: optionalFlagValue(args2, "--goal"),
    requirements: optionalFlagValues(args2, "--requirement"),
    assumptions: optionalFlagValues(args2, "--assumption"),
    ...
    dependsOnMissionNumbers: args2.includes("--depends-on-mission-number") ? readFlagValues(args2, "--depends-on-mission-number").map((value2) => {
      if (!/^\d+$/u.test(value2)) throw new Error("--depends-on-mission-number must contain integers greater than or equal to 1.");
      const number2 = Number(value2);
      if (!Number.isSafeInteger(number2) || number2 < 1) throw new Error("--depends-on-mission-number must contain integers greater than or equal to 1.");
      return number2;
    }) : void 0,
    parentMissionNumber: optionalPositiveInteger(args2, "--parent-mission-number"),
    branchKind: optionalFlagValue(args2, "--branch-kind"),
    branchReason: optionalFlagValue(args2, "--branch-reason"),
    stopParentReason: optionalFlagValue(args2, "--stop-parent-reason"),
    handoffArtifactPaths: optionalFlagValues(args2, "--handoff-artifact"),
    requestedDisposition: optionalFlagValue(args2, "--requested-disposition"),
    synthesis: optionalFlagValue(args2, "--synthesis"),
    hypotheses: args2.includes("--hypothesis-json") ? parseRepeatedJson(args2, "--hypothesis-json") : void 0,
    routes: args2.includes("--route-json") ? parseRepeatedJson(args2, "--route-json") : void 0,
    openQuestions: args2.includes("--open-question-json") ? parseRepeatedJson(args2, "--open-question-json") : void 0,
    evidenceRefs: optionalFlagValues(args2, "--evidence-ref"),
    reasonCodes: optionalFlagValues(args2, "--reason-code")
  });
  if (args2.includes("--next-action-json")) result.nextAction = parseJson(readFlagValue(args2, "--next-action-json"), "--next-action-json");
  return result;
}
function lessonsUpdateArgs(args2) {
  return {
    binding: readFlagValue(args2, "--binding"),
    markdown: readFlagValue(args2, "--markdown")
  };
}
function sourceArgs(root, args2, action) {
  if (action === "query") return definedObject({ missionId: missionIdForNumber(root, args2), sourceId: optionalFlagValue(args2, "--source-id") });
  if (action === "verify") return definedObject({
    missionId: missionIdForNumber(root, args2),
...
function reviewArgs(root, args2) {
  const modes = ["scope", "archive"].filter((mode3) => args2.includes(`--${mode3}`));
  if (modes.length > 1) throw new Error("dove review accepts one operation mode.");
  const mode2 = modes[0] ?? "scope";
  const missionId2 = missionIdForNumber(root, args2);
  if (mode2 === "scope") return { mode: mode2, args: { missionId: missionId2, reviewMissionBinding: readFlagValue(args2, "--review-mission-binding"), hostKind: readFlagValue(args2, "--host-kind"), artifactPaths: readFlagValues(args2, "--artifact") } };
  return { mode: mode2, args: definedObject({
    missionId: missionId2,
    scopeBinding: optionalFlagValue(args2, "--scope-binding-json") ? parseJson(optionalFlagValue(args2, "--scope-binding-json"), "--scope-binding-json") : void 0,
    status: optionalFlagValue(args2, "--status"),
    verdict: optionalFlagValue(args2, "--verdict"),
    summary: optionalFlagValue(args2, "--summary"),
    findings: optionalFlagValue(args2, "--findings-json") ? parseJson(optionalFlagValue(args2, "--findings-json"), "--findings-json") : void 0,
    actionItems: optionalFlagValues(args2, "--action-item"),
    report: optionalFlagValue(args2, "--report"),
    provenance: optionalFlagValue(args2, "--provenance-json") ? parseJson(optionalFlagValue(args2, "--provenance-json"), "--provenance-json") : void 0
  }) };
}
function statusArgs(args2) {
  ...
  return definedObject({ missionNumber: optionalPositiveInteger(args2, "--mission-number"), detail, language });
}
function wantsJson(args2) {
  return args2.includes("--json") || readFlagValue(args2, "--format") === "json";
}
function toolForCommand(command3, { lessonsAction: lessonsAction2 = "read", sourceAction: sourceAction2 = "query", args: args2 = [] } = {}) {
  switch (command3) {
    case "init":
      return null;
    case "mission":
      return "create_dove_mission";
    case "status":
      return "query_dove_status";
    case "lessons":
      return lessonsAction2 === "update" ? "update_dove_lessons" : "read_dove_lessons";
    case "source":
      return sourceAction2 === "register" ? "register_source" : sourceAction2 === "verify" ? "verify_source" : "query_sources";
    case "experiment":
      return "record_dove_experiment";
    case "draft":
      return "record_dove_draft";
    case "figure":
      return "record_dove_figure";
    case "review":
      return args2.includes("--archive") ? "archive_review_record" : "scope_review_record";
    case "rebuttal":
      return "record_dove_rebuttal";
    default:
      return null;
  }
}
function publicReportFor(toolName, result, args2) {
  ...
  return publicResult(toolName, data, classifyInvocationOutcome(data, operation), {
    includeTechnicalAppendix: toolName === "query_dove_status" && readFlagValue(args2, "--detail") === "full"
  });
}
function cliResponseLanguage(target2, args2) {
  return resolveDoveResponseLanguage(target2, definedObject({ language: optionalFlagValue(args2, "--language") }));
}
function printResult(toolName, result, args2, target2 = process4.cwd()) {
  const envelope2 = publicReportFor(toolName, result, args2);
  if (wantsJson(args2)) console.log(JSON.stringify(envelope2, null, 2));
  else console.log(renderPublicReport(envelope2.report, { language: cliResponseLanguage(target2, args2) }));
}
function printError(toolName, error, args2, target2 = process4.cwd()) {
  const operation = toolName ? operationForTool(toolName) : null;
  const envelope2 = operation ? publicErrorResult(toolName, error, classifyInvocationError(error, operation)) : { report: { status: "blocked", message: String(error instanceof Error ? error.message : error) } };
  if (wantsJson(args2)) console.log(JSON.stringify(envelope2, null, 2));
  else console.error(renderPublicReport(envelope2.report, { language: cliResponseLanguage(target2, args2) }));
}
function printOperationalError(error, args2 = []) {
  const message = error instanceof Error ? error.message : String(error);
  if (wantsJson(args2)) console.log(JSON.stringify({ status: "blocked", message }, null, 2));
...
async function readStdin() {
  ...
  for await (const chunk of process4.stdin) input += chunk;
  return input;
}
var parsed;
try {
  parsed = parseDoveCli(process4.argv.slice(2));
} catch (error) {
  const raw = process4.argv.slice(2);
  const rawCommand = raw[0];
  const rawAction = raw[1];
  const toolName = BUSINESS_COMMANDS.has(rawCommand) ? toolForCommand(rawCommand, {
...
if (!command2) {
  ...
  try {
    resolveInstalledProjectRoot(process4.cwd());
...
if (["help", "--help", "-h"].includes(command2) || parsed.args.includes("--help")) {
  usage();
  process4.exit(0);
}
if (command2 === "--version") {
...
var lessonsAction = "read";
if (command2 === "lessons" && ["read", "update"].includes(rawTarget)) {
...
var selectedTool = command2 === "workspace" ? "manage_dove_workspace" : toolForCommand(command2, { lessonsAction, sourceAction, args });
var target = process4.cwd();
try {
  ...
  if (command2 === "init") {
    ...
    try {
      target = resolveProjectRootForInit(requestedProject, { cwd: process4.cwd(), hostIds: PROJECT_HOST_IDS });
    } catch (error) {
      if (!(error instanceof Error) || !error.message.includes("Dove project integration is already initialized at")) throw error;
      target = resolveInstalledProjectRoot(requestedProject ?? process4.cwd());
      ...
      writeIntegrationResult("init", result3, args);
      process4.exit(0);
    }
    const result2 = initializeProjectIntegration(target, {
      ...PACKAGE_OPTIONS,
      hosts: selectedHosts(args)
    });
    writeIntegrationResult("init", integrationResult(result2), args);
    process4.exit(0);
  }
  if (command2 === "sync") {
    target = projectStart(void 0, args, { allowPositional: false });
    const result2 = syncProjectIntegration(target, {
      ...PACKAGE_OPTIONS,
      hosts: selectedHosts(args)
    });
    writeIntegrationResult("sync", integrationResult(result2), args);
    process4.exit(0);
  }
  if (command2 === "doctor") {
    target = projectStart(void 0, args, { allowPositional: false });
    const result2 = inspectProjectDoctor(target, {
      ...PACKAGE_OPTIONS,
      packageRoot: PACKAGE_ROOT2,
      executablePath: __filename2,
      claudeCommand: process4.env.DOVE_CLAUDE_COMMAND || "claude"
    });
    writeDoctorResult(result2);
    process4.exit(result2.healthy ? 0 : 1);
  }
  if (command2 === "mcp") {
    const action = rawTarget;
    if (action !== "serve") throw new Error("dove mcp accepts only serve.");
    if (projectFlag(args) === void 0) throw new Error("dove mcp serve requires --project <dir>.");
    target = resolveInstalledProjectRoot(projectStart(void 0, args, { allowPositional: false }));
  ...
  if (command2 === "hook") {
    ...
    if (projectFlag(args) === void 0) throw new Error("dove hook user-prompt-submit requires --project <dir>.");
    target = resolveInstalledProjectRoot(projectStart(void 0, args, { allowPositional: false }));
    assertClaudeInstalled(target);
    const output = userPromptSubmitOutput(await readStdin());
    if (output !== null) process4.stdout.write(JSON.stringify(output));
    process4.exit(0);
  }
  if (command2 === "workspace") {
    const action = rawTarget;
    target = resolveInstalledProjectRoot(projectStart(void 0, args, { allowPositional: false }));
  ...
  if (command2 === "mission") {
    ...
    if (input.operation === "reevaluate-research-decision") {
      const result2 = runMutation(target, "reevaluate-research-decision", args, (clean) => {
      ...
      printResult(selectedTool, result2, args, target);
      process4.exit(0);
    }
    ...
    if (ignoredMissionNumber !== void 0 || ignoredMissionGoal !== void 0) throw new Error("Mission creation does not accept --mission-number or --mission-goal.");
    ...
    const parentMissionId = parentMissionNumber === void 0 ? void 0 : resolveMissionNumber(target, parentMissionNumber, { operation: "Dove CLI parent mission selection" }).missionId;
  ...
  if (command2 === "lessons") {
    ...
    const result2 = runMutation(target, "update-dove-lessons", args, (clean) => updateDoveLessons(target, lessonsUpdateArgs(clean)), "direct-process");
    printResult(selectedTool, result2, args, target);
    process4.exit(0);
  }
  let result;
  ...
  if (command2 === "experiment") {
    result = runMutation(target, "run-experience-workflow", args, (clean) => runExperienceWorkflow(target, definedObject({
      missionId: missionIdForNumber(target, clean),
      experimentId: optionalFlagValue(clean, "--experiment-id"),
      title: optionalFlagValue(clean, "--title"),
      protocol: optionalFlagValue(clean, "--protocol-json") ? parseJson(optionalFlagValue(clean, "--protocol-json"), "--protocol-json") : void 0,
      result: optionalFlagValue(clean, "--result-json") ? parseJson(optionalFlagValue(clean, "--result-json"), "--result-json") : void 0
    })));
  }
  const archiveArgs = (clean) => definedObject({
    missionId: missionIdForNumber(target, clean),
    artifactPath: optionalFlagValue(clean, "--artifact-path"),
    referencePaths: optionalFlagValues(clean, "--reference-path"),
    qa: optionalFlagValues(clean, "--qa"),
    findings: optionalFlagValues(clean, "--finding")
  });
  if (command2 === "draft") result = runMutation(target, "record-dove-draft", args, (clean) => recordDoveDraft(target, archiveArgs(clean)));
  if (command2 === "figure") result = runMutation(target, "record-dove-figure", args, (clean) => recordDoveFigure(target, { ...archiveArgs(clean), caption: optionalFlagValue(clean, "--caption") }));
  if (command2 === "review") {
  ...
  if (command2 === "rebuttal") result = runMutation(target, "record-dove-rebuttal", args, (clean) => recordDoveRebuttal(target, { ...archiveArgs(clean), findingRefs: optionalFlagValues(clean, "--finding-ref") }));
