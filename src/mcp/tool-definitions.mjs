...
const OPERATOR_PUBLIC_TOOL_DESCRIPTIONS = {
  ...
  create_dove_task: "Turn a user goal into a confirmable Dove mission contract, then materialize it and hand off to the recommended next workflow after approval.",
...
const baseToolDefinitions = [
  ...
  { name: "create_dove_task", description: "Convert a user demand into a proposal-only mission work contract with compact task card and preActionGuidance; mission is a durable work/progress object, and after approval it materializes the task contract and returns recommended handoff routes without executing or recording a pass.", inputSchema: { type: "object", properties: { id: { type: "string" }, initId: { type: "string" }, initTitle: { type: "string" }, initObjective: { type: "string" }, initGoal: { type: "string" }, projectTitle: { type: "string" }, workspaceTitle: { type: "string" }, projectObjective: { type: "string" }, projectGoal: { type: "string" }, initDomain: { type: "string" }, projectDomain: { type: "string" }, initStatus: { type: "string" }, initArtifactRefs: { type: "array", items: { type: "string" } }, goal: { type: "string" }, objective: { type: "string" }, prompt: { type: "string" }, title: { type: "string" }, summary: { type: "string" }, stage: { type: "string" }, missionStage: { type: "string" }, domain: { type: "string" }, doveDomain: { type: "string" }, missionDomain: { type: "string" }, level: { type: "number" }, taskLevel: { type: "number" }, missionLevel: { type: "number" }, checklist: { type: ["boolean", "array"] }, autoChecklist: { type: "boolean" }, createChecklist: { type: "boolean" }, checklistItems: { type: "array", items: { type: ["object", "string"] } }, subtasks: { type: "array", items: { type: ["object", "string"] } }, creatorKind: { type: "string" }, dependencies: { type: "array", items: { type: "string" } }, dependencyIds: { type: "array", items: { type: "string" } }, blockedBy: { type: "array", items: { type: "string" } }, blockerIds: { type: "array", items: { type: "string" } }, evidenceExpectations: { type: "array", items: { type: "string" } }, artifactRefs: { type: "array", items: { type: "string" } }, artifactPaths: { type: "array", items: { type: "string" } }, lessonIds: { type: "array", items: { type: "string" } }, workContract: { type: "object", properties: workContractProps }, deliverables: { type: "array", items: { type: "string" } }, outOfScope: { type: "array", items: { type: "string" } }, outOfScopeItems: { type: "array", items: { type: "string" } }, evidenceContract: { type: "array", items: { type: "string" } }, doneCriteria: { type: "array", items: { type: "string" } }, practicalImpact: { type: "string" }, recommendedRoutes: { type: "array", items: { type: "object" } }, contextPolicy: { type: "string" }, status: { type: "string" }, executionContract: { type: "object", properties: executionContractProps }, nextAction: { type: "string" }, confirmed: { type: "boolean" }, confirm: { type: "boolean" } } } },
  ...
  { name: "run_dove_review_loop", description: "Run bounded local evidence-aware review, draft, and experience iterations with the max iteration count from Dove settings by default; each iteration inspects local review evidence first. Draft substeps require draftBody or draft.body, and experience substeps require an explicit goal/title/idea/experimentId before any review-loop artifacts are prepared. Isolated or audio reviewer handoff is explicit-only and is not prepared by this ordinary loop. Returns Reviewer preActionGuidance, recalled lessons, and foreground stop conditions.", inputSchema: { type: "object", properties: withTaskTarget({ runId: { type: "string" }, maxIterations: { type: "number" }, artifactPaths: { type: "array", items: { type: "string" } }, finalPlanPaths: { type: "array", items: { type: "string" } }, finalResultPaths: { type: "array", items: { type: "string" } }, draft: { type: "object" }, draftBody: { type: "string" }, sectionId: { type: "string" }, experience: { type: "object" }, experienceGoal: { type: "string" } }) } },
