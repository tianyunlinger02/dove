...
export const toolDefinitions = [
  ...
  { name: "query_dove_status", description: "Read the compact authoritative Dove durable mission statusHome/dailyHome by default, with ranked next actions, proposal-only boundary action cards, and single-dialog status-adjustment preview; pass detail: full or full: true only when full dashboard/task/runtime details are explicitly needed.", inputSchema: { type: "object", properties: { domain: { type: "string" }, doveDomain: { type: "string" }, missionDomain: { type: "string" }, stage: { type: "string" }, missionStage: { type: "string" }, packetId: { type: "string" }, packetIds: { type: "array", items: { type: "string" } }, status: { type: "string" }, statuses: { type: "array", items: { type: "string" } }, includeArchived: { type: "boolean" }, detail: { type: "string" }, view: { type: "string" }, resultMode: { type: "string" }, full: { type: "boolean" }, includeDetails: { type: "boolean" } } } },
  ...
  {
    name: "run_dove_auto",
    description: "Run demand-to-task intake with compact task/auto cards and, after explicit confirmation, bounded foreground iterations until completion or a boundary, returning a localized resultCard summary.",
    inputSchema: {
      type: "object",
  ...
  { name: "run_dove_operator", description: "Preview compact queue cards, then after explicit confirmation run one foreground operator pass across ready/in-progress missions, create pending blocker-investigation plan missions for blocked work, and return a localized resultCard.", inputSchema: { type: "object", properties: { confirmed: { type: "boolean" }, confirm: { type: "boolean" }, runId: { type: "string" }, taskResults: { type: "array", items: { type: "object", properties: operatorTaskResultProps } }, results: { type: "array", items: { type: "object", properties: operatorTaskResultProps } }, passResults: { type: "array", items: { type: "object", properties: operatorTaskResultProps } } } } },
  ...
  { name: "run_audio_review", description: "Prepare an isolated audio review and import a handoff if one is already present; otherwise return import instructions plus a localized resultCard.", inputSchema: { type: "object", properties: withTaskTarget({ runId: { type: "string" }, instructions: { type: "string" }, finalPlanPaths: { type: "array", items: { type: "string" } }, finalResultPaths: { type: "array", items: { type: "string" } }, artifactPaths: { type: "array", items: { type: "string" } }, reviewedArtifactPaths: { type: "array", items: { type: "string" } }, handoffPath: { type: "string" }, reportPath: { type: "string" } }) } },
