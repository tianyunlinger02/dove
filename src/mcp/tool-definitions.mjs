...
export const toolDefinitions = [
  ...
  { name: "query_dove_status", description: "Read the authoritative Dove durable mission dailyHome with ranked next actions, proposal-only boundary action cards, and single-dialog status-adjustment contract without mutating durable state; adjustment items exclude completed and killed missions.", inputSchema: { type: "object", properties: { domain: { type: "string" }, doveDomain: { type: "string" }, missionDomain: { type: "string" }, stage: { type: "string" }, missionStage: { type: "string" }, packetId: { type: "string" }, packetIds: { type: "array", items: { type: "string" } }, status: { type: "string" }, statuses: { type: "array", items: { type: "string" } }, includeArchived: { type: "boolean" } } } },
