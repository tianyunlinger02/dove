...
- `init` creates supported software integration and the Dove agent surface in one transaction. It creates the minimal researcher-owned `RESEARCH.md` only when the research directory is absent; existing research directories remain untouched, even if their overview is missing. Human output reports whether the overview was actually written.
