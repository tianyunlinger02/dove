...
Claude Code remains the supported project initialization path. Project initialization installs the Dove agent surface, SessionStart/prompt hooks, the minimal researcher-owned `RESEARCH.md` bootstrap, pinned external paper-acquisition and hosted Exa MCP fragments plus hidden support Skills, and a project-scoped `permissions.deny` entry for built-in `WebFetch` while preserving built-in `WebSearch`. Dove does not register a research-state service, install the external runtime, approve project trust, write credentials, add CLI/shell/fetch fallbacks, or claim that the minimal research entry is substantive research content.
