...
const COMMAND_SURFACES_BASE = [
  ...
  },
  {
    ...
    constraints: ["Treat the user request as one figure intent; do not ask the user to manually sequence material preparation, result import, or validation.", "Treat `providerId: \"none\"` as a first-class plan-only/manual-output path: record the figure backlog item, material bundle, generation prompt, and awaiting-provider-output or missing-required-materials boundary instead of routing the operator to a separate low-level figure-plan write.", "Treat `providerId: \"gpt-image2\"` as the built-in OpenAI image provider path only when the operator explicitly selects it or config sets it as default; require `OPENAI_API_KEY` through env-var secret reference and do not store inline API keys.", "Resolve the durable task packet before any figure workflow write, then analyze linked sections, claims, experiments, sources, notes, review concerns, and material hints automatically.", "Use redacted Dove config and env-var secret references for external drawing providers; never store inline API keys, tokens, or secrets.", "Do not mark a final figure ready unless it comes from a validated generation import with durable provenance and caption.", "Captions must explain the figure purpose and linked evidence."]
  },
  {
    id: "dove.experience",
...
const COMMAND_UX_DETAILS = {
  ...
  "dove.status": {
    dailyFlow: ["Use this to answer the ordinary operator question: what should I do next?", "Default output should be a compact human translation layer: statusHome.headline, exactly one statusHome.nextStep, statusHome.needsAttention, statusHome.changes, and statusHome.showMore; keep currentContext, preActionGuidance, durableContextNotice, project state, blockers/reconciliation, raw boundary/action cards, and mission lists behind explicit mission/full/debug expansion."],
    targetingBehavior: "Default output is not a mission board or audit report: do not render packet ids, mission lists, boundary/gap codes, blocked counts, execution-gap counts, or required-evidence blocks unless showMissions/includeMissionDetails/full/debug or a normal prompt such as show current missions asks for expansion.",
    confirmationBehavior: "Do not ask for status changes during default `/dove:status`. Use compact adjustment cards and at most one confirmation dialog only after explicit status-change intent or requestStatusAdjustment/includeStatusAdjustmentPreview; no parseable packetId-to-status adjustment means no mutation.",
    expectedOutcome: "The operator sees one Dove state line, one recommended next action, one why/needs line, and one expansion/no-write hint; confirmed status adjustments still return localized resultCard summaries.",
  ...
  "dove.figure": {
    dailyFlow: ["Use this when the user describes the figure they want once, including where it should help the paper or task.", "Dove should gather linked materials, prepare generation/import, write caption provenance, and validate QA without exposing low-level figure tools.", "Use `providerId: \"none\"` for plan-only/manual-output figures; the same workflow records the backlog/material bundle and returns the next import boundary.", "Use `providerId: \"gpt-image2\"` only for explicit foreground OpenAI image generation with `OPENAI_API_KEY` supplied through the environment."],
    targetingBehavior: "Resolve the figure request to one durable task packet before any figure write.",
    confirmationBehavior: "Ask for packet confirmation when the figure target is unclear; provider calls require explicit safe configuration, while plan-only figure preparation stays inside the same confirmed workflow call.",
    expectedOutcome: "A figure plan/run, safe import when available, caption provenance, QA status, or an explicit awaiting-provider/material boundary is recorded.",
