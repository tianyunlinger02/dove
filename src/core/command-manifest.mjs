...
const COMMAND_SURFACES_BASE = [
  ...
  },
  {
    ...
    constraints: ["First call `run_dove_operator` without confirmation to return the proposal-only execution contract, including `autoRunnableTasks`, `hostPassRequiredTasks`, blocked missions, pending skipped missions, and `writes: []`.", "Use interactive confirmation controls when the host supports them before passing `confirmed: true`.", "Run in the current foreground call only; do not schedule background or daemon continuation after the response ends.", "For ready and in-progress missions, run one safe internal workflow step when available or collect one real host pass result in order; pass per-task results to `run_dove_operator` so Dove records lifecycle and runtime state.", "Do not claim real engineering, paper, or experiment work happened when neither a safe internal step nor an actual host pass result exists; let `run_dove_operator` record awaiting host results instead.", "When no safe internal step or actual host pass result exists, persist an `awaiting-host-pass-result` boundary rather than marking work complete.", "Preserve durable role handoff metadata while running queue passes; do not expose planner/builder/reviewer as separate slash commands.", "For blocked missions, create pending child plan missions that investigate the blocker reason and link back to the blocked mission."]
  },
  {
    id: "dove.lessons",
    title: "Dove lessons",
...
const COMMAND_UX_DETAILS = {
  ...
  "dove.mission": {
    dailyFlow: ["Use this for one concrete user demand that should become a durable task and receive one bounded foreground pass.", "Describe the desired outcome in normal language; Dove converts it into title, stage, domain, level, checklist, evidence expectations, and execution route."],
    ...
    confirmationBehavior: "Show the converted contract first, then ask whether to approve and run one pass, adjust, or cancel.",
    expectedOutcome: "After approval, the task packet exists and the host either records the pass result or persists an explicit boundary with evidence requirements and role handoff."
  },
  "dove.auto": {
    dailyFlow: ["Use this when the user wants Dove to continue through bounded foreground iterations after the same demand-to-task intake as mission.", "Start from a new demand or an existing durable task; auto should propose concrete safe steps before consuming the iteration budget."],
    targetingBehavior: "Selects an existing packet when the target is clear, otherwise proposes a new task contract.",
    confirmationBehavior: "Require explicit approval of the selected/proposed task, max iteration budget, and concrete foreground steps.",
    expectedOutcome: "Each foreground iteration is recorded in runtime results and stops at completion, blocker, review/provider boundary, or budget exhaustion with an explicit boundary."
  },
  "dove.status": {
    dailyFlow: ["Use this as the quick dashboard: report the host-visible development situation first.", "Keep durable mission details actionable: only show missions that can be adjusted, surface actionable boundaries, and omit the mission list when none are adjustable."],
    targetingBehavior: "Shows non-init adjustment targets except completed and killed missions; do not show counts or recent completed/killed recaps.",
    confirmationBehavior: "Use at most one confirmation dialog for status changes; no parseable packetId-to-status adjustment means no mutation.",
    expectedOutcome: "The operator sees current work, actionable boundaries, blockers, next action, and optional guarded status adjustments without hidden writes or noisy mission summaries."
  },
  "dove.operator": {
    dailyFlow: ["Use this to inspect the ready/in-progress queue, blocked queue, and pending queue, then run one foreground operator pass after confirmation.", "Do not claim real work happened unless the host supplies actual pass results or a safe internal step can run."],
    targetingBehavior: "Works over the active mission queue rather than one ad hoc target.",
    confirmationBehavior: "Preview the queue with writes: [] first; require approval before recording results or creating blocker investigation missions.",
    expectedOutcome: "Runnable work is recorded from real results, blocked work gets pending investigation missions, and unresolved host work remains awaiting evidence through explicit boundaries."
  },
  "dove.lessons": {
