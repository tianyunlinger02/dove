...
4. This request has one listed project action: `node ./bin/dove.mjs rebuttal . --target "<task title>" --issue "<reviewer issue>"` from the project root; Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
