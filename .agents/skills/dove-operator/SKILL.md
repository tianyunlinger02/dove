...
4. This request has one listed project action: `node ./bin/dove.mjs operator . --confirmed` from the project root; Run it only when the needed material is present; then summarize the real artifact state or material boundary instead of inspecting internal files directly.
5. Run operator only after approval; if no real result or safe built-in step exists, report the required material instead of claiming progress.
