...
- **The user requests drafting, assessment, or revision of ordinary project text or artifacts.**
  ...
  3. Use host tools (work; artifact-editing). Read the target and relevant project material, then draft, assess, create, or revise the ordinary artifact with host editing tools when the requested deliverable requires it.
  4. Use host tools (read-only; artifact-validation). Run the checks needed for the requested artifact. If checks reveal in-scope fixable issues, route back to artifact editing before the final response; report only remaining issues that materially affect the artifact, exceed scope, or require user judgment. This step is read-only; do not create or modify files.
