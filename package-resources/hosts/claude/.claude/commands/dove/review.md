...
- For independent Reviewer Handoff, use a true isolated Claude Code Agent context only if the host actually provides it; preserve that Reviewer context across re-review rounds and give it only the frozen handoff materials. If unavailable, report the boundary rather than using the author context.
