...
The project has a current research mainline confirmed by the user. The overview may include one ordinary top-level line `Mainline: <text>` near its beginning as the statusline source, not a heading, list, quote, code block, frontmatter, or schema. Research maintains this concise line only on first user confirmation or a user-confirmed material mainline change, and only when overview maintenance is already warranted. Missing lines, routine invocations, local method changes, and provisional results do not trigger backfilling. Status remains read-only: it reports explicit decisions and conflicts without inferring the mainline from a Git branch or latest Mission, Run, or Review.

Update, read-only SessionStart, reinstall, and uninstall preserve existing `.dove/research/**`, `.dove/reviews/**`, and `.dove/runs/**` content. Fresh bootstrap remains unchanged; existing research trees are not migrated for display. Research Markdown is not package-managed integration.
...
When `.claude/settings.json` has no existing `statusLine`, Claude integration installs the read-only `dove hook statusline` display. The first line shows Claude Code's native model, total context capacity, remaining-context percentage, the mainline text in cyan without a label, Git branch, and current-session duration; the second line contains only the absolute project path. Only a nonempty `NO_COLOR` disables color. The only research read is the root `.dove/research/RESEARCH.md`, limited to 64 KiB, for its unique ordinary `Mainline: <text>` line. Missing, empty, duplicate, oversized, or unreadable sources omit the mainline, without guessing from other prose or records. No research-tree scan, Mission, Review, Run, or transcript read is used; the branch is not a confirmed scientific mainline and session duration is not cumulative research time. An existing user or Trellis status line remains user-owned and is not replaced or adopted. `UserPromptSubmit` and intake remain retired with no replacement per-prompt hook. See [Installation](INSTALL.md) for exact ownership rules.
