...
For a normal non-slash request, the host handles general routing. The installed `UserPromptSubmit` hook uses a conservative research-relatedness gate only to decide whether to inject hidden `dove-intake`; it does not select a Skill, authorize execution or writes, or decide completion. Intake is a thin zero-write bridge and the Dove model decides whether to answer, clarify, or use one or more optional capabilities. Before hook output, the user-installed CLI may independently hot-sync package-managed project integration; this lifecycle bridge never touches `.dove/research/` and does not imply same-session host reload.

A research-related judgment prompt may receive Dove context, but the gate does not decide whether work follows. The Dove model weighs evidence, task risk, user preference, and the research mainline, then answers directly, asks one consequential clarification, or performs authorized useful work. Bare follow-ups such as “继续” or “下一步” are resolved from the current conversation by the host and do not receive a special Dove route without research context.
