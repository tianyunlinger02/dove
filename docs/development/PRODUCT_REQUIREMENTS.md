...
- 区分支持、反驳、证据不足和当前对照无法识别；不得无限补查来推迟接受反证，新方向成功不得覆盖原命题失败。承认事实上的否定不需要批准，改变确认主线或完成含义才需要用户决策。
...
- 评价链须从实际输入、输出、评价依据和指标追溯到比较，检查 reference/proxy 适用性、失败与排除的分母处理，以及适用的配对、不确定性、相关性和最终测试独立性。消融依据实际代码、配置和输出识别混杂，区分整套实现、组件增益与机制证据；无法识别贡献时修最小必要对照或限定结论，不以更多运行替代。
