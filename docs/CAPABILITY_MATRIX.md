...
|---|---|---|---|
| Packaged workflow surface | Implemented | neutral core + generated optional host adapters | `src/core/command-manifest.mjs` is the canonical command inventory and includes daily-use UX metadata; generated OpenCode, Claude Code, Codex, Cursor, and shared agent-skill adapters present the same top-level Dove commands with daily flow before guardrails. |
| Task-centered Dove kernel | Implemented | `.dove/state.json`, `.dove/task-packets/*`, `.dove/runtime/results.json`, `project:dove.init`, `project:dove.mission`, `project:dove.auto`, `project:dove.status`, `project:dove.operator` | One level-0 init goal anchors classified tasks for paper, experiment, and engineering work; mission converts natural-language demand into a task contract, requires interactive approval when the host supports it, materializes the approved task/checklist, then records one bounded foreground pass. Completed plan passes convert supplied plan outputs into pending level-3+ missions. Any system checklist/subtask children must be deeper than their parent mission. |
...
| Query/navigation workflow UX | Implemented | `project:dove.status`, `query_dove_status`, `apply_dove_status_adjustments` | Users inspect the live development situation from host-visible context; status is read-only by default and only displays non-init missions that are eligible for adjustment, excluding `completed` and `killed`. It omits mission counts and completed/killed recaps, and one confirmed status dialog can adjust displayed missions with machine choices `pending`, `ready`, `in-progress`, `blocked`, `completed`, or `killed`. |
| Explicit bounded auto path | Implemented | `project:dove.auto`, `run_dove_auto`, `.dove/runtime/*`, `.dove/state.json.settings.auto.maxIterations` | Auto uses the same demand-to-task intake as mission, can start directly from a new demand or existing task, returns a proposal-only `proposedTask`/`selectedTask` confirmation contract, requires explicit confirmation, runs foreground-only bounded iterations, records per-iteration runtime summaries, and stops at explicit boundaries. It does not continue as a background daemon after a response ends. |
| Operator queue pass | Implemented | `project:dove.operator`, `run_dove_operator`, `.dove/runtime/results.json`, `.dove/task-packets/*` | Operator previews auto-runnable, host-pass-required, blocked, and pending queues; confirmed foreground passes run safe internal steps or record real host-supplied task results without pretending work happened, and blocked missions create pending blocker-investigation plan missions. |
...
> a host-neutral, task-centered, evidence-aware workflow system with optional multi-host adapters, demand-to-task mission contracts, completed-plan-to-mission conversion, confirmed status adjustment UX, foreground operator queue passes, first-class paper/experiment/engineering domains, authoritative `.dove/` state, planner/builder/reviewer separation, durable review/rebuttal/experience/version/figure workflows, explicit lessons, one-pass mission execution, and bounded foreground auto execution.
