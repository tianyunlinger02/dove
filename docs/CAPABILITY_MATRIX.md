...
| Review + revision loop | Implemented | `src/core/reviews.mjs`, `project:dove.review`, `project:dove.review-loop`, `.dove/revision-plans` | Produces durable local findings and action items by inspecting claims, sources, notes, drafts, experiments, figures, and concerns; the top-level loop starts with local evidence-aware review before optional draft or experience substeps. |
