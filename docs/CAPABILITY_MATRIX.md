...
> a host-neutral, task-centered, evidence-aware workflow system with optional multi-host adapters, demand-to-task mission contracts, completed-plan-to-mission conversion, status daily home screen UX, compact confirmation cards, foreground operator queue passes, workflow-goal validation gates, first-class boundaries and role handoffs, append-only runtime event/result logs, first-class paper/experiment/engineering domains, authoritative `.dove/` state, planner/builder/reviewer separation, durable review/rebuttal/experience/version/figure workflows, explicit lessons, one-pass mission execution, and bounded foreground auto execution.
