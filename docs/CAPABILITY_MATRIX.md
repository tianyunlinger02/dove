...
- stop when further action is unlikely to resolve a material uncertainty.

## Skills

| Skill | Status | Boundary |
|---|---|---|
| `dove.research` | Implemented | Completes one bounded research or project pass; judgment follow-ups answer directly, and Markdown is maintained only for explicit record/update/save requests or clear mainline, conclusion, decision, or priority changes. |
...
| `dove.experiment` | Implemented | Handles design, requested execution, existing-result analysis, and retrospective records; judgment follow-ups stay advisory, and experiment Markdown is maintained when explicitly requested or when results change a research decision. |
...
| `dove.rebuttal` | Implemented | Keeps response and revision on the author side. |
| `dove.lessons` | Implemented | Reads `lessons/LESSONS.md` and relevant linked themes, or naturally maintains a researcher-owned Lesson and its summary. Built-in themes remain package-managed; all Lessons are advisory and source explanation is optional. |
...
| Separate reviewer session | User-managed | The user chooses and starts a read-only reviewer session or person within the declared artifact scope. |
...
| Claude project initialization | Implemented | Installs generated Claude resources, the Dove agent, prompt and stop hooks, current installation metadata, and the default research Markdown tree. |
...
| Integration update | Implemented | Refreshes recognized integration, creates missing summaries, completes current standard navigation in `RESEARCH.md` and `lessons/LESSONS.md`, and replaces all six package-managed built-in Lessons themes with current package content. Other research documents remain researcher-owned. |
| Deprecated Lessons cleanup | Direct | Update deletes `.dove/research/LESSONS.md` and `lessons/additional-lessons.md` without migration or fallback and removes the Additional migrated Lessons link. |
