...
|---|---|---|---|
| Packaged workflow surface | Implemented | neutral core + generated optional host adapters | `src/core/command-manifest.mjs` is the canonical command inventory; OpenCode is the default adapter; Claude Code, Codex, Cursor, and shared agent-skill adapter surfaces receive the same generic and paper-domain Dove commands as Dove-only adapter files, not repository-local development scaffolding. |
