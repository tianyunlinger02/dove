...
- Explicit update replaces local edits to manifest-owned integration with a notice. SessionStart skips those edits and syncs the rest with a `systemMessage` reminder. Neither rewrites research records.
- `UserPromptSubmit` and intake are retired without a replacement per-prompt hook. Dove does not install or manage `statusLine`; the old helper is available only for user-owned composition scripts.
