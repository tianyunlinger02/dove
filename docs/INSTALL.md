...
- `.dove/install/manifest.json` and the full quality reference `.dove/install/RESEARCH_QUALITY.md`, shared by the selected hosts;
...
Bare `dove` is the project lifecycle menu. In an unconfigured interactive project it offers Install and then a Claude Code / DeepSeek Harness checkbox menu, with Claude selected initially and at least one platform required. Setup distinguishes `uninitialized`, `current`, `needs-update`, and `blocked`; initialized projects expose safe update or platform changes when applicable, Complete Reinstall, uninstall, details, and exit. If valid manifest-owned integration needs refreshing, the first screen gives a concise explanation and offers explicit update and details; it does not dump the full Doctor panel or silently overwrite files. Unsupported or ambiguous state offers diagnosis rather than adoption or repair. Complete Reinstall and uninstall show their real scope and require an explicit default-No confirmation, while `.dove/research/**`, `.dove/reviews/**`, `.dove/runs/**`, and `DOCTOR.md` remain preserved. `NO_COLOR=1` disables ANSI styling, redirected or piped output is clean text without the mascot, and `--json` or `--format json` emits only the direct machine-readable integration result.
...
The Claude rule and standalone DSH Skills carry a short core, not the full quality criteria. Before substantive grading, consequential route/investment decisions, experiment scale-up, core or submission conclusions, and decision-changing counterevidence, task changes, or cross-dimensional tradeoffs, Dove proactively reads `.dove/install/RESEARCH_QUALITY.md`. It selects relevant criteria, reuses already-read guidance, and reports unavailable guidance without inventing grades. This shared package resource is separate from optional researcher-owned Lessons and grants no permissions. Isolated review receives its own canonical package copy, not access to this author-side path.
...
`SessionStart` uses the user-installed `dove` on `PATH` for read-only project integration inspection. It never writes integration, research, review or run files, shared configuration, or cleanup state. Non-current integration may produce a `systemMessage` suggesting explicit `dove update`; unsupported or ambiguous state requires diagnosis. Startup/clear returns no output when there is no warning. Inspection does not reload already-active Claude context.
...
Current Dove resources install no `UserPromptSubmit` hook. During explicit update, reinstall, or uninstall, a manifest-owned exact old Dove `UserPromptSubmit` hook is removed while unrelated user or Trellis prompt hooks are preserved.

Dove does not install or expose a Claude `Stop` hook. If `.claude/settings.json` has no existing `statusLine`, Claude integration installs the manifest-owned read-only command `dove hook statusline --project "$CLAUDE_PROJECT_DIR"`. Its first line displays host model/context facts, the optional mainline text in cyan without a label, Git branch, and current-session duration; its second line is only the absolute project path. Only a nonempty `NO_COLOR` disables color. It reads only the root `.dove/research/RESEARCH.md` within 64 KiB for one ordinary `Mainline: <text>` line, omitting unavailable or ambiguous mainlines without guessing or backfilling. It does not scan research documents or read Mission, Review, Run, or transcripts; the branch is not a confirmed scientific mainline and session duration is not cumulative research time. See [Usage](USAGE.md#research-markdown) for the optional line's maintenance boundary. A pre-existing user or Trellis status line is preserved and not adopted, and therefore is not recorded in Dove's manifest. Explicit update/reinstall may restore a locally edited Dove-owned line and report the replacement; host removal or uninstall removes a still-owned line while preserving a user-modified one. Old Stop entries may be identified read-only, but they are not part of supported managed ownership and remain untouched.
...
Within `.dove/install/`, Dove manages its manifest and the exact `RESEARCH_QUALITY.md` resource, not the whole directory. The manifest records initialized hosts and the integration contract needed by `update` and `doctor`. Other files, including optional researcher feedback `DOCTOR.md`, are not package-owned.
...
`dove update` reads `.dove/install/manifest.json` and refreshes only the hosts already recorded there. For valid manifest-owned Dove integration resources, explicit update replaces local edits, names them in the human output, and reports them as `replacedLocalEdits` in JSON. This does not authorize replacing unowned files or unrelated configuration; use user-owned locations for customizations you need to keep.

If the manifest is absent, malformed, unsupported, contradictory, unsafe, or contains unknown ownership state, `dove update` fails closed. It must not reconstruct a manifest from generated adapters, hooks, MCP declarations, status-line settings, or copied runtime.

There is no adoption or revision-1.0 migration path. Update and Complete Reinstall require a valid current installation manifest and never clean up unknown installation files or legacy directories. For an already initialized project, update refreshes only package-managed integration. It does not create missing summaries, complete navigation, replace Lessons materials, or rewrite `.dove/research/**`; existing research documents remain researcher-owned.

## Uninstall

Run `dove uninstall` from an initialized project. Dove first shows the exact removal scope and defaults to No. After confirmation it removes manifest-owned host files, the Dove SessionStart hook, retired exact Dove prompt hook entries when recorded by the manifest, permission and MCP fragments, `.dove/install/manifest.json`, and the manifest-owned `.dove/install/RESEARCH_QUALITY.md` reference. It preserves `.dove/research/**`, `.dove/reviews/**`, `.dove/runs/**`, `.dove/install/DOCTOR.md`, all legacy markers and unknown installation files, unrelated host settings, unrelated hooks, unrelated MCP servers, any pre-existing user-owned `permissions.deny` entry for `WebFetch`, and ordinary project files. If no conflicting installation residue remains, a project retaining only researcher-owned context is then classified as uninitialized rather than offered an update. Preserved unknown files under `.dove/install/` or an unsupported legacy installation directory can still leave setup blocked; uninstall does not delete them to force an uninitialized state. Drift, symlinks, or changed transaction preconditions block replacement; caught promotion failures trigger rollback. If restoration fails, Dove keeps the transaction root and backups and reports their absolute paths. This does not promise crash recovery or filesystem-wide atomicity.
...
Doctor is read-only. Project integration has four states: `uninitialized` (no installation manifest), `current` (managed integration matches), `needs-update` (valid ownership with refreshable differences), and `blocked` (invalid, unsupported, or ambiguous integration). These are software states, not research grades or a migration workflow. Doctor reports separate static dimensions rather than collapsing them into one package check:

- **user CLI** — the current program's package version and whether its runtime files are present;
- **project integration** — the project integration package version recorded in `.dove/install/manifest.json`, separately from the validity and currentness of managed resources, paths needing explicit update, and manifest-owned local edits update would replace; a missing recorded version stays unknown, not filled from the program version; and
...
- `.dove/runs/` holds local execution receipts for Dove run supervisors and is preserved across update, read-only SessionStart, reinstall, and uninstall.
