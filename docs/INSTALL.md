...
5. let only explicit schema 8 init, mission, receipt, and domain workflows create the state they own
...
# Install every safe adapter surface, including user-level Claude Code commands and commands
npx dove install . --force --host all

# Sync only the user-level Claude Code /dove:* commands and commands
...
The doctor command is read-only. It checks installed runtime/adapters and reports a healthy schema 8 workspace, an absent workspace, or a legacy/invalid workspace that requires explicit archive-reset, without creating or refreshing `.dove/` artifacts. For Claude it checks only whether the generated command adapters are present; settings and shell configuration are outside doctor health.
...
Adapter copying is allowlisted to Dove project surfaces only: `.opencode/commands/dove*.md`, `.opencode/skills/dove-*`, `.cursor/commands/dove-*.md`, `.codex/skills/dove-*`, `.agents/skills/dove-*`, `.opencode.json`, and the Dove-native `AGENTS.md`. The `claude` host is user-level: `install`/`sync` render only the generated commands to `~/.claude/commands/dove` (or `DOVE_CLAUDE_CONFIG_DIR` for isolated validation) without copying `.claude/commands/dove` into the target project or touching any other Claude or shell configuration. Local development scaffolding, host settings, and project-local `.claude/commands/dove` files are not installed as Dove product surfaces. Managed package paths are declared by the command manifest; `.dove/` is outside that managed install boundary.
