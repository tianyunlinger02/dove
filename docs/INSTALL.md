...
Dove does not install or expose a Claude `Stop` hook. If `.claude/settings.json` has no existing `statusLine`, Claude integration installs the manifest-owned read-only command `dove hook statusline --project "$CLAUDE_PROJECT_DIR"`. It preserves the absolute project path and displays the host-provided model, total context capacity, remaining-context percentage and current-session duration, plus the Git branch. It reads no research Markdown, Review, Run, or transcript content; the branch is not a confirmed scientific mainline and session duration is not cumulative research time. A pre-existing user or Trellis status line is preserved and not adopted, and therefore is not recorded in Dove's manifest. Explicit update/reinstall may restore a locally edited Dove-owned line and report the replacement; host removal or uninstall removes a still-owned line while preserving a user-modified one. Old Stop entries may be identified read-only, but they are not part of supported managed ownership and remain untouched.
